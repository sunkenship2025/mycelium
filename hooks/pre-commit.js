...
const ErrorMessages = {
  AVOID_DIRECT_IMPORT_FROM_PACKAGES: [
    ...
    `import { your_import1 } from "@myceliumhq/common-all/path1";`,
    `import { your_import2 } from "@myceliumhq/common-server/path2";`,
    `While the above imports should look as:`,
    `import { your_import1 } from "@myceliumhq/common-all";`,
    `import { your_import2 } from "@myceliumhq/common-server";`,
  ].join("\n"),
};
  ...
  return checkToken({
    ...
    forbiddenTokens: {
      ...
      [ErrorMessages.AVOID_DIRECT_IMPORT_FROM_PACKAGES]: {
        rgx: /import.*((common-frontend|common-all|common-server|engine-server|mycelium-cli|pods-core|api-server|common-test-utils|engine-test-utils)\/)/,
        fileRgx: /\.ts[x]?$/,
      },
    },
  });
