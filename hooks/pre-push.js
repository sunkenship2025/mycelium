...
function main() {
  ...
  checkCircularDependencies(
    path.resolve(rootPath, "packages/mycelium-cli/src"),
  ...
  checkCircularDependencies(
    path.resolve(rootPath, "packages/mycelium-plugin-views/src"),
    PLUGIN_VIEWS_CIRCULAR_DEP_THRESHOLD
  );

  checkCircularDependencies(
    path.resolve(rootPath, "packages/mycelium-viz/src"),
  ...
  return checkToken({
    ...
    forbiddenTokens: {
      ...
      "rel import of monorepo pkg": {
        rgx: /(\.\.\/(common-frontend|common-all|common-server|engine-server|mycelium-cli|pods-core|api-server|common-test-utils|engine-test-utils))/,
        fileRgx: /\.ts[x]?$/,
      },
    },
  });
