...
type Metadata = Partial<{
  /**
   * When was mycelium first installed
   */
  firstInstall: number;
  /**
  ...
  /**
   * Set if a user has activated a mycelium workspace
   */
  myceliumWorkspaceActivated: number;
  /**
  ...
  /**
   * The most recently opened Mycelium workspaces
   */
...
export class MetadataService {
  ...
  static metaFilePath() {
    return path.join(os.homedir(), FOLDERS.MYCELIUM_SYSTEM_ROOT, "meta.json");
  }
  ...
  setMyceliumWorkspaceActivated() {
    this.setMeta("myceliumWorkspaceActivated", Time.now().toSeconds());
  }
