...
export type WorkspaceServiceCreateOpts = {
  ...
  /**
   * create mycelium.code-workspace file
   */
...
const ROOT_NOTE_TEMPLATE = [
  "# Welcome to Mycelium",
  "",
  `This is the root of your mycelium vault. If you decide to publish your entire vault, this will be your landing page. You are free to customize any part of this page except the frontmatter on top.`,
  "",
  "## Lookup",
  "",
  "This section contains useful links to related resources.",
  "",
  "- [Getting Started Guide](https://link.mycelium.so/6b25)",
  "- [Discord](https://link.mycelium.so/6b23)",
  "- [Home Page](https://wiki.mycelium.so/)",
  "- [Github](https://link.mycelium.so/6b24)",
  "- [Developer Docs](https://docs.mycelium.so/)",
].join("\n");
...
export class WorkspaceService implements Disposable, IWorkspaceService {
  ...
  static async isWorkspaceVault(fpath: string) {
    return (
      // Config file exists
      (await fs.pathExists(path.join(fpath, CONSTANTS.MYCELIUM_CONFIG_FILE))) &&
  ...
  get user(): DUser {
    const fpath = path.join(this.wsRoot, CONSTANTS.MYCELIUM_USER_FILE);
  ...
  async setConfig(config: MyceliumConfig) {
  ...
  setCodeWorkspaceSettingsSync(config: WorkspaceSettings) {
    writeJSONWithCommentsSync(
      path.join(this.wsRoot, CONSTANTS.MYCELIUM_WS_NAME),
      config
    );
  }
   ...
   * @param opts.config - if passed it, make modifications on passed in config instead of {wsRoot}/mycelium.yml
   * @param opts.updateConfig - default: true, add to mycelium.yml
   * @param opts.updateWorkspace - default: false, add to mycelium.code-workspace. Make sure to keep false for Native workspaces.
   * @returns
   */
  async addVault(
    opts: {
    ...
    if (updateWorkspace) {
      const wsPath = path.join(this.wsRoot, MYCELIUM_WS_NAME);
   ...
   * @param opts.noAddToConfig: don't add to mycelium.yml
   * @param opts.addToCodeWorkspace: add to mycelium.code-workspace
   * @returns void
   *
   * Effects:
   *   - updates `mycelium.yml` if `noAddToConfig` is not set
   *   - create directory
  ...
  async createSelfContainedVault(opts: {
    ...
    if (opts.newVault) {
      ...
      // create mycelium.code-workspace
      WorkspaceConfig.write(vaultPath, [], {
        overrides: {
          ...
          settings: {
            ...
            [MYCELIUM_VSCODE_CONFIG_KEYS.ENABLE_SELF_CONTAINED_VAULTS_WORKSPACE]:
              true,
          },
        },
      });
      // Also add a gitignore, so files like `.mycelium.port` are ignored if the
  ...
  async migrateVaultToSelfContained({ vault }: { vault: DVault }) {
    ...
    const config = DConfig.getRaw(this.wsRoot) as MyceliumConfig;
    ...
    // Create or update the config file (mycelium.yml) inside the wsRoot/vault
    if (
      !(await fs.pathExists(
        path.join(oldFolder, CONSTANTS.MYCELIUM_CONFIG_FILE)
      ))
    ) {
    ...
    // Create or update the workspace file (mycelium.code-workspace) inside the wsRoot/vault
    if (
      !(await fs.pathExists(path.join(oldFolder, CONSTANTS.MYCELIUM_WS_NAME)))
    ) {
  ...
  async convertVaultLocal({
    ...
    // Update `mycelium.yml`, removing the remote from the converted vault
    const config = this.config;
  ...
  private static async generateCommitMessage({
    ...
    return [
      "Mycelium workspace sync",
      "",
      "## Synced vaults:",
      ...vaults.map((vault) => `- ${VaultUtils.getName(vault)}`),
      "",
      `Mycelium version: ${version}`,
   ...
   *   - root.schema.yml
   *
   * NOTE: mycelium.yml only gets created if you are adding a workspace...
   * @param opts
   */
  ...
  async cloneVaultWithAccessToken(opts: { vault: DVault }) {
    ...
    if (!vault.remote || vault.remote.type !== "git") {
      throw new MyceliumError({ message: "cloning non-git vault" });
    }
  ...
  async pullVault(opts: { vault: DVault }) {
    ...
    const wsRoot = this.wsRoot;
    if (!vault.remote || vault.remote.type !== "git") {
      throw new MyceliumError({ message: "pulling non-git vault" });
    }
  ...
  async runMigrationsIfNecessary({
    ...
    if (
      ...
      if (!_.isEmpty(changes)) {
        ...
        myceliumConfig = data.myceliumConfig;
      }
    }
  ...
  writePort(port: number) {
    const wsRoot = this.wsRoot;
    // mycelium-cli can overwrite port file. anything that needs the port should connect to `portFilePathExtension`
