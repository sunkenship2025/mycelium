...
export class WorkspaceUtils {
  ...
  static async getCodeWorkspaceSettings(
    ...
    const wsConfigPath = path.join(wsRoot, CONSTANTS.MYCELIUM_WS_NAME);
  ...
  static async getWorkspaceType({
    ...
    if (
      ...
      path.basename(workspaceFile.fsPath) === CONSTANTS.MYCELIUM_WS_NAME
    ) {
  ...
  static async getWorkspaceTypeFromDir(dir: string) {
    if (fs.pathExistsSync(path.join(dir, CONSTANTS.MYCELIUM_WS_NAME))) {
      return WorkspaceType.CODE;
    }
    const wsRoot = await findDownTo({
      base: dir,
      fname: CONSTANTS.MYCELIUM_CONFIG_FILE,
      returnDirPath: true,
    });
    if (!wsRoot) return WorkspaceType.NONE;
    if (fs.pathExistsSync(path.join(wsRoot, CONSTANTS.MYCELIUM_CONFIG_FILE))) {
  ...
  static async findWSRootsInWorkspaceFolders(
    ...
    const myceliumWorkspaceFolders = await Promise.all(
      folders.map((folder) =>
        findDownTo({
          base: folder,
          fname: CONSTANTS.MYCELIUM_CONFIG_FILE,
    ...
    return myceliumWorkspaceFolders.filter(isNotUndefined);
  }

  /**
   * Check if a file is a mycelium note (vs a regular file or something else entirely)
   */
  static async isMyceliumNote({
    wsRoot,
    vaults,
    fpath,
    ...
    // if markdown file, check if it is in a mycelium vault
