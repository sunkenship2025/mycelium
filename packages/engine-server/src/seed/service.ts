...
import { DConfig, simpleGit, writeYAML } from "@myceliumhq/common-server";
...
export class SeedService {
  ...
  protected async getSeedOrErrorFromId(
    id: string
  ): Promise<SeedConfig | MyceliumError> {
    const maybeSeed = await this.registry.info({ id });
    if (!maybeSeed) {
      return MyceliumError.createFromStatus({
  ...
  async addSeed({
    ...
    if (seedOrError instanceof MyceliumError) {
      return {
        error: seedOrError,
      };
    }
  ...
  async init(opts: { seed: SeedConfig; wsRoot: string; mode: SeedInitMode }) {
    ...
    const cpath = path.join(wsRoot, CONSTANTS.MYCELIUM_SEED_CONFIG);
    ...
    if (!_.has(seeds, id)) {
      return {
        error: new MyceliumError({
          status: ERROR_STATUS.DOES_NOT_EXIST,
          message: `seed with id ${id} not in mycelium.yml`,
        }),
      };
    }
    const seedOrError = await this.getSeedOrErrorFromId(id);
