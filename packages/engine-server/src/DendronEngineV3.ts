...
export class MyceliumEngineV3 extends EngineV3Base implements DEngine {
  ...
  constructor(props: MyceliumEngineOptsV3) {
    super(props);
  ...
  async init(): Promise<DEngineInitResp> {
    ...
    try {
      ...
      if (_.isUndefined(schemas)) {
        return {
          data: defaultResp,
          error: MyceliumError.createFromStatus({
      ...
      const hookErrors: IMyceliumError[] = [];
      ...
      let error: IMyceliumError | undefined;
      switch (_.size(allErrors)) {
        case 0: {
          break;
        }
        case 1: {
          error = new MyceliumError(allErrors[0]);
          break;
        }
        default:
          error = new MyceliumCompositeError(allErrors);
      }
      this.logger.info({
        ctx: "init:ext",
        error,
      ...
      return {
        data: defaultResp,
        error: MyceliumError.createPlainError({
          payload,
          message,
          status,
          severity: ERROR_SEVERITY.FATAL,
  ...
  async info(): Promise<EngineInfoResp> {
    ...
    if (!version) {
      return {
        error: MyceliumError.createPlainError({
          message: "Unable to read Mycelium version",
        }),
      };
    }
    return {
  ...
  async renderNote({
    ...
    try {
      data = await this._renderNote({
        ...
        dest: dest || MyceliumASTDest.HTML,
      });
    } catch (error) {
      return {
        error: new MyceliumError({
  ...
  async getDecorations(opts: GetDecorationsOpts): Promise<GetDecorationsResp> {
    ...
    try {
      ...
      if (errors && errors.length > 1)
        error = new MyceliumCompositeError(errors);
  ...
  private async initSchema(): Promise<RespWithOptError<SchemaModuleProps[]>> {
      ...
      await Promise.all(
        (this.vaults as DVault[]).map(async (vault) => {
          ...
          if (maybeFiles.error || maybeFiles.data.length === 0) {
            ...
            return {
              error: new MyceliumCompositeError([
                new MyceliumError({
          ...
          return {
            ...
            error: _.isNull(errors)
              ? undefined
              : new MyceliumCompositeError(errors),
          };
        })
      );
    ...
    return {
      error: errors.length > 0 ? new MyceliumCompositeError(errors) : undefined,
  ...
  private async initNotes(
    ...
    const allNotesList = await Promise.all(
      this.vaults.map(async (vault) => {
        ...
        const cachePath = path.join(vpath, CONSTANTS.MYCELIUM_CACHE_FILE);
    ...
    return {
      ...
      error:
        errors.length === 0 ? undefined : new MyceliumCompositeError(errors),
    };
  }
    ...
    return _.map(noteDicts.notesById, (noteFrom: NoteProps) => {
      try {
        if (
          ...
          (maxNoteLength || CONSTANTS.MYCELIUM_DEFAULT_MAX_NOTE_LENGTH)
        ) {
...
export const createEngineV3 = ({ wsRoot }: WorkspaceOpts) => {
  const engine = MyceliumEngineV3.create({ wsRoot });
