...
import { createLogger, DConfig, DLogger } from "@myceliumhq/common-server";
...
type MyceliumEngineClientOpts = {
  vaults: DVault[];
  ws: string;
};

export class MyceliumEngineClient implements DEngineClient, EngineEventEmitter {
  ...
  static create({
    ...
    return new MyceliumEngineClient({ api, vaults, ws, history });
  }
  ...
  static getPort({ wsRoot }: { wsRoot: string }): number {
    ...
    if (!fs.pathExistsSync(portFile)) {
      throw new MyceliumError({ message: "no port file" });
    }
  ...
  async init(): Promise<DEngineInitResp> {
    ...
    if (!resp.data) {
      throw new MyceliumError({ message: "no data" });
    }
  ...
  async deleteSchema(
    ...
    if (!resp?.data?.notes) {
      throw new MyceliumError({ message: "bad delete operation" });
    }
  ...
  async sync(): Promise<DEngineInitResp> {
    ...
    if (!resp.data) {
      throw new MyceliumError({ message: "no data", payload: resp });
    }
    const { notes, config } = resp.data;
    this.notes = notes;
    this.noteFnames = NoteFnameDictUtils.createNotePropsByFnameDict(this.notes);
