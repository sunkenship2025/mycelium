...
import { DConfig, DLogger, vault2Path } from "@myceliumhq/common-server";
...
// NOTE: This file has been forked in plugin-core to enable Mycelium Web
// Extension
...
export class NoteParserV2 {
  ...
  async parseFiles(
  ...
  ): Promise<{ noteDicts: NoteDicts; errors: IMyceliumError[] }> {
    ...
    const errors: IMyceliumError<any>[] = [];
    ...
    if (_.isUndefined(fileMetaDict[1])) {
      return {
        noteDicts,
        errors: [
          MyceliumError.createFromStatus({
      ...
      await asyncLoopOneAtATime(
        ...
        async (ent) => {
          try {
            ...
            const myceliumError = ErrorFactory.wrapIfNeeded(err);
            // A fatal error would kill the initialization
            myceliumError.severity = ERROR_SEVERITY.MINOR;
            myceliumError.message =
              `Failed to read ${ent.fpath} in ${vault.fsPath}: ` +
              myceliumError.message;
            errors.push(myceliumError);
          }
        }
      );
      lvl += 1;
  ...
  private async parseNoteProps(opts: {
    ...
    try {
      ...
      if (!ErrorUtils.isMyceliumError(_err)) {
  ...
  private async file2NoteWithCache({
    ...
    let error: IMyceliumError | null = null;
    try {
