...
export class FileStorage implements DStore {
  ...
  async init(): Promise<StoreV2InitResp> {
    ...
    try {
      ...
      let error: IMyceliumError | undefined = errors[0];
      if (errors.length > 1) {
  ...
  async initSchema(): Promise<DEngineInitSchemaResp> {
    ...
    const result = {
      ...
      error: _.isEmpty(errors)
        ? undefined
        : new MyceliumError({ message: "multiple errors", payload: errors }),
    };
    return result;
  }
  ...
  async renameNote(opts: RenameNoteOpts): Promise<NoteChangeEntry[]> {
    ...
    if (!oldVault) {
      throw new MyceliumError({ message: "vault not set for loation" });
    }
    ...
    if (ErrorUtils.isErrorResp(resp)) {
      throw new MyceliumError({ message: "file not found" });
    }
