...
export class DoctorService implements Disposable {
    ...
    switch (action) {
      case DoctorActionsEnum.REMOVE_DEPRECATED_CONFIGS: {
        ...
        if (pathsToDelete.length > 0) {
          ...
          if (backupPath instanceof MyceliumError) {
            return {
              exit: true,
              error: backupPath,
            };
      ...
      case DoctorActionsEnum.FIX_REMOTE_VAULTS: {
        ...
        if (vaultsToFix.length > 0) {
          ...
          if (out instanceof MyceliumError) {
            return {
              exit: true,
              error: out,
            };
      ...
      default:
        throw new MyceliumError({
          message:
            "Unexpected Doctor action. If this is something Mycelium should support, please create an issue on our Github repository.",
        });
    }
  ...
  /** Returns the path for the backup if it was able to create one, or a MyceliumError if one occurred during backup. */
  async createBackup(
    wsRoot: string,
    backupInfix: string
  ): Promise<string | MyceliumError> {
    try {
