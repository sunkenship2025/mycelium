...
import { MyceliumEngineClient } from "../engineClient";
import {
  EngineUtils,
  getWSMetaFilePath,
  openPortFile,
...
export class EngineConnector {
  /**
   * Conencts to the {@link MyceliumEngine}
   *
   * @remarks
  ...
  static instance() {
    if (!this._ENGINE_CONNECTOR) {
      throw new MyceliumError({ message: "no workspace" });
    }
  ...
  async init(opts?: EngineConnectorInitOpts) {
    ...
    if (opts?.portOverride) {
      ...
      if (!engine) {
        throw new MyceliumError({ message: "error connecting" });
      }
  ...
  async tryToConnect({ port }: { port: number }) {
    ...
    const myceliumEngine = MyceliumEngineClient.create({
      port,
      ws: wsRoot,
      vaults,
      logger: this.logger,
    });
    const resp = await myceliumEngine.info();
    if (resp.error) {
      this.logger.info({ ctx, msg: "can't connect", error: resp.error });
      return false;
    } else {
      this.logger.info({ ctx, msg: "connected", info: resp.data });
      return myceliumEngine;
    }
  }
  ...
  private async _connect(opts: {
    wsRoot: string;
  }): Promise<false | { engine: MyceliumEngineClient; port: number }> {
