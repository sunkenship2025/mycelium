...
import { createDisposableLogger, DLogger } from "@myceliumhq/common-server";
...
export class MigrationService {
  static async applyMigrationRules({
    ...
    if (!_.isEmpty(changes)) {
      ...
      // TODO: this should only be set if the migration is backwards incompatible with previous mycelium versions
      ...
      await wsService.setConfig(data.myceliumConfig);
  ...
  static async collectMigrationChanges({
    ...
    await _.reduce(
      ...
      async (prev, change) => {
        ...
        const { myceliumConfig, wsConfig } = data;
        const out = await change.func({ myceliumConfig, wsConfig, wsService });
