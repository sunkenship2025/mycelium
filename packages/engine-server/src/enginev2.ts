...
type MyceliumEnginePropsV2 = Required<MyceliumEngineOptsV2>;
...
export class MyceliumEngineV2 implements DEngine {
  ...
  constructor(props: MyceliumEnginePropsV2) {
  ...
  /**
   * @deprecated
   * For accessing a specific note by id, see {@link MyceliumEngineV2.getNote}.
  ...
  /**
   * @deprecated see {@link MyceliumEngineV2.findNotes}
   */
  get noteFnames() {
  ...
  async init(): Promise<DEngineInitResp> {
    ...
    try {
      ...
      const hookErrors: MyceliumError[] = [];
  ...
  async renderNote({
    ...
    const ctx = "MyceliumEngineV2:renderNote";
  ...
  async renameNote(opts: RenameNoteOpts): Promise<RenameNoteResp> {
    try {
      ...
      let error = err;
      if (err instanceof MyceliumError) error = error2PlainObject(err);
...
export const createEngine = ({ wsRoot }: WorkspaceOpts) => {
  const engine = MyceliumEngineV2.create({ wsRoot });
