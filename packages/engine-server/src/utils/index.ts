...
export function refLink2Stringv2(opts: {
  ...
  const linkParts = [`![[`];
  if (useVaultPrefix) {
    linkParts.push(CONSTANTS.MYCELIUM_DELIMETER + link.from.vaultName! + "/");
  }
  linkParts.push(name);
  if (anchorStart) {
    if (rawAnchors) {
...
export function getWSMetaFilePath({ wsRoot }: { wsRoot: string }) {
  const fsPath = path.join(wsRoot, CONSTANTS.MYCELIUM_WS_META);
  return fsPath;
}
...
export function parseMyceliumRef(ref: string) {
...
export function parseFileLink(ref: string): DNoteRefLink {
  ...
  if (_.isUndefined(fname)) {
    throw new MyceliumError({ message: `fname for ${ref} is undefined` });
  }
...
export const getCachePath = (vpath: string): string => {
  return path.join(vpath, CONSTANTS.MYCELIUM_CACHE_FILE);
};
