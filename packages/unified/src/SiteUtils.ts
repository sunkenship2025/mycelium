...
export class SiteUtils {
  ...
  static create403StaticNote(opts: { vaults: DVault[] }) {
    return NoteUtils.create({
      ...
      body: [
        "[Mycelium](https://mycelium.so/) (the tool used to generate this site) lets authors selective publish content. You will see this page whenever you click on a link to an unpublished page",
        "",
  ...
  //   config: IntermediateMyceliumConfig;
  ...
  //         ConfigUtils.getSite(config) as MyceliumSiteConfig
  //       )
  ...
  //   // navOrder is the same order that is in mycelium.yml
  ...
  //     const siteFM = note.custom || ({} as MyceliumSiteFM);
  ...
  static getSitePrefixForNote(config: MyceliumConfig) {
  ...
  static validateConfig(sconfig: MyceliumPublishingConfig): {
