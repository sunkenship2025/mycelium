...
function runDecorator(
  ...
  const { node } = opts;
  switch (node.type) {
    case MyceliumASTTypes.BLOCK_ANCHOR:
    ...
    case MyceliumASTTypes.FRONTMATTER:
...
export async function runAllDecorators(
  ...
  const allErrors: IMyceliumError[] = [];
  ...
  for (const { range, text } of ranges) {
    ...
    await MdastUtils.visitAsync(tree, [], async (nodeIn) => {
      ...
      const node = nodeIn as NonOptional<MyceliumASTNode, "position">;
