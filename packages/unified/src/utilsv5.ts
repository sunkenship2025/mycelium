...
// @ts-ignore
import mermaid from "@myceliumhq/remark-mermaid";
...
import { myceliumHoverPreview } from "./remark/myceliumPreview";
import { myceliumPub, MyceliumPubOpts } from "./remark/myceliumPub";
...
import { MyceliumASTDest, UnistNode } from "./types";
...
export class MDUtilsV5 {
  ...
  static getProcOpts(proc: Processor): ProcOptsV5 {
    const _data = proc.data("myceliumProcOptsv5") as ProcOptsV5;
    return _data || {};
  }

  static getProcData(proc: Processor): ProcDataFullV5 {
    const _data = proc.data("myceliumProcDatav5") as ProcDataFullV5;
    return _data || {};
  }
  ...
  static setProcData(proc: Processor, opts: Partial<ProcDataFullV5>) {
    const _data = proc.data("myceliumProcDatav5") as ProcDataFullV5;
    return proc.data("myceliumProcDatav5", { ..._data, ...opts });
  }

  static setProcOpts(proc: Processor, opts: ProcOptsV5) {
    const _data = proc.data("myceliumProcOptsv5") as ProcOptsV5;
    return proc.data("myceliumProcOptsv5", { ..._data, ...opts });
  }
  ...
  static shouldApplyPublishingRules(proc: Processor): boolean {
    return (
      this.getProcData(proc).dest === MyceliumASTDest.HTML &&
  ...
  /**
   * Used for processing a Mycelium markdown note
   */
  static _procRemark(
    opts: ProcOptsV5,
    data: OptionalExceptFor<ProcDataFullOptsV5, "config">
  ) {
    const errors: MyceliumError[] = [];
      ...
      case ProcMode.FULL:
        {
          ...
          // NOTE: order matters. this needs to appear before `myceliumPub`
          if (data.dest === MyceliumASTDest.HTML) {
          ...
          // Add flavor specific plugins. These need to come before `myceliumPub`
          ...
          if (
            ...
            proc = proc.use(myceliumHoverPreview);
          }
  ...
  /**
   * Parse Mycelium Markdown Note. No compiler is attached.
   * @param opts
   * @param data
   * @returns
   */
  ...
  static procRemarkParseNoData(
    ...
    data: Partial<ProcDataFullOptsV5> & { dest: MyceliumASTDest }
  ) {
