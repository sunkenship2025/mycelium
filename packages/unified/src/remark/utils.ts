...
export function addError(proc: Processor, err: MyceliumError) {
  const errors = proc.data("errors") as MyceliumError[];
  errors.push(err);
...
export function getNoteOrError(
  notes: NoteProps[],
  hint: any
): { error: MyceliumError | undefined; note: undefined | NoteProps } {
  let error: MyceliumError | undefined;
  let note: NoteProps | undefined;
  if (_.isUndefined(notes)) {
    error = new MyceliumError({ message: `no note found. ${hint}` });
    return { error, note };
  }
  if (notes.length > 1) {
...
export class LinkUtils {
  static astType2DLinkType(type: MyceliumASTTypes): DLink["type"] {
    switch (type) {
      case MyceliumASTTypes.WIKI_LINK:
        return "wiki";
      case MyceliumASTTypes.REF_LINK_V2:
        return "ref";
      default:
        throw new MyceliumError({ message: `invalid type conversion: ${type}` });
    }
  }
   ...
   *
   * - type: filter by {@link MyceliumASTTypes}
  ...
  static findLinksFromBody({
    ...
    const remark = MDUtilsV5.procRemarkParseFull(
      { flavor: ProcFlavor.REGULAR },
      {
        ...
        fname: note.fname,
        vault: note.vault,
        dest: MyceliumASTDest.MD_MYCELIUM,
        config,
      }
    );
    const out = remark.parse(content) as MyceliumASTNode;
  ...
  static parseLinkV2(opts: {
    ...
    if (out && out.groups) {
      ...
      if (value) {
        // remove spaces
        value = _.trim(value);
        ({ vaultName, link: value } = parseMyceliumURI(value));
  ...
  static parseNoteRefRaw(ref: string): DNoteRefLinkRaw {
    ...
    let vaultName: string | undefined;
    ({ vaultName, link: ref } = parseMyceliumURI(ref));
  ...
  static renderNoteLink({
    ...
    switch (dest) {
      case MyceliumASTDest.MD_MYCELIUM: {
        ...
        const vaultPrefix =
          link.from.vaultName && link.data.xvault
            ? `${CONSTANTS.MYCELIUM_DELIMETER}${link.from.vaultName}/`
            : "";
  ...
  static async findLinkCandidates({
    ...
    const tree = remark.parse(content) as MyceliumASTNode;
...
export class AnchorUtils {
    ...
    visit(header, (node) => {
      switch (node.type) {
        ...
        case MyceliumASTTypes.USERTAG:
          ...
          break;
        case MyceliumASTTypes.INLINE_CODE:
  ...
  static headerTextPosition(header: Heading): Position {
    ...
    visit(
      ...
      [
        MyceliumASTTypes.TEXT,
        MyceliumASTTypes.WIKI_LINK,
        MyceliumASTTypes.HASHTAG,
        MyceliumASTTypes.BLOCK_ANCHOR,
      ],
      (node) => {
        if (node.type === MyceliumASTTypes.BLOCK_ANCHOR && end) {
  ...
  static anchorNode2anchor(
    ...
    if (node.type === MyceliumASTTypes.HEADING) {
    ...
    } else if (node.type === MyceliumASTTypes.BLOCK_ANCHOR) {
      return [
        `^${node.id}`,
        {
          type: "block",
...
const NODE_TYPES_TO_EXTRACT = [
  MyceliumASTTypes.BLOCK_ANCHOR,
  MyceliumASTTypes.HEADING,
  MyceliumASTTypes.LIST,
  MyceliumASTTypes.LIST_ITEM,
  MyceliumASTTypes.TABLE,
  MyceliumASTTypes.PARAGRAPH,
];

export class RemarkUtils {
  /**
   * Use this to [[Get the line offset of the frontmatter|mycelium://mycelium.docs/pkg.plugin-core.dev.cook#get-the-line-offset-of-the-frontmatter]]
   * Given a string representation of a Mycelium note,
  ...
  static findAnchors(content: string): Anchor[] {
    const parser = MDUtilsV5.procRemarkParseNoData(
      {},
      { dest: MyceliumASTDest.HTML }
    );
    const parsed = parser.parse(content);
    return [
      ...(selectAll(MyceliumASTTypes.HEADING, parsed) as Heading[]),
      ...(selectAll(MyceliumASTTypes.BLOCK_ANCHOR, parsed) as BlockAnchor[]),
    ];
  }
  ...
  static isRoot(node: Node): node is Parent {
    return node.type === MyceliumASTTypes.ROOT;
  }
  ...
  static isParagraph(node: Node): node is Paragraph {
    return node.type === MyceliumASTTypes.PARAGRAPH;
  }

  static isTable(node: Node): node is Table {
    return node.type === MyceliumASTTypes.TABLE;
  }

  static isTableRow(node: Node): node is TableRow {
    return node.type === MyceliumASTTypes.TABLE_ROW;
  }

  static isTableCell(node: Node): node is TableCell {
    return node.type === MyceliumASTTypes.TABLE_CELL;
  }

  static isList(node: Node): node is List {
    return node.type === MyceliumASTTypes.LIST;
  }

  static isNoteRefV2(node: Node): node is NoteRefNoteV4 {
    return node.type === MyceliumASTTypes.REF_LINK_V2;
  }

  static isImage(node: Node): node is Image {
    return node.type === MyceliumASTTypes.IMAGE;
  }

  static isExtendedImage(node: Node): node is ExtendedImage {
    return node.type === MyceliumASTTypes.EXTENDED_IMAGE;
  }

  static isText(node: Node): node is Text {
    return node.type === MyceliumASTTypes.TEXT;
  }

  static isLink(node: Node): node is Link {
    return node.type === MyceliumASTTypes.LINK;
  }

  static isWikiLink(node: Node): node is WikiLinkNoteV4 {
    return node.type === MyceliumASTTypes.WIKI_LINK;
  }

  static isFootnoteDefinition(node: Node): node is FootnoteDefinition {
    return node.type === MyceliumASTTypes.FOOTNOTE_DEFINITION;
  }

  static isFrontmatter(node: Node): node is FrontmatterContent {
    return node.type === MyceliumASTTypes.FRONTMATTER;
  }

  static isHTML(node: Node): node is HTML {
    return node.type === MyceliumASTTypes.HTML;
  }

  static isCode(node: Node): node is Code {
    return node.type === MyceliumASTTypes.CODE;
  }

  static isYAML(node: Node): node is YAML {
    return node.type === MyceliumASTTypes.YAML;
  }

  static isHashTag(node: Node): node is HashTag {
    return node.type === MyceliumASTTypes.HASHTAG;
  }

  static isUserTag(node: Node): node is UserTag {
    return node.type === MyceliumASTTypes.USERTAG;
  }
  ...
  static convertLinksFromDotNotation(
    ...
    return function (this: Processor) {
      return (tree: Node, _vfile: VFile) => {
        const root = tree as MyceliumASTRoot;
    ...
    return function (this: Processor) {
      return async (tree: Node, _vfile: VFile) => {
        ...
        await asyncLoop(wikiLinks, async (linkNode) => {
          ...
          if (existingNote) {
            const publishingConfig = ConfigUtils.getPublishing(myceliumConfig);
    ...
    return function (this: Processor) {
      return (tree: Node, _vfile: VFile) => {
        ...
        const idx = _.findIndex(
          root.children,
          (ent) => ent.type === MyceliumASTTypes.HEADING && ent.depth === 1
        );
        if (idx >= 0) {
  ...
  static async extractBlocks({
    ...
    for (const node of nodesToSearch) {
      ...
      if (node.type === MyceliumASTTypes.PARAGRAPH) {
        ...
        if (parent.children.length === 1) {
          ...
          if (child.type === MyceliumASTTypes.BLOCK_ANCHOR) {
      ...
      if (node.type === MyceliumASTTypes.LIST) {
        visit(node, [MyceliumASTTypes.LIST_ITEM], (listItem: ListItem) => {
          // The list item might have a block anchor inside of it.
          let anchor: DNoteAnchorPositioned | undefined;
          visit(
            listItem,
            [MyceliumASTTypes.BLOCK_ANCHOR, MyceliumASTTypes.LIST],
            (inListItem) => {
              // Except if we hit a nested list, because then the block anchor refers to the item in the nested list
              if (inListItem.type === MyceliumASTTypes.LIST) return "skip";
              [, anchor] =
      ...
      if (node.type === MyceliumASTTypes.HEADING) {
      ...
      } else if (node.type !== MyceliumASTTypes.LIST) {
        ...
        visit(node, [MyceliumASTTypes.BLOCK_ANCHOR], (child) => {
          [, anchor] =
            AnchorUtils.anchorNode2anchor(child as BlockAnchor, slugger) || [];
        });
      }
  ...
  static extractFootnoteDefs(root: Node): FootnoteDefinition[] {
    return selectAll(MyceliumASTTypes.FOOTNOTE_DEFINITION, root).filter(
  ...
  static extractFMTags(body: string) {
    ...
    visit(noteAST, [MyceliumASTTypes.FRONTMATTER], (frontmatter: YAML) => {
  ...
  static getNoteUrl(opts: {
    ...
    if (!root) {
      throw new MyceliumError({ message: "no urlRoot set" });
    }
