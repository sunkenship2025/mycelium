...
import { MyceliumASTDest, MyceliumASTTypes, HashTag } from "../types";
import { MDUtilsV5 } from "../utilsv5";

/** All sorts of punctuation marks and quotation marks from different languages. Please add any that may be missing.
 *
 * Be warned that this excludes period (.) as it has a special meaning in Mycelium. Make sure to handle it appropriately depending on the context.
 *
  ...
  if (visitors) {
    visitors.hashtag = (node: HashTag): string | Element => {
      ...
      switch (dest) {
        ...
        case MyceliumASTDest.MD_ENHANCED_PREVIEW:
          return `[${node.value}](${prefix}${node.fname})`;
        default:
          throw new MyceliumError({ message: "Unable to render hashtag" });
      }
    };
  }
}
