import { vault2Path } from "@myceliumhq/common-all";
...
import { MyceliumASTTypes, HashTag, UserTag, WikiLinkNoteV4 } from "../types";
...
function modifyWikilinkValueToCommandUri({
  ...
  node.value = `command:mycelium.gotoNote?${encodedArgs}`;
}
...
function modifyTagValueToCommandUri({
  ...
  (node as unknown as Link).url = `command:mycelium.gotoNote?${encodedArgs}`;
...
export function myceliumHoverPreview(
  ...
  function transformer(tree: Node, _file: VFile) {
    visit(
      tree,
      [
        MyceliumASTTypes.FRONTMATTER,
        MyceliumASTTypes.IMAGE,
        MyceliumASTTypes.EXTENDED_IMAGE,
...
export { PluginOpts as MyceliumPreviewOpts };
