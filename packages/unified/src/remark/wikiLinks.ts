    ...
    visitors.wikiLink = function (node: WikiLinkNoteV4) {
      ...
      if (pOpts.mode === ProcMode.NO_DATA) {
        ...
        const vaultPrefix = data.vaultName
          ? `${CONSTANTS.MYCELIUM_DELIMETER}${data.vaultName}/`
          : "";
      ...
      // if converting back to mycelium md, no further processing
      if (dest === MyceliumASTDest.MD_MYCELIUM) {
        return LinkUtils.renderNoteLink({
          link: {
            ...
            type: LinkUtils.astType2DLinkType(MyceliumASTTypes.WIKI_LINK),
      ...
      if (copts.useId && dest === MyceliumASTDest.HTML) {
        let notes;
      ...
      switch (dest) {
        case MyceliumASTDest.MD_REGULAR: {
...
function attachParser(proc: Unified.Processor) {
  ...
  function parseLink(linkMatch: string) {
    ...
    if (_.isNull(out)) {
      throw new MyceliumError({ message: `link is null: ${linkMatch}` });
    }
    if (pOpts.mode === ProcMode.NO_DATA) {
      return out;
    }
