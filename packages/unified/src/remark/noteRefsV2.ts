...
export class NoteRefUtils {
  static dnodeRefLink2String(link: DNoteRefLink) {
    ...
    const vaultPrefix = link.data.vaultName
      ? `${CONSTANTS.MYCELIUM_DELIMETER}${link.data.vaultName}/`
      : "";
...
function shouldRenderPretty({ proc }: { proc: Processor }): boolean {
  ...
  if (
    _.includes([MyceliumASTDest.MD_MYCELIUM, MyceliumASTDest.MD_REGULAR], dest)
  ) {
    return false;
  }
...
function attachCompiler(proc: Unified.Processor, _opts?: CompilerOpts) {
  ...
  if (visitors) {
    visitors.refLinkV2 = (node: NoteRefNoteV4) => {
      ...
      if (dest === MyceliumASTDest.MD_MYCELIUM) {
...
// ^m0vy37pdpzgy
/**
 * This exists because {@link myceliumPub} converts note refs using the AST
 */
export function convertNoteRefToHAST(
  opts: ConvertNoteRefOpts & { procOpts: any }
): { error: MyceliumError | undefined; data: Parent[] | undefined } {
  ...
  if (refLvl >= MAX_REF_LVL) {
    return {
      error: new MyceliumError({ message: "too many nested note refs" }),
...
function removeListItems({
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (list.ancestor.type !== MyceliumASTTypes.LIST) continue;
...
function removeExceptSingleItem(nodes: ParentWithIndex[]) {
  ...
  _.forEach(nodes, ({ ancestor }) => {
    if (ancestor.type === MyceliumASTTypes.LIST_ITEM) {
  ...
  closestListItem.children = closestListItem.children.filter(
    (node) => !(node.type === MyceliumASTTypes.LIST)
  );
}
...
function convertNoteRefToMDAST(
  ...
  const bodyAST: MyceliumASTNode = noteRefProc.parse(
    note.body
  ) as MyceliumASTNode;
...
function findAnchor({
  ...
  nodes: MyceliumASTNode["children"];
  match: string;
}): FindAnchorResult {
...
function findBlockAnchor({
  ...
  MdastUtils.visitParentsIndices({
    nodes,
    test: MyceliumASTTypes.BLOCK_ANCHOR,
  ...
  if (!_.isEmpty(foundAncestors)) {
    ...
    if (foundAncestors[0].ancestor.type === MyceliumASTTypes.LIST) {
