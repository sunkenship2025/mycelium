...
type PluginOpts = NoteRefsOptsV2 & {
  ...
  insertTitle?: boolean;
  /**
   * Don't publish pages that are dis-allowd by mycelium.yml
   */
...
type MyceliumUnifiedHandlerMatchOpts = {
  pOpts: ProcOptsV5;
  pData: ProcDataFullOptsV5;
};

type MyceliumUnifiedHandlerHandleOpts<T = any> = {
  proc: Processor;
  parent: Node;
  cOpts?: T;
};

type MyceliumUnifiedHandlerNextAction = undefined | number;
...
class NodeUrlHandler {
  ...
  static match(node: Node | any, { pData }: MyceliumUnifiedHandlerMatchOpts) {
    return (
      (node.type === MyceliumASTTypes.IMAGE ||
        node.type === MyceliumASTTypes.EXTENDED_IMAGE ||
        node.type === MyceliumASTTypes.LINK) &&
      pData.dest === MyceliumASTDest.HTML
    );
  }

  static handle(
    node: Image | Link,
    { proc, cOpts }: MyceliumUnifiedHandlerHandleOpts<PluginOpts>
  ): { node: Image | Link; nextAction?: MyceliumUnifiedHandlerNextAction } {
...
function shouldInsertTitle({ proc }: { proc: Processor }) {
  ...
  if (
    ...
    const config = data.config as MyceliumConfig;
...
function plugin(this: Unified.Processor, opts?: PluginOpts): Transformer {
  ...
  function transformer(tree: Node, _file: VFile) {
    ...
    if (mode !== ProcMode.IMPORT && !insideNoteRef && root.children) {
      if (!fname || !vault) {
        ...
        throw new MyceliumError({
          message: `myceliumPub - no fname or vault for node: ${JSON.stringify(
            tree
          )}`,
        });
      }
      ...
      if (insertTitle) {
        ...
        root.children.splice(
          idx,
          0,
          u(MyceliumASTTypes.HEADING, { depth: 1 }, [u("text", note.title)])
        );
      }
    }
    visitParents(tree, (node, ancestors) => {
      ...
      if (node.type === MyceliumASTTypes.HASHTAG) {
      ...
      if (node.type === MyceliumASTTypes.USERTAG) {
      ...
      if (node.type === MyceliumASTTypes.WIKI_LINK) {
        ...
        // If the target is Mycelium, no processing of links is needed
        if (dest === MyceliumASTDest.MD_MYCELIUM) return;
        ...
        if (mode !== ProcMode.IMPORT) {
          ...
          if (!note) {
            error = new MyceliumError({ message: `no note found. ${value}` });
          }
        }
        ...
        if (value === "403") {
          ...
          _node.data = {
            ...
            hProperties: {
              title: "Private",
              href: "https://wiki.mycelium.so/notes/hfyvYGJZQiUwQaaxQO27q.html",
              target: "_blank",
              class: "private",
            },
            hChildren: [
      ...
      if (node.type === MyceliumASTTypes.REF_LINK_V2) {
        // If the target is Mycelium, no processing of refs is needed
        if (dest === MyceliumASTDest.MD_MYCELIUM) return;
      ...
      if (node.type === MyceliumASTTypes.BLOCK_ANCHOR) {
        // no transform
...
export { plugin as myceliumPub };
export { PluginOpts as MyceliumPubOpts };
