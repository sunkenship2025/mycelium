...
- > NOTE: After you press enter, Mycelium will create and open the `mycelium` note. Use `<CTRL>-<TAB>` to come back to this note

You just created your first note!

- > NOTE: Notes in Mycelium are just plain text markdown with some [frontmatter](https://wiki.mycelium.so/notes/ffec2853-c0e0-4165-a368-339db12c8e4b) on the top. You can edit them in Mycelium or using ~~vim~~ your favourite text editor.

## Find a Note

1. Use `Ctrl+L` / `Cmd+L` to bring up the lookup prompt again
1. Type `mycelium` and press `<ENTER>`

- > TIP: you don't have to type out the entire query, press `<TAB>` to autocomplete

You just `looked up` a note!

- > NOTE: in Mycelium, you can find or create notes using the lookup prompt
...
- > NOTE: hierarchies in Mycelium are just `.` delimited files. This makes each note both a file and a folder and makes it easy to keep your notes organized

- > TIP: You can use the [Mycelium Tree View](https://wiki.mycelium.so/notes/hur7r6gr3kqa56s2vme986j) to view your hierarchy. If it's not currently in focus, you can use `CTRL+SHIFT+P`/`CMD+SHIFT+P` to open the command prompt and type in `Mycelium: focus on tree view` to make it appear
...
1. Open [[tutorial.one]], bring up the command prompt and type `Mycelium: Rename Note`
1. Replace `tutorial` with `my-note` and then press `<ENTER>`
1. You just refactored the note!

- > NOTE: when you rename a note, Mycelium updates all links and references of the original note being renamed. Try switching back to [[tutorial]] to see the updated link!
- > TIP: in addition to renaming one note at a time, mycelium has [an entire collection](https://wiki.mycelium.so/notes/srajljj10V2dl19nCSFiC) of refactoring commands that let you change headers, move around sections, and refactor entire hierarchies!
