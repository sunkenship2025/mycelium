...
import { file2Note, vault2Path } from "@myceliumhq/common-server";
...
/**
 * See [[Workspace Watcher|mycelium://mycelium.docs/pkg.plugin-core.ref.workspace-watcher]] for more docs
 */
export class WorkspaceWatcher {
  ...
  private _extension: IMyceliumExtension;
  ...
  private async onDidSaveNote(document: TextDocument) {
    ...
    if (
      ...
      mainVault &&
      !fname.startsWith("mycelium.hist")
    ) {
      const date = Time.now().toFormat("y.MM.dd");
      const historyFile = `mycelium.hist.${date}`;
  ...
  onWillRenameFiles(args: FileWillRenameEvent) {
    ...
    try {
      ...
      // No-op if we are not dealing with a Mycelium note.
  ...
  /**
   * Mycelium will perform changes like moving the cursor when first opening a Mycelium note
   * @returns boolean : returns `true` if Mycelium made changes during `onFirstOpen` and `false` otherwise
   */
  private async onFirstOpen(editor: TextEditor) {
    ...
    // don't apply actions to non-mycelium notes
    // NOTE: in the future if we add `onFirstOpen` actions to non-mycelium notes, this logic will need to be updated
    if (!(await WorkspaceUtils.isMyceliumNote({ wsRoot, vaults, fpath }))) {
      return false;
    }
