...
export type MyceliumDecoration<T = any> = {
  /**
...
type MyceliumNoteRefDecoration = Required<
  MyceliumDecoration<NoteRefDecorator["data"]>
>;
...
async function addInlineNoteRefs(opts: {
  decorations: MyceliumNoteRefDecoration[];
...
// see [[Decorations|mycelium://mycelium.docs/pkg.plugin-core.ref.decorations]] for further docs
export async function updateDecorations(editor: TextEditor): Promise<{
  ...
  try {
    ...
    if (config.dev?.enableExperimentalInlineNoteRef) {
      const noteRefDecorators: MyceliumNoteRefDecoration[] =
        vscodeDecorations.filter((ent) => {
          return ent.type === EDITOR_DECORATION_TYPES.noteRef;
        }) as MyceliumNoteRefDecoration[];
...
function mapDecoration(decoration: Decoration): MyceliumDecoration | undefined {
...
function mapTimestamp(decoration: DecorationTimestamp): MyceliumDecoration {
...
): MyceliumNoteRefDecoration | undefined {
  return mapBasicDecoration(decoration) as MyceliumNoteRefDecoration;
}
