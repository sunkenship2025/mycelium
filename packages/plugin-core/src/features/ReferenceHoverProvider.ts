...
import { findNonNoteFile, vault2Path } from "@myceliumhq/common-server";
import { AnchorUtils, MyceliumASTDest, ProcFlavor } from "@myceliumhq/unified";
...
export default class ReferenceHoverProvider implements vscode.HoverProvider {
  private async provideHoverNonNote({
    ...
    const nonMyceliumURIMessage = this.handleNonMyceliumUri(refAtPos.refText);
    if (!_.isUndefined(nonMyceliumURIMessage)) return nonMyceliumURIMessage;
    ...
    if (validationResp.isValid) {
      ...
      return `Note ${refAtPos.ref}${vaultName} is missing, ${ctrlClickToCreate}use "Mycelium: Go to Note" command to create it.`;
    } else {
  ...
  /** Returns a message if this is a non-mycelium URI. */
  private handleNonMyceliumUri(refText: string): string | undefined {
    const [first, second] = refText.split("|");
    const maybeUri = second || first;
    const maybe = containsNonMyceliumUri(maybeUri);
    // Not a URI, or is mycelium://, so it must be a note (or image) and the rest of the code can handle this.
  ...
  public async provideHover(
    ...
    try {
      ...
      // No-op if we're not in a Mycelium Workspace
      if (
        !(await ExtensionProvider.isActiveAndIsMyceliumNote(document.uri.fsPath))
      ) {
        return null;
      }
      ...
      const referenceText = ["![["];
      if (refAtPos.vaultName)
        referenceText.push(`mycelium://${refAtPos.vaultName}/`);
      ...
      if (rendered.error) {
        const error =
          rendered.error instanceof MyceliumError
            ? rendered.error
            : new MyceliumError({
