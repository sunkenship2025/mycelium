...
import { MDUtilsV5 } from "@myceliumhq/unified";
...
import { MyceliumContext, MYCELIUM_COMMANDS, ICONS } from "../constants";
  ...
  implements TreeDataProvider<Backlink>, Disposable
{
   ...
   * engine
   */
  constructor(engineEvents: EngineEventEmitter, config: MyceliumConfig) {
  ...
  public set sortOrder(sortOrder: BacklinkPanelSortOrder) {
    if (sortOrder !== this._sortOrder) {
      ...
      VSCodeUtils.setContextStringValue(
        MyceliumContext.BACKLINKS_SORT_ORDER,
        sortOrder
      );
  ...
  private getAllBacklinksInNoteFromRefs = (
    ...
    return refs.map((ref) => {
      ...
      backlink.command = {
        command: MYCELIUM_COMMANDS.GOTO_BACKLINK.key,
        arguments: [
          ref.location.uri,
      ...
      if (ref.isCandidate) {
        backlink.command = {
          command: "mycelium.convertCandidateLink",
