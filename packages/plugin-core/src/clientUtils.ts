...
export class MyceliumClientUtilsV2 {
  ...
  static genNoteName(
      ...
      case LookupNoteTypeEnum.scratch: {
        dateFormat =
          ExtensionProvider.getExtension().getWorkspaceSettingOrDefault({
            wsConfigKey: "mycelium.defaultScratchDateFormat",
            myceliumConfigKey: "workspace.scratch.dateFormat",
          });
        addBehavior =
          ExtensionProvider.getExtension().getWorkspaceSettingOrDefault({
            wsConfigKey: "mycelium.defaultScratchAddBehavior",
            myceliumConfigKey: "workspace.scratch.addBehavior",
          });
        name = ExtensionProvider.getExtension().getWorkspaceSettingOrDefault({
          wsConfigKey: "mycelium.defaultScratchName",
          myceliumConfigKey: "workspace.scratch.name",
        });
        break;
      }
    ...
    const prefix = MyceliumClientUtilsV2.genNotePrefix(
      currentNoteFname,
      addBehavior
    );
  ...
  static getSchemaModByFname = async ({
    ...
    if (!smod) {
      throw new MyceliumError({ message: "no note found" });
    }
    return smod;
  };
