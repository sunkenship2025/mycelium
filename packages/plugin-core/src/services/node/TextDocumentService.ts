...
import { DConfig, DLogger } from "@myceliumhq/common-server";
import { EngineUtils, WorkspaceUtils } from "@myceliumhq/engine-server";
 ...
 * See [[Note Sync Service|mycelium://mycelium.docs/pkg.plugin-core.ref.note-sync-service]] for
 * additional docs
 */
export class TextDocumentService implements ITextDocumentService {
  ...
  _extension: IMyceliumExtension;

  /**
   *
   * @param ext Instance of IMyceliumExtension
   ...
   */
  constructor(ext: IMyceliumExtension, textDocumentEvent: Event<TextDocument>) {
    this.L = Logger;
    this._extension = ext;
