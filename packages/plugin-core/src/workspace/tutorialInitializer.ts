...
import { file2Note, SegmentClient, vault2Path } from "@myceliumhq/common-server";
import {
  ...
  implements WorkspaceInitializer
{
  ...
  async onWorkspaceOpen(opts: { ws: DWorkspaceV2 }): Promise<void> {
    ...
    if (fs.pathExistsSync(rootUri.fsPath)) {
      ...
      Logger.error({
        ctx,
        error: new MyceliumError({ message: `Unable to find tutorial.md` }),
      });
    }
  ...
  async onWorkspaceActivate(opts: {
    ...
    if (!skipOpts?.skipTreeView) {
      ...
      vscode.commands.executeCommand("mycelium.treeView.focus");
    }
  }
}
