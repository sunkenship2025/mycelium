...
import { SubProcessExitType } from "@myceliumhq/api-server";
...
import { getDurationMilliseconds, GitUtils } from "@myceliumhq/common-server";
import {
  HistoryService,
  MetadataService,
  WorkspaceService,
...
import { MyceliumContext, MYCELIUM_COMMANDS } from "../constants";
...
import { MyceliumCodeWorkspace } from "./codeWorkspace";
import { MyceliumNativeWorkspace } from "./nativeWorkspace";
...
function _setupTreeViewCommands(
  ...
  if (
    !existingCommands.includes(MYCELIUM_COMMANDS.TREEVIEW_LABEL_BY_TITLE.key)
  ) {
  ...
  if (
    !existingCommands.includes(MYCELIUM_COMMANDS.TREEVIEW_LABEL_BY_FILENAME.key)
  ) {
    vscode.commands.registerCommand(
   ...
   * Enablement is set to be MyceliumContext.DEV_MODE
   *
   * TODO: fix tree item register issue and flip the dev mode flag.
   */
  if (!existingCommands.includes(MYCELIUM_COMMANDS.TREEVIEW_EXPAND_ALL.key)) {
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.TREEVIEW_EXPAND_ALL.key,
  ...
  if (!existingCommands.includes(MYCELIUM_COMMANDS.TREEVIEW_EXPAND_STUB.key)) {
    vscode.commands.registerCommand(
...
async function getOrPromptWSRoot(workspaceFolders: string[]) {
  if (!workspaceFolders) {
    Logger.error({ msg: "No mycelium.yml found in any workspace folder" });
    return undefined;
  }
  if (workspaceFolders.length === 1) {
    ...
    const selectedRoot = await VSCodeUtils.showQuickPick(
      ...
      }),
      {
        ignoreFocusOut: true,
        canPickMany: false,
        title: "Select Mycelium workspace to load",
      }
    );
    if (!selectedRoot) {
      await vscode.window.showInformationMessage(
        "You skipped loading any Mycelium workspace, Mycelium is not active. You can run the 'Developer: Reload Window' command to reactivate Mycelium."
      );
      Logger.info({
        msg: "User skipped loading a Mycelium workspace",
        workspaceFolders,
      });
      return null;
    }
...
/**
 * Get version of Mycelium when workspace was last activated
 */
async function getAndCleanPreviousWSVersion({
  ...
  // to .mycelium.ws:
  if (previousWorkspaceVersionFromWSService === "mycelium-cli") {
...
async function checkNoDuplicateVaultNames(vaults: DVault[]): Promise<boolean> {
  ...
  if (duplicates.size > 0) {
    ...
    await vscode.window
      .showErrorMessage(
        `Following vault names have duplicates: ${duplicateVaultNames} See https://mycelium.so/notes/a6c03f9b-8959-4d67-8394-4d204ab69bfe.html#multiple-vaults-with-the-same-name to fix`,
        txt
      )
      .then((resp) => {
        if (resp === txt) {
          vscode.commands.executeCommand(
            "vscode.open",
            vscode.Uri.parse(
              "https://mycelium.so/notes/a6c03f9b-8959-4d67-8394-4d204ab69bfe.html#multiple-vaults-with-the-same-name"
            )
          );
        }
      });
...
async function postReloadWorkspace({
  ...
  if (!wsService) {
    ...
    Logger.error({
      msg: errorMsg,
      error: new MyceliumError({ message: errorMsg }),
    });
    return;
  }
  ...
  // we are moving everything to mycelium.yml
  // see [[2021 06 Deprecate Workspace Settings|proj.2021-06-deprecate-workspace-settings]]
  if (previousWsVersion === CONSTANTS.MYCELIUM_INIT_VERSION) {
    Logger.info({ ctx, msg: "no previous global version" });
    vscode.commands
      .executeCommand(MYCELIUM_COMMANDS.UPGRADE_SETTINGS.key)
      .then((changes) => {
        Logger.info({ ctx, msg: "postUpgrade: new wsVersion", changes });
      });
    wsService.writeMeta({ version: MyceliumExtension.version() });
  } else {
    const newVersion = MyceliumExtension.version();
    if (semver.lt(previousWsVersion, newVersion)) {
      ...
      try {
        changes = await vscode.commands.executeCommand(
          MYCELIUM_COMMANDS.UPGRADE_SETTINGS.key
        );
        Logger.info({
          ctx,
...
function togglePluginActiveContext(enabled: boolean) {
  ...
  VSCodeUtils.setContext(MyceliumContext.PLUGIN_ACTIVE, enabled);
  VSCodeUtils.setContext(MyceliumContext.HAS_CUSTOM_MARKDOWN_VIEW, enabled);
}
...
export class WorkspaceActivator {
  ...
  > {
    ...
    const currentVersion = MyceliumExtension.version();
    const wsService = new WorkspaceService({ wsRoot });
    const myceliumConfig = workspace.config;
  ...
  async activate({
    ...
    MetadataService.instance().setMyceliumWorkspaceActivated();
    ...
    const workspace = MyceliumExtension.tryWorkspaceFile()?.fsPath || wsRoot;
  ...
  async initCodeWorkspace({ context, wsRoot }: WorkspaceActivatorOpts) {
    ...
    const ws = new MyceliumCodeWorkspace({
      wsRoot,
  ...
  async initNativeWorkspace({ context, wsRoot }: WorkspaceActivatorOpts) {
    const assetUri = VSCodeUtils.getAssetUri(context);
    const ws = new MyceliumNativeWorkspace({
      wsRoot,
      logUri: context.logUri,
      assetUri,
    });
    ...
    if (ext.type === WorkspaceType.NATIVE) {
      const workspaceFolders =
        await WorkspaceUtils.findWSRootsInWorkspaceFolders(
          MyceliumExtension.workspaceFolders()!
        );
      if (!workspaceFolders) {
        return;
      }
      ...
      return path.dirname(MyceliumExtension.workspaceFile().fsPath);
    }
  }
  ...
  async verifyOrStartServerProcess({
    ...
    const { port, subprocess } = await ExtensionUtils.startServerProcess({
      ...
      onExit: (type: SubProcessExitType) => {
        const txt = "Restart Mycelium";
        vscode.window
          .showErrorMessage("Mycelium engine encountered an error", txt)
          .then(async (resp) => {
            if (resp === txt) {
