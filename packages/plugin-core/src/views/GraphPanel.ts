...
import { MetadataService, WorkspaceUtils } from "@myceliumhq/engine-server";
...
export class GraphPanel implements vscode.WebviewViewProvider {
  public static readonly viewType = MyceliumTreeViewKey.GRAPH_PANEL;
  ...
  public set showBacklinks(displayBacklinks: boolean | undefined) {
    if (
      ...
      VSCodeUtils.setContext(
        MyceliumContext.GRAPH_PANEL_SHOW_BACKLINKS,
        displayBacklinks
      );
      this.postMessage({
  ...
  public set showOutwardLinks(displayOutwardLinks: boolean | undefined) {
    if (
      ...
      this._showOutwardLinks = displayOutwardLinks;
      VSCodeUtils.setContext(
        MyceliumContext.GRAPH_PANEL_SHOW_OUTWARD_LINKS,
        displayOutwardLinks
      );
      this.postMessage({
        type: GraphViewMessageEnum.toggleGraphEdges,
  ...
  public set showHierarchy(displayHierarchy: boolean | undefined) {
    if (
      ...
      this._showHierarchy = displayHierarchy;
      VSCodeUtils.setContext(
        MyceliumContext.GRAPH_PANEL_SHOW_HIERARCHY,
        displayHierarchy
      );
      this.postMessage({
        type: GraphViewMessageEnum.toggleGraphEdges,
  ...
  public async resolveWebviewView(
    ...
    await WebViewUtils.prepareTreeView({
      ext: this._ext,
      key: MyceliumTreeViewKey.GRAPH_PANEL,
      webviewView,
    });
