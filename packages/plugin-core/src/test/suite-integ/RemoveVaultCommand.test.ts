...
import { FileTestUtils } from "@myceliumhq/common-test-utils";
import { setupWS } from "@myceliumhq/engine-test-utils";
...
function getWorkspaceFile() {
  const settings = fs.readJSONSync(
    MyceliumExtension.workspaceFile().fsPath
...
suite("GIVEN RemoveVaultCommand", function () {
  ...
  describeMultiWS("WHEN removing a regular vault", {}, () => {
    test("THEN the vault is removed", async () => {
      ...
      const config = DConfig.getRaw(wsRoot) as MyceliumConfig;
  ...
  describe("WHEN removing a vault when override is present", () => {
    describeMultiWS(
      ...
      () => {
        test("THEN the vault is removed from mycelium.yml, and override is not merged into config", async () => {
          ...
          // before remove, mycelium.yml has 3 vaults
          ...
          // after remove, we have 2 vaults in mycelium.yml
  ...
  describeSingleWS("WHEN there's only one vault left after remove", {}, () => {
    test("THEN duplicateNoteBehavior is omitted", async () => {
      ...
      const configNew = readYAML(configPath) as MyceliumConfig;
  ...
  describeSingleWS("WHEN a published vault is removed", {}, () => {
    test("THEN the vault is removed from duplicateNoteBehavior payload", async () => {
      ...
      const configOrig = DConfig.getRaw(wsRoot) as MyceliumConfig;
