import { NoteProps, NotePropsMeta, VaultUtils } from "@myceliumhq/common-all";
...
import { MockMyceliumExtension } from "../MockMyceliumExtension";
...
suite("PreviewLinkHandler", () => {
    ...
    () => {
      ...
      describe("WHEN clicking on a web URL", () => {
        test("THEN opening is left to VSCode", async () => {
          ...
          const out = await handler.onLinkClicked({
            data: {
              href: "https://wiki.mycelium.so/#getting-started",
              id: note.id,
            },
          });
  ...
  describe(`extractNoteIdFromHref`, () => {
    ...
    it(`WHEN id not present in href THEN default onto passed in id`, () => {
      const linkHandler = new PreviewLinkHandler(new MockMyceliumExtension({}));
