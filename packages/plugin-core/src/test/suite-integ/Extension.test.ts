...
import { VAULTS } from "@myceliumhq/engine-test-utils";
...
function getDefaultConfig() {
  const defaultConfig: MyceliumConfig = {
...
suite("missing default config detection", () => {
  describeMultiWS(
    "GIVEN mycelium.yml with missing default key",
    {
      modConfigCb: (config) => {
        // @ts-ignore
...
suite("deprecated config detection", () => {
  describeMultiWS(
    "GIVEN mycelium.yml with deprecated key",
    {
      modConfigCb: (config) => {
        // @ts-ignore
...
suite("duplicate config entry detection", () => {
  describeMultiWS(
    "GIVEN mycelium.yml with duplicate config",
    {
      preActivateHook: async ({ wsRoot }) => {
  ...
  describeMultiWS("GIVEN mycelium.yml without duplicate config", {}, () => {
...
suite("localhost blocked on user's machine", () => {
  ...
  describeMultiWS(
    ...
    { timeout: 5e3 },
    () => {
      test("THEN mycelium inits", async () => {
        const pingArgs =
          os.platform() === "win32"
            ? "ping -n 1 127.0.0.1"
            : "ping -c 1 127.0.0.1";
