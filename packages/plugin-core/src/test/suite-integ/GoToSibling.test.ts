import { DVault, WorkspaceOpts } from "@myceliumhq/common-all";
import {
  CreateNoteOptsV4,
  NoteTestUtilsV4,
...
suite("GoToSibling", () => {
  describe("WHEN non-journal note is open", async () => {
    ...
    describeSingleWS("no active mycelium note", {}, () => {
      test("Warning message should appear", async () => {
        await VSCodeUtils.closeAllEditors();
        // Create a file that is not a Mycelium note and show it on editor
  ...
  describe("WHEN journal note is open", async () => {
    ...
    describeSingleWS(
      "WHEN date config for journal notes is note default",
      {
        ...
        modConfigCb: (config) => {
          // Change journal date config on mycelium.yml for the current workspace
