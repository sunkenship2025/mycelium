...
import { DConfig, tmpDir, vault2Path } from "@myceliumhq/common-server";
import {
  EngineTestUtilsV4,
  FileTestUtils,
  NoteTestUtilsV4,
...
import { HistoryService, MetadataService } from "@myceliumhq/engine-server";
import {
  ENGINE_HOOKS,
  ENGINE_HOOKS_MULTI,
  TestEngineUtils,
...
export function expectQuickPick(quickPick: MyceliumQuickPickerV2 | undefined) {
...
function getButtonByType(
  btnType: ButtonType,
  buttons: vscode.QuickInputButton[] & MyceliumBtn[]
) {
...
function getSelectionTypeButtons(
  ...
  const [selection2linkBtn, selectionExtractBtn] = getButtonsByTypeArray(
    ...
    buttons
  ) as vscode.QuickInputButton[] & MyceliumBtn[];
...
suite("NoteLookupCommand", function () {
  ...
  const getTodayInScratchDateFormat = () => {
    const dateFormat = MyceliumExtension.configuration().get<string>(
    ...
    describeMultiWS(
      "WHEN schema template references to a template note that lies in a different vault using xvault notation",
      {
        ...
        postSetupHook: async ({ wsRoot, vaults }) => {
          ...
          const template: SchemaTemplate = {
            id: `mycelium://${VaultUtils.getName(vaults[2])}/template.ch2`,
            type: "note",
          };
          await NoteTestUtilsV4.setupSchemaCrossVault({
            wsRoot,
    ...
    describeMultiWS(
      "WHEN schema template references to a template note that lies in a different vault using xvault notation that points to the wrong vault",
      {
        ...
        postSetupHook: async ({ wsRoot, vaults }) => {
          ...
          const template: SchemaTemplate = {
            id: `mycelium://missingVault/template.ch2`,
            type: "note",
          };
          await NoteTestUtilsV4.setupSchemaCrossVault({
            wsRoot,
  ...
  describe("onAccept with lookupConfirmVaultOnCreate", () => {
    const modConfigCb = (config: MyceliumConfig) => {
  ...
  describe("modifiers", () => {
    ...
    test("selection modifier set to none in configs", (done) => {
      runLegacyMultiWorkspaceTest({
        ctx,
        modConfigCb: (config: MyceliumConfig) => {
          ConfigUtils.setNoteLookupProps(
            config,
            "selectionMode",
