...
import { NotePropsMeta, NoteUtils, VaultUtils } from "@myceliumhq/common-all";
import { vault2Path } from "@myceliumhq/common-server";
import { AssertUtils, NoteTestUtilsV4 } from "@myceliumhq/common-test-utils";
import { DoctorActionsEnum } from "@myceliumhq/engine-server";
import { ENGINE_HOOKS } from "@myceliumhq/engine-test-utils";
    ...
    () => {
      test("THEN fix links", async () => {
        ...
        const file = await NoteTestUtilsV4.createNote({
          fname: "first",
          body: [
            "[[mycelium://vault2/second]]",
            "[[somenote|mycelium://vault2/somenote]]",
            "[[some note|mycelium://vault2/something]]",
          ].join("\n"),
          vault: vault1,
          wsRoot,
        });
    ...
    () => {
      test("THEN fix all links", async () => {
        ...
        await NoteTestUtilsV4.createNote({
          fname: "first",
          body: [
            "[[mycelium://vault2/cross2]]",
            "[[mycelium://vault1/broken]]",
            "[[somenote|mycelium://vault1/somenote]]",
            "[[some note|mycelium://vault1/something]]",
          ].join("\n"),
          vault: vault1,
          wsRoot,
        });
        await NoteTestUtilsV4.createNote({
          fname: "second",
          body: [
            "[[mycelium://vault1/cross1]]",
            "[[mycelium://vault2/broken2]]",
            "[[somenote|mycelium://vault2/somenote2]]",
            "[[some note|mycelium://vault2/something2]]",
          ].join("\n"),
          vault: vault2,
          wsRoot,
        });
    ...
    () => {
      test("THEN remove airtableId from note FM and update it with pods namespace", async () => {
        ...
        try {
          ...
          podIdQuickPickStub.onCall(0).returns(Promise.resolve("mycelium.task"));
          await cmd.run();
          const note = (await engine.getNoteMeta("foo.bar")).data!;
          expect(note?.custom.airtableId).toBeFalsy();
          expect(note?.custom.pods.airtable["mycelium.task"]).toEqual(
            "airtableId-one"
          );
        } finally {
          gatherInputsStub.restore();
