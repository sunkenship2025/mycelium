...
import { readYAMLAsync, tmpDir } from "@myceliumhq/common-server";
import {
  getWSMetaFilePath,
  MetadataService,
  openWSMetaFile,
...
import { TestEngineUtils } from "@myceliumhq/engine-test-utils";
...
function lapsedMessageTest({
  ...
  svc.setMeta("myceliumWorkspaceActivated", workspaceActivated);
...
suite("GIVEN SetupWorkspace Command", function () {
  ...
  describe("WHEN initializing a CODE workspace", function () {
    ...
    describe("AND workspace has not been set up yet", () => {
      test("THEN Dendon does not activate", async () => {
        ...
        const myceliumState = MetadataService.instance().getMeta();
        expect(isNotUndefined(myceliumState.firstInstall)).toBeTruthy();
        expect(isNotUndefined(myceliumState.firstWsInitialize)).toBeFalsy();
      });
    });

    describe("AND a new workspace is being created", () => {
      test("THEN Mycelium creates the workspace correctly", async () => {
        ...
        const resp = await readYAMLAsync(path.join(wsRoot, "mycelium.yml"));
        expect(resp).toEqual(
        ...
        const myceliumState = MetadataService.instance().getMeta();
        expect(isNotUndefined(myceliumState.firstInstall)).toBeTruthy();
        expect(isNotUndefined(myceliumState.firstWsInitialize)).toBeTruthy();
        expect(
        ...
        expect(resp).toContain({
          workspace: {
            vaults: [
              {
                fsPath: "templates",
                name: "mycelium.templates",
                seed: "mycelium.templates",
              },
              {
                fsPath: "vault",
              },
    ...
    describeSingleWS(
      ...
      {
        preSetupHook: async () => {
          MyceliumExtension.version = () => "0.0.1";
        },
        selfContained: false,
      },
      () => {
        test("THEN Mycelium initializes", async () => {
          ...
          const settings = fs.readJSONSync(
            path.join(wsRoot, "mycelium.code-workspace")
          );
          expect(settings).toEqual(genDefaultSettings());
          expect(fs.readdirSync(vault)).toEqual(
            [CONSTANTS.MYCELIUM_CACHE_FILE].concat(genEmptyWSFiles())
          );
        });
      }
    );
  ...
  describe("WHEN initializing a NATIVE workspace", function () {
    ...
    test("not active, initial create ws", async () => {
      ...
      expect(
        await fs.pathExists(path.join(wsRoot, CONSTANTS.MYCELIUM_CONFIG_FILE))
      ).toBeTruthy();
      expect(
        await fs.pathExists(path.join(wsRoot, CONSTANTS.MYCELIUM_WS_NAME))
      ).toBeFalsy();
    });
  });

  describe("WHEN initializing a self contained vault as a workspace", () => {
    test("THEN Mycelium correctly creates a workspace", async () => {
      const wsRoot = tmpDir().name;
      ...
      const firstFile = await fs.pathExists(
        path.join(wsRoot, CONSTANTS.MYCELIUM_CONFIG_FILE)
      );
