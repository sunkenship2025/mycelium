import { MyceliumError, DVault, NoteProps } from "@myceliumhq/common-all";
import {
  NoteTestUtilsV4,
    ...
    beforeEach(() => {
      preSetupHook = async ({
        ...
        await NoteTestUtilsV4.createNote({
          ...
          wsRoot,
          vault: vaults[0],
          body: "[[Foo|mycelium://vault1/origin#foo-header]]\n\n",
        });
      };
    });
      ...
      test("THEN only reference to moved header is updated", (done) => {
        runLegacyMultiWorkspaceTest({
          ...
          onInit: onInitFunc(async () => {
            ...
            expect(
              refNote2.body.includes("[[Foo|mycelium://vault1/dest#foo-header]]")
            );
      ...
      test("THEN vault prefix is added to bare links if there are notes with same name as destination in different vaults", (done) => {
        runLegacyMultiWorkspaceTest({
          ...
          onInit: onInitFunc(async () => {
            ...
            expect(
              refNote.body.includes("[[Foo|mycelium://vault1/dest#foo-header]]")
            );
            expect(refNote.body.includes("[[Origin|dest]]")).toBeFalsy();
            done();
          }),
        ...
        runLegacyMultiWorkspaceTest({
          ...
          onInit: onInitFunc(async () => {
            ...
            try {
              ...
              wasThrown = true;
              expect(error instanceof MyceliumError).toBeTruthy();
