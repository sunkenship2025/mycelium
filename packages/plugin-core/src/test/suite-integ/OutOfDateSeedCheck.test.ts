import { DConfig, tmpDir } from "@myceliumhq/common-server";
...
import { ConfigUtils, FOLDERS, MyceliumConfig } from "@myceliumhq/common-all";
...
suite("GIVEN out of date seed check", function () {
  describeSingleWS("WHEN there's a seed with an out-of-date path", {}, () => {
    ...
    test("THEN Mycelium prompts to update the seed config", () => {
    ...
    test("THEN seed config is correctly updated", async () => {
      ...
      const conf = DConfig.getRaw(wsRoot) as MyceliumConfig;
