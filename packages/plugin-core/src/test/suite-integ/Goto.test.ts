...
import { runMochaHarness } from "@myceliumhq/common-test-utils";
...
const executeGotoCmd = async (ext: IMyceliumExtension) => {
  // return vscode.commands.executeCommand(MYCELIUM_COMMANDS.GOTO.key);
  return new GotoCommand(ext).execute();
};

/* Before each candidate */
// const ext = ExtensionProvider.getExtension();
// vscode.commands.executeCommand(MYCELIUM_COMMANDS.GOTO.key);
      ...
      () => {
        test("THEN goto the external link", async () => {
          ...
          expect(data).toContain({
            ...
            fullPath: "https://www.mycelium.so/",
            fromProxy: false,
          });
          avoidPopUp.restore();
        });
