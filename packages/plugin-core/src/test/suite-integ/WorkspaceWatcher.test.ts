...
import { NoteTestUtilsV4, FileTestUtils } from "@myceliumhq/common-test-utils";
...
suite("WorkspaceWatcher", function () {
  ...
  describeSingleWS(
    ...
    () => {
      test("WHEN user renames a file outside of mycelium rename command, THEN all of its references are also updated", async () => {
  ...
  describeSingleWS(
    ...
    () => {
      test("WHEN user renames a file outside of mycelium rename command, THEN the title of fileName is also updated", async () => {
        const { engine, wsRoot, vaults } = ExtensionProvider.getDWorkspace();
        const previewProxy = new MockPreviewProxy();
        const extension = ExtensionProvider.getExtension();
  ...
  describe("GIVEN the user opening a file", () => {
    ...
    describeSingleWS(
      "AND WHEN user opens non mycelium file for the first time",
      {},
      () => {
    ...
    describeSingleWS(
      "AND WHEN user opens non mycelium markdown file for the first time",
      {},
      () => {
        test("THEN do not affect frontmatter", async () => {
          const { wsRoot } = ExtensionProvider.getDWorkspace();
    ...
    describeSingleWS(
      "WHEN user opens mycelium note for the first time",
      {},
      () => {
        let note: NoteProps;
        before(async () => {
