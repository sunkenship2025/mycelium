import { InstallStatus, Time } from "@myceliumhq/common-all";
    ...
    describeMultiWS(
      "AND WHEN activate",
      {
        preActivateHook: async () => {
          ...
          stubMyceliumWhenNotFirstInstall();
          setupSpies();
        },
        afterHook,
        timeout: 1e4,
    ...
    describeMultiWS(
      "AND WHEN activate",
      {
        preActivateHook: async ({ ctx }) => {
          mockHomeDirStub = TestEngineUtils.mockHomeDir();
          setupSpies();
          stubMyceliumWhenFirstInstall(ctx);
        },
