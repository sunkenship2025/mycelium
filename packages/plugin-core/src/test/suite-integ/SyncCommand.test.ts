...
import { FileTestUtils, NoteTestUtilsV4 } from "@myceliumhq/common-test-utils";
import {
  Git,
  SyncActionStatus,
  WorkspaceUtils,
} from "@myceliumhq/engine-server";
import { GitTestUtils } from "@myceliumhq/engine-test-utils";
...
suite("workspace sync command", function () {
  ...
  describe("WHEN repo has remote, but branch has no upstream", () => {
    test("THEN Mycelium commits and warns about missing upstream", (done) => {
      runLegacyMultiWorkspaceTest({
        ctx,
        onInit: async ({ wsRoot, vaults }) => {
          const remoteDir = tmpDir().name;
  ...
  describeSingleWS(
    ...
    () => {
      test("THEN Mycelium skips committing", async () => {
  ...
  describeSingleWS(
    ...
    () => {
      test("THEN Mycelium stashes and restores the changes", async () => {
  ...
  describeSingleWS(
    ...
    () => {
      test("THEN Mycelium still restores local changes", async () => {
        const { vaults, wsRoot, engine } = ExtensionProvider.getDWorkspace();
        const remoteDir = tmpDir().name;
        await GitTestUtils.createRepoForRemoteWorkspace(wsRoot, remoteDir);
        // Add everything and push, so that there's no untracked changes
  ...
  describeSingleWS(
    ...
    () => {
      test("THEN rebase works, and Mycelium still restores local changes", async () => {
        const { vaults, wsRoot, engine } = ExtensionProvider.getDWorkspace();
        const remoteDir = tmpDir().name;
        await GitTestUtils.createRepoForRemoteWorkspace(wsRoot, remoteDir);
        const rootNote = (
  ...
  describeSingleWS(
    ...
    () => {
      test("THEN pull fails, and Mycelium restores local changes", async () => {
        const { vaults, wsRoot, engine } = ExtensionProvider.getDWorkspace();
        const remoteDir = tmpDir().name;
        await GitTestUtils.createRepoForRemoteWorkspace(wsRoot, remoteDir);
        const rootNote = (
  ...
  describeSingleWS(
    ...
    () => {
      test("THEN Mycelium skips doing stuff", async () => {
        const { vaults, wsRoot, engine } = ExtensionProvider.getDWorkspace();
        const remoteDir = tmpDir().name;
        await GitTestUtils.createRepoForRemoteWorkspace(wsRoot, remoteDir);
        const rootNote = (
  ...
  describeSingleWS(
    ...
    () => {
      test("THEN Mycelium warns that it can't connect to the remote", async () => {
        const { wsRoot } = ExtensionProvider.getDWorkspace();
        const remoteDir = tmpDir().name;
        await GitTestUtils.createRepoForRemoteWorkspace(wsRoot, remoteDir);
...
/** Override the config option in `mycelium.yml`, then add commit that change. */
