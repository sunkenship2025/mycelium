import { GitUtils, tmpDir, vault2Path } from "@myceliumhq/common-server";
...
describeSingleWS(
  ...
  () => {
    test("THEN codespace url is copied to the clipboard", async () => {
      ...
      expect(resp).toContain("https://github.dev/myceliumhq/mycelium/blob/");
    });
  }
);
