...
import { FileTestUtils, SinonStubbedFn } from "@myceliumhq/common-test-utils";
import { Git, WorkspaceService } from "@myceliumhq/engine-server";
import {
  checkVaults,
  GitTestUtils,
  setupWS,
...
function enableSelfCOntainedVaults(config: MyceliumConfig) {
  config.dev!.enableSelfContainedVaults = true;
  return config;
}

function disableSelfContainedVaults(config: MyceliumConfig) {
  ...
  describeSingleWS(
    ...
    () => {
      ...
      test("locally overriden vault is not merged into config", async () => {
        ...
        // mycelium.yml should have one vault;
        ...
        await cmd.run();
        // mycelium.yml should now have two vault
  ...
  describeSingleWS(
    ...
    () => {
      ...
      test("THEN the vault is under `dependencies/localhost`, and is self contained", async () => {
        ...
        expect(
          await readYAMLAsync(
            path.join(vaultPath, CONSTANTS.MYCELIUM_CONFIG_FILE)
          )
        ).toBeTruthy();
        expect(
