...
suite("AirtableExportCommand", function () {
  ...
  describe("GIVEN AirtableExportPodCommand is run with Note scope with podId mycelium.task", () => {
    describeSingleWS(
      ...
      () => {
        test("THEN note frontmatter should be updated with airtable metadata", async () => {
          ...
          expect(n?.custom.pods.airtable["mycelium.task"]).toEqual(airtableId);
        });
      }
    );
    describeSingleWS(
      ...
      () => {
        test("THEN new airtable id should be appended in the note frontmatter", async () => {
          ...
          const n = getNoteFromTextEditor();
          expect(n?.custom.pods.airtable["mycelium.task"]).toEqual(airtableId);
          expect(n?.custom.pods.airtable["mycelium.test"]).toEqual("airtable-1");
        });
      }
    );
