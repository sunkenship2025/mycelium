import { assert, DVault, NoteProps, NoteUtils } from "@myceliumhq/common-all";
import { vault2Path } from "@myceliumhq/common-server";
import {
  NoteTestUtilsV4,
  testAssertsInsideCallback,
} from "@myceliumhq/common-test-utils";
import { ENGINE_HOOKS, ENGINE_HOOKS_MULTI } from "@myceliumhq/engine-test-utils";
    ...
    describeSingleWS(
      ...
      () => {
        test("THEN export payload must contain the selection as note body", async () => {
          ...
          expect((payload?.payload as NoteProps[])[0].fname).toEqual("root");
          expect((payload?.payload as NoteProps[]).length).toEqual(1);
          expect(payload?.payload[0].body).toEqual("# Welcome to Mycelium");
        });
      }
    );
  });
