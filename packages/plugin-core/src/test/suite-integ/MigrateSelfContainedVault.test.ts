import { SinonStubbedFn } from "@myceliumhq/common-test-utils";
...
import { DConfig, pathForVaultRoot } from "@myceliumhq/common-server";
...
async function verifyVaultNotMigrated({
  ...
  expect(
    await fs.pathExists(path.join(vaultFolder, CONSTANTS.MYCELIUM_CONFIG_FILE))
  ).toBeFalsy();
  expect(
    await fs.pathExists(path.join(vaultFolder, CONSTANTS.MYCELIUM_WS_NAME))
  ).toBeFalsy();
  // No notes folder
