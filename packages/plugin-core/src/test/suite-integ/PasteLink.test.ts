  ...
  describeMultiWS(
    ...
    () => {
      test("THEN gets link with metadata", async () => {
        ...
        utils.clipboard.writeText("https://mycelium.so");
        sinon
        ...
        expect(formattedLink).toEqual(`[Mycelium Home](https://mycelium.so)`);
      });
    }
  );
  ...
  describeMultiWS(
    ...
    () => {
      test("THEN gets raw link", async () => {
        ...
        const formattedLink = await new PasteLinkCommand().run();
        expect(formattedLink).toEqual(`<https://mycelium.so>`);
      });
    }
  );
});
