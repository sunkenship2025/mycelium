import { NoteProps, VaultUtils } from "@myceliumhq/common-all";
import {
  AssertUtils,
  FileTestUtils,
  NoteTestUtilsV4,
} from "@myceliumhq/common-test-utils";
import { ENGINE_HOOKS_MULTI } from "@myceliumhq/engine-test-utils";
...
suite("GIVEN ReferenceHoverProvider", function () {
  ...
  describeMultiWS(
    "AND WHEN used in non-mycelium file",
    {
      ctx,
          ...
          onInit: async ({ vaults }) => {
            ...
            await AssertUtils.assertInString({
              ...
              match: [
                ...
                `use "Mycelium: Go to Note" command`,
              ],
            });
            done();
          },
        ...
        runLegacyMultiWorkspaceTest({
          ...
          preSetupHook: async ({ wsRoot, vaults }) => {
            ...
            await NoteTestUtilsV4.createNote({
              ...
              wsRoot,
              fname: "source",
              body: `[[mycelium://${VaultUtils.getName(vaults[1])}/target]]`,
            });
          },
          onInit: async ({ vaults }) => {
            const editor = await WSUtils.openNoteByPath({
        ...
        runLegacyMultiWorkspaceTest({
          ...
          preSetupHook: async ({ wsRoot, vaults }) => {
            ...
            await NoteTestUtilsV4.createNote({
              ...
              wsRoot,
              fname: "source",
              body: `[[My note: with an alias|mycelium://${VaultUtils.getName(
                vaults[1]
          ...
          test("THEN display message to create it with go to note", async () => {
            ...
            expect(
              await AssertUtils.assertInString({
                body: hover!.contents.join(""),
                match: [
                  'Note foo.bar.baz is missing, use "Mycelium: Go to Note" command to create it.',
                ],
              })
            ).toBeTruthy();
          });
