...
} from "@myceliumhq/engine-test-utils";
...
export const MYCELIUM_REMOTE =
  "https://github.com/myceliumhq/mycelium-site-vault.git";
export const MYCELIUM_REMOTE_VAULT = {
  fsPath: "mycelium-site-vault",
  remote: {
    type: "git" as const,
    url: MYCELIUM_REMOTE,
  },
...
type SetupWorkspaceType = {
  /** The type of workspace to create for the test, Native (w/o mycelium.code-worksace) or Code (w/ mycelium.code-workspace) */
  workspaceType?: WorkspaceType;
  /** If true, create a self contained vault as the workspace.
   *
   * Setting this option will also override the VSCode setting `mycelium.enableSelfContainedVaultWorkspace`.
   *
...
export type SetupLegacyWorkspaceMultiOpts = SetupCodeConfigurationV2 &
  SetupWorkspaceType & {
    ...
    setupWsOverride?: Omit<Partial<SetupWorkspaceOpts>, "workspaceType">;
    /**
     * Overrid default Mycelium settings (https://mycelium.so/notes/eea2b078-1acc-4071-a14e-18299fc28f48.html)
     */
...
export async function setupLegacyWorkspaceMulti(
  ...
  if (copts.workspaceType === WorkspaceType.CODE) {
    ...
    workspaceFile = MyceliumExtension.workspaceFile();
    workspaceFolders = MyceliumExtension.workspaceFolders();
...
export function setupBeforeAfter(
  ...
  beforeEach(async () => {
    // MyceliumWorkspace.getOrCreate(ctx);
...
export function describeMultiWS(
  ...
  opts: SetupLegacyWorkspaceMultiOpts & {
    ...
    /**
     * Run after the workspace is crated, but before mycelium is activated
     */
     ...
     * See [[Breakpoints|mycelium://mycelium.docs/pkg.plugin-core.qa.debug#breakpoints]] for more details
     */
    timeout?: number;
    noSetInstallStatus?: boolean;
    skipMigrations?: boolean;
...
export function subscribeToEngineStateChange(
  ...
  const engineClient = toMyceliumEngineClient(ExtensionProvider.getEngine());
  return engineClient.onEngineNoteStateChanged(callback);
}

export function toMyceliumEngineClient(engine: DEngineClient) {
  return engine as MyceliumEngineClient;
}
