...
import { file2Note } from "@myceliumhq/common-server";
import { SetupHookFunction } from "@myceliumhq/common-test-utils";
...
export function genDefaultSettings() {
  return {
    extensions: {
      recommendations: [
        "mycelium.mycelium",
        "mycelium.mycelium-paste-image",
        "mycelium.mycelium-markdown-shortcuts",
        "redhat.vscode-yaml",
      ],
      unwantedRecommendations: [
        "mycelium.mycelium-markdown-links",
        "mycelium.mycelium-markdown-notes",
        "mycelium.mycelium-markdown-preview-enhanced",
...
/**
 * Setup MyceliumExtension config options
 * @param opts
 */
export function setupCodeConfiguration(opts: SetupCodeConfigurationV2) {
  ...
  MyceliumExtension.configuration = () => {
    const config: any = {
      mycelium: {
        rootDir: ".",
      },
...
export async function resetCodeWorkspace() {
  // @ts-ignore
  MyceliumExtension.workspaceFile = () => {
    return undefined;
  };
  // @ts-ignore
...
export const stubWorkspaceFile = (wsRoot: string) => {
  const wsPath = path.join(wsRoot, "mycelium.code-workspace");
...
export const stubWorkspaceFolders = (wsRoot: string, vaults: DVault[]) => {
  ...
  MyceliumExtension.workspaceFolders = () => folders;
};
