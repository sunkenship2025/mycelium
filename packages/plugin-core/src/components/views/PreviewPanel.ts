...
export class PreviewPanel implements PreviewProxy, vscode.Disposable {
  ...
  /**
   *
   * @param param0 extension - IMyceliumExtension implementation. linkHandler -
  ...
  private setupCallbacks(): void {
    ...
    // If the user changes focus, then the newly in-focus Mycelium note should be
  ...
  private rewriteImageUrls = memoize({
    fn: (note: NoteProps, panel: vscode.WebviewPanel) => {
      ...
      visit(
        tree,
        [MyceliumASTTypes.IMAGE, MyceliumASTTypes.EXTENDED_IMAGE],
        (image: Image) => {
