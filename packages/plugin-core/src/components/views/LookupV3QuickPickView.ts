...
import {
  ButtonType,
  MyceliumBtn,
  IMyceliumQuickInputButton,
} from "../lookup/ButtonTypes";
import { ILookupViewModel } from "../lookup/LookupViewModel";
import { MyceliumQuickPickerV2, VaultSelectionMode } from "../lookup/types";
...
export class LookupV3QuickPickView implements Disposable {
  ...
  private setupViewModel(): void {
    ...
    this._disposables.push(
      this._viewState.nameModifierMode.bind(async (newValue) => {
        ...
        const validButtons: MyceliumBtn[] = [];
  ...
  private updateButtonsOnQuickPick(...btns: MyceliumBtn[]): void {
    const newButtons = this._quickPick!.buttons.map((b: MyceliumBtn) => {
  ...
  private onTriggerButton = (btn: QuickInputButton) => {
    const btnType = (btn as IMyceliumQuickInputButton).type;
    ...
    AnalyticsUtils.track(LookupEvents.LookupModifierToggledByUser, {
      ...
      type: (btn as IMyceliumQuickInputButton).type,
      pressed: (btn as IMyceliumQuickInputButton).pressed,
    });
  };
}
