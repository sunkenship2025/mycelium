import { assertUnreachable, DVault, VaultUtils } from "@myceliumhq/common-all";
import { DLogger } from "@myceliumhq/common-server";
import { HistoryEvent } from "@myceliumhq/engine-server";
import {
  CopyAsFormat,
...
import { MyceliumContext } from "../../constants";
  ...
  public static async createNewServiceConfig(connectionType: ExternalService) {
    switch (connectionType) {
      ...
      case ExternalService.GoogleDocs: {
        ...
        vscode.window.showInformationMessage(
          "Google OAuth is a beta feature. Please contact us at support@mycelium.so or on Discord to first gain access. Then, try again and authenticate with Google on your browser to continue."
        );
        break;
      }
  ...
  private static getDescriptionForScope(scope: PodExportScope): string {
    switch (scope) {
      ...
      case PodExportScope.Workspace:
        return "Exports all notes in the Mycelium workspace";
  ...
  private static getDescriptionForPodType(type: PodV2Types): string {
    switch (type) {
      ...
      case PodV2Types.MarkdownExportV2:
        return "Formats Mycelium markdown and exports it to the clipboard or local file system";

      case PodV2Types.GoogleDocsExportV2:
        return "Formats Mycelium note to google doc";
  ...
  > {
    const items = getAllCopyAsFormat().map<QuickPickItem>((value) => {
      ...
      return {
        ...
        detail: `Format Mycelium note to ${value} and copy it to the clipboard`,
      };
    });
