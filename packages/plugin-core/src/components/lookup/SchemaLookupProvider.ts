...
import { getDurationMilliseconds } from "@myceliumhq/common-server";
...
import { MyceliumQuickPickerV2, MyceliumQuickPickState } from "./types";
  ...
  onDidAccept(opts: {
    ...
    return async () => {
      ...
      if (
        ...
        picker.state = MyceliumQuickPickState.PENDING_NEXT_PICK;
      ...
      if (isMultiLevel) {
        window
          .showInformationMessage(
            "It looks like you are trying to create a multi-level [schema](https://wiki.mycelium.so/notes/c5e5adde-5459-409b-b34d-a0d75cbb1052.html). This is not supported. If you are trying to create a note instead, run the `> Note Lookup` command or click on `Note Lookup`",
            ...["Note Lookup"]
          )
