...
import { MyceliumBtn, getButtonCategory } from "./ButtonTypes";
import {
...
import {
  MyceliumQuickPickerV2,
  MyceliumQuickPickState,
  VaultSelectionMode,
} from "./types";
...
export async function showDocAndHidePicker(
  uris: Uri[],
  picker: MyceliumQuickPickerV2
) {
  const ctx = "showDocAndHidePicker";
  const maybeSplitSelection = _.find(picker.buttons, (ent: MyceliumBtn) => {
    return getButtonCategory(ent) === "split" && ent.pressed;
  });
  let viewColumn = ViewColumn.Active;
  if (maybeSplitSelection) {
    const splitType = (maybeSplitSelection as MyceliumBtn).type;
...
export class ProviderAcceptHooks {
  ...
  static oldNewLocationHook: OnAcceptHook = async ({
    ...
    if (newNote && !isStub) {
      ...
      return {
        error: new MyceliumError({ message: errMsg }),
      };
    }
...
export class PickerUtilsV2 {
  static createMyceliumQuickPick(
    ...
    const quickPick =
      window.createQuickPick<DNodePropsQuickInputV2>() as MyceliumQuickPickerV2;
    quickPick.title = title;
    quickPick.state = MyceliumQuickPickState.IDLE;
  ...
  static createMyceliumQuickPickItem(
  ...
  static createMyceliumQuickPickItemFromNote(
    opts: NoteProps
  ): DNodePropsQuickInputV2 {
    return {
      ...opts,
  ...
  static getValue(picker: MyceliumQuickPickerV2) {
    return picker.value;
  }

  static getSelection(picker: MyceliumQuickPickerV2): DNodePropsQuickInputV2[] {
  ...
  static hasNextPicker = (
  ...
  ): quickpick is Required<MyceliumQuickPickerV2> => {
    ...
    const isNewPickAllowed = ["lookup", "mycelium.moveHeader"];
    return (
  ...
  static resetPaginationOpts(picker: MyceliumQuickPickerV2) {
