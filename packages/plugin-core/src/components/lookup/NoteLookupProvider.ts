...
import { IMyceliumQuickInputButton } from "./ButtonTypes";
import {
  CREATE_NEW_LABEL,
    ...
    return async () => {
      ...
      picker.buttons.forEach((button) => {
        AnalyticsUtils.track(LookupEvents.LookupModifiersSetOnAccept, {
          command: this.id,
          type: (button as IMyceliumQuickInputButton).type,
          pressed: (button as IMyceliumQuickInputButton).pressed,
        });
      });
      ...
      if (!this.opts.noHidePickerOnAccept) {
        picker.state = MyceliumQuickPickState.FULFILLED;
        picker.hide();
      }
