...
import { HistoryService, WorkspaceUtils } from "@myceliumhq/engine-server";
import { LinkUtils } from "@myceliumhq/unified";
...
import { ButtonType, MyceliumBtn } from "./ButtonTypes";
import type {
  CreateQuickPickOpts,
  ILookupControllerV3,
...
import { MyceliumQuickPickerV2, VaultSelectionMode } from "./types";
 ...
 * see [[mycelium://mycelium.docs/pkg.plugin-core.t.lookup.arch]]
 */
export class LookupControllerV3 implements ILookupControllerV3 {
  ...
  private _quickPick?: MyceliumQuickPickerV2;
  ...
  private _initButtons: MyceliumBtn[];
  ...
  public get quickPick(): MyceliumQuickPickerV2 {
  ...
  public get cancelToken() {
    if (_.isUndefined(this._cancelTokenSource)) {
      throw new MyceliumError({ message: "no cancel token" });
    }
  ...
  get provider() {
    if (_.isUndefined(this._provider)) {
      throw new MyceliumError({ message: "no provider" });
    }
  ...
  public async prepareQuickPick(
    ...
    const quickpick = PickerUtilsV2.createMyceliumQuickPick(opts);
  ...
  private getButtonFromArray(type: ButtonType, buttons: MyceliumBtn[]) {
    return _.find(buttons, (value) => value.type === type);
  }

  private getButton(type: ButtonType): MyceliumBtn | undefined {
    if (this._quickPick) {
   ...
   * @param buttons
   */
  private initializeViewStateFromButtons(buttons: MyceliumBtn[]) {
    if (
  ...
  private onJournalButtonToggled(enabled: boolean) {
    ...
    if (enabled) {
      quickPick.modifyPickerValueFunc = () => {
        try {
          return MyceliumClientUtilsV2.genNoteName(LookupNoteTypeEnum.journal);
        } catch (error) {
  ...
  private onScratchButtonToggled(enabled: boolean) {
    ...
    if (enabled) {
      quickPick.modifyPickerValueFunc = () => {
        try {
          return MyceliumClientUtilsV2.genNoteName(LookupNoteTypeEnum.scratch);
        } catch (error) {
          return { noteName: "", prefix: "" };
        }
      };
  ...
  private async onTaskButtonToggled(enabled: boolean) {
    ...
    if (enabled) {
      quickPick.modifyPickerValueFunc = () => {
        try {
          return MyceliumClientUtilsV2.genNoteName(LookupNoteTypeEnum.task);
        } catch (error) {
          return { noteName: "", prefix: "" };
        }
      };
        ...
        if (!_.isUndefined(document)) {
          ...
          if (leaveTrace) {
            ...
            const link = NoteUtils.createWikiLink({
              note,
              useVaultPrefix: MyceliumClientUtilsV2.shouldUseVaultPrefix(
