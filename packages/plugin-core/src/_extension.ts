...
import { MyceliumExtension, getDWorkspace, getExtension } from "./workspace";
...
export async function _activate(
  ...
  const maybeUUIDPath = path.join(os.homedir(), CONSTANTS.MYCELIUM_ID);
  ...
  try {
    ...
    const ws = await MyceliumExtension.getOrCreate(context, {
    ...
    if (!existingCommands.includes(MYCELIUM_COMMANDS.RELOAD_INDEX.key)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(
          MYCELIUM_COMMANDS.RELOAD_INDEX.key,
    ...
    // Need to recompute this for tests, because the instance of MyceliumExtension doesn't get re-created.
    ...
    if (await MyceliumExtension.isMyceliumWorkspace()) {
      ...
      const extensions = Extensions.getMyceliumExtensionRecommendations().map(
        ({ id, extension: ext }) => {
          return {
            id,
      ...
      // ws not active
      Logger.info({ ctx, msg: "mycelium not active" });
    ...
    if (!opts?.skipInteractiveElements) {
      await showWelcomeOrWhatsNew({
        extensionInstallStatus,
        isSecondaryInstall,
        version: MyceliumExtension.version(),
    ...
    if (MyceliumExtension.isActive(context)) {
      HistoryService.instance().add({
        source: "extension",
        action: "activate",
      });
...
async function showWelcomeOrWhatsNew({
    ...
    case InstallStatus.UPGRADED: {
      ...
      vscode.window
        .showInformationMessage(
          `Mycelium has been upgraded to ${version}`,
          buttonAction
        )
        .then((resp) => {
          if (resp === buttonAction) {
            ...
            vscode.commands.executeCommand(
              "vscode.open",
              vscode.Uri.parse(
                "https://mycelium.so/notes/9bc92432-a24c-492b-b831-4d5378c1692b.html"
              )
            );
          }
        });
  ...
  // Show lapsed users (users who have installed Mycelium but haven't initialied
...
async function _setupCommands({
  ...
  if (requireActiveWorkspace === true) {
    if (!existingCommands.includes(MYCELIUM_COMMANDS.GO_NEXT_HIERARCHY.key)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(
          MYCELIUM_COMMANDS.GO_NEXT_HIERARCHY.key,
    ...
    if (!existingCommands.includes(MYCELIUM_COMMANDS.GO_PREV_HIERARCHY.key)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(
          MYCELIUM_COMMANDS.GO_PREV_HIERARCHY.key,
          sentryReportingCallback(async () => {
    ...
    if (!existingCommands.includes(MYCELIUM_COMMANDS.TOGGLE_PREVIEW.key)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(
          MYCELIUM_COMMANDS.TOGGLE_PREVIEW.key,
    ...
    if (!existingCommands.includes(MYCELIUM_COMMANDS.TOGGLE_PREVIEW_LOCK.key)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(
          MYCELIUM_COMMANDS.TOGGLE_PREVIEW_LOCK.key,
          sentryReportingCallback(async (args) => {
            if (args === undefined) {
              args = {};
            }
    ...
    if (!existingCommands.includes(MYCELIUM_COMMANDS.SHOW_SCHEMA_GRAPH.key)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(
          MYCELIUM_COMMANDS.SHOW_SCHEMA_GRAPH.key,
          sentryReportingCallback(async () => {
    ...
    if (!existingCommands.includes(MYCELIUM_COMMANDS.SHOW_NOTE_GRAPH.key)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(
          MYCELIUM_COMMANDS.SHOW_NOTE_GRAPH.key,
          sentryReportingCallback(async () => {
    ...
    if (!existingCommands.includes(MYCELIUM_COMMANDS.CONFIGURE_UI.key)) {
      context.subscriptions.push(
        vscode.commands.registerCommand(
          MYCELIUM_COMMANDS.CONFIGURE_UI.key,
          sentryReportingCallback(async () => {
  ...
  ExtensionUtils.addCommand({
    context,
    key: MYCELIUM_COMMANDS.SEED_ADD.key,
  ...
  ExtensionUtils.addCommand({
    context,
    key: MYCELIUM_COMMANDS.SEED_REMOVE.key,
    cmd: new SeedRemoveCommand(),
    existingCommands,
  });

  if (!existingCommands.includes(MYCELIUM_COMMANDS.SEED_BROWSE.key)) {
    context.subscriptions.push(
      vscode.commands.registerCommand(
        MYCELIUM_COMMANDS.SEED_BROWSE.key,
        sentryReportingCallback(async () => {
