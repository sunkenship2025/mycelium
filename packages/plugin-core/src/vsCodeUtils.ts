...
import { goUpTo, resolvePath, tmpDir } from "@myceliumhq/common-server";
...
import { MyceliumContext, GLOBAL_STATE } from "./constants";
...
export class VSCodeUtils {
  ...
  static getActiveTextEditorOrThrow() {
    ...
    if (!editor) {
      throw new MyceliumError({ message: "no active editor" });
    }
    return editor;
  }
  ...
  static getInstallStatusForWorkspace({
    ...
    if (
      ...
      previousWorkspaceVersion === CONSTANTS.MYCELIUM_INIT_VERSION
    ) {
  ...
  static getInstallStatusForExtension({
    ...
    if (
      ...
      previousGlobalVersion === CONSTANTS.MYCELIUM_INIT_VERSION
    ) {
      return InstallStatus.INITIAL_INSTALL;
    }
  ...
  static setContext(key: MyceliumContext, status: boolean) {
    vscode.commands.executeCommand("setContext", key, status);
  }

  static setContextStringValue(key: MyceliumContext, value: string) {
  ...
  /** Opposite of `toRangeObject`, which is required to call Mycelium APIs. */
