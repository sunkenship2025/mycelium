...
import { DConfig, isInsidePath } from "@myceliumhq/common-server";
import { AnchorUtils } from "@myceliumhq/unified";
  ...
  implements Disposable
{
  ...
  key = MYCELIUM_COMMANDS.COPY_NOTE_LINK.key;
    ...
    if (anchorType === "prompt") {
      ...
      const preference = await window.showQuickPick(
        ...
        ],
        {
          ...
          ignoreFocusOut: true,
          title:
            "What type of anchors should Mycelium create for links in non-note files?",
        }
      );
  ...
  private async createNoteLink(editor: TextEditor, note: NotePropsMeta) {
    ...
    return {
      link: NoteUtils.createWikiLink({
        ...
        useVaultPrefix: MyceliumClientUtilsV2.shouldUseVaultPrefix(engine),
