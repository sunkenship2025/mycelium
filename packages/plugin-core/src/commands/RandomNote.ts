import { ConfigUtils, NotePropsMeta, NoteUtils } from "@myceliumhq/common-all";
...
export class RandomNoteCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.RANDOM_NOTE.key;
  ...
  async execute(_opts: CommandOpts): Promise<CommandOutput> {
    ...
    if (noteCount === 0) {
      window.showInformationMessage(
        "No notes match the search pattern. Adjust the patterns with the Mycelium:Configure (yaml) command"
      );
      return;
    }
