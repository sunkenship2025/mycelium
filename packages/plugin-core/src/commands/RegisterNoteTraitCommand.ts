import { CONSTANTS, MyceliumError } from "@myceliumhq/common-all";
 ...
 * https://wiki.mycelium.so/notes/EQoaBI8A0ZcswKQC3UMpO/ for examples and
 * documentation.
 *
 ...
 * @type {object}
 * @property {string} currentNoteName The name of the currently opened Mycelium
...
export class RegisterNoteTraitCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.REGISTER_NOTE_TRAIT.key;
  ...
  async execute(opts: CommandOpts): Promise<CommandOutput> {
    ...
    const scriptPath = path.join(
      wsRoot,
      CONSTANTS.MYCELIUM_USER_NOTE_TRAITS_BASE,
      opts.traitId + ".js"
    );
