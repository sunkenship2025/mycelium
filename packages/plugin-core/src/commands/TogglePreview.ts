import { NoteProps, NoteUtils } from "@myceliumhq/common-all";
...
export class TogglePreviewCommand extends InputArgCommand<
  ...
  TogglePreviewCommandOutput
> {
  key = MYCELIUM_COMMANDS.TOGGLE_PREVIEW.key;
  _panel: PreviewProxy;
   ...
   * behavior of showing the contents of the currently in-focus Mycelium note.
   */
  async execute(opts?: TogglePreviewCommandOpts) {
    ...
    if (note) {
      ...
      // We can't find the note, so this is not in the Mycelium workspace.
