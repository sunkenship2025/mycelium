import { DNodeUtils, NoteChangeEntry, VaultUtils } from "@myceliumhq/common-all";
...
/**
 * This is not `Mycelium: Rename Note`. For that, See [[../packages/plugin-core/src/commands/RenameNoteCommand.ts]]
...
export class RenameNoteV2aCommand extends BaseCommand<
  ...
  CommandInput
> {
  key = MYCELIUM_COMMANDS.RENAME_NOTE_V2A.key;
  ...
  async showResponse(res: CommandOutput) {
    ...
    if (changed.length > 0 && !this.silent) {
      window.showInformationMessage(`Mycelium updated ${changed.length} files`);
    }
  }
