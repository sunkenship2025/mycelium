...
import { RemarkUtils } from "@myceliumhq/unified";
...
export class DoctorCommand extends BasicCommand<CommandOpts, CommandOutput> {
  key = MYCELIUM_COMMANDS.DOCTOR.key;
  ...
  async showIncompatibleExtensionPreview(opts: {
    ...
    const contents = [
      "# Extensions that are incompatible with Mycelium.",
      "",
      "The extensions listed below are known to be incompatible with Mycelium.",
      "",
      "Neither Mycelium nor the extension may function properly when installed concurrently.",
      "",
      "Consider disabling the incompatible extensions when in a Mycelium Workspace.",
      ...
      "",
      "See [Incompatible Extensions](https://wiki.mycelium.so/notes/9Id5LUZFfM1m9djl6KgpP) for more details.",
      "",
  ...
  async showFixInvalidFileNamePreview(opts: {
    ...
    const contents = [
      ...
      "",
      "Please see [Restrictions](https://wiki.mycelium.so/notes/v21pacjod0eqgdhb7zo7fvw) to learn more about file name restrictions.",
      "",
      "***",
      canRenameContent,
      "",
  ...
  private async reload() {
    ...
    if (_.isUndefined(engine)) {
      throw new MyceliumError({ message: "no engine found." });
    }
    return engine;
  }
  ...
  async execute(opts: CommandOpts) {
    ...
    if (_.isUndefined(wsRoot)) {
      throw new MyceliumError({ message: "rootDir undefined" });
    }
    if (_.isUndefined(config)) {
      throw new MyceliumError({ message: "no config found" });
    }
    ...
    if (opts.data?.note) {
      ...
      if (opts.scope === "file") {
        ...
        if (_.isUndefined(document)) {
          throw new MyceliumError({ message: "No note open." });
        }
    ...
    switch (opts.action) {
      ...
      case DoctorActionsEnum.ADD_MISSING_DEFAULT_CONFIGS: {
        ...
        if (out.resp) {
          const OPEN_CONFIG = "Open mycelium.yml and Backup";
          const message =
            opts.action === DoctorActionsEnum.REMOVE_DEPRECATED_CONFIGS
              ? `Deprecated configs removed. Backup of mycelium.yml created in ${out.resp.backupPath}`
              : `Missing defaults added. Backup of mycelium.yml created in ${out.resp.backupPath}`;
          window
