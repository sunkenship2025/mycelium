...
export class TaskCompleteCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.TASK_COMPLETE.key;
  ...
  async execute(_opts: CommandOpts) {
    ...
    if (complete === undefined) {
      ...
      await VSCodeUtils.showMessage(
        ...
        "You have no task statuses marked as complete. Please add something to 'taskCompleteStatus' in your configuration file. See: https://wiki.mycelium.so/notes/SEASewZSteDK7ry1AshNG#taskcompletestatus",
        {},
        { title }
      ).then((pressed) => {
