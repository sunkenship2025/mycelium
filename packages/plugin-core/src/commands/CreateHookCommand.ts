import { MyceliumError, DHookType, IMyceliumError } from "@myceliumhq/common-all";
...
type CommandOutput = { error: IMyceliumError } | void;

const hookTemplate = `
/**
 @params wsRoot: string, root of your current workspace
 @params note: Object with following properties https://github.com/myceliumhq/mycelium/blob/master/packages/common-all/src/types/foundation.ts#L66:L66
 @params NoteUtils: utilities for working with notes. [code](https://github.com/myceliumhq/mycelium/blob/master/packages/common-all/src/dnode.ts#L323:L323)
...
export class CreateHookCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.CREATE_HOOK.key;
  ...
  async execute({ hookName, hookFilter }: CommandOpts) {
    ...
    if (fs.existsSync(scriptPath)) {
      const error = MyceliumError.createPlainError({
