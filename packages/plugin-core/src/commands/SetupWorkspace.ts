...
import { resolveTilde } from "@myceliumhq/common-server";
import { WorkspaceService, WorkspaceUtils } from "@myceliumhq/engine-server";
...
type CommandOpts = CommandInput & {
  ...
  /** Create self contained vaults, overriding the Mycelium VSCode setting. */
...
export class SetupWorkspaceCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.INIT_WS.key;
  ...
  async gatherInputs(): Promise<CommandInput | undefined> {
    ...
    if (
      ...
      // If there's a non-Mycelium workspace already open, offer to convert that to a Mycelium workspace first
      ...
      if (
        ...
        rootDirRaw = await VSCodeUtils.gatherFolderPath({
          ...
          override: {
            title: "Path for Mycelium Native Workspace",
    ...
    if (!this.isEmptyDirectory(rootDir) && !skipConfirmation) {
        ...
        [
          {
            ...
            detail: `Continue creating a workspace, putting Mycelium files into the existing folder.`,
          },
          {
      ...
      if (resp === undefined || resp.label === EXISTING_ROOT_ACTIONS.ABORT) {
        vscode.window.showInformationMessage(
          "did not initialize mycelium workspace"
        );
        return false;
      }
  ...
  async execute(
    ...
    if (!skipOpenWs) {
      vscode.window.showInformationMessage("opening mycelium workspace");
      if (workspaceType === WorkspaceType.CODE) {
        VSCodeUtils.openWS(
          vscode.Uri.file(path.join(rootDir, CONSTANTS.MYCELIUM_WS_NAME)).fsPath
        );
