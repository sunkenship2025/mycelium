...
import { MYCELIUM_COMMANDS, MYCELIUM_REMOTE_VAULTS } from "../constants";
...
export class AddExistingVaultCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.ADD_EXISTING_VAULT.key;
  ...
  generateRemoteEntries = (): SourceQuickPickEntry[] => {
    return MYCELIUM_REMOTE_VAULTS.map(
  ...
  async gatherVaultSelfContained(
    ...
    if (sourceType === VaultType.LOCAL) {
      ...
      const remote = await VSCodeUtils.showInputBox({
        ...
        placeHolder: "git@github.com:myceliumhq/mycelium.git",
        ignoreFocusOut: true,
      });
      // Cancelled
    ...
    const sourceTypeSelected = await VSCodeUtils.showQuickPick([
      {
        ...
        picked: true,
        detail: "eg. /home/mycelium/hello-vault",
        description:
          "A local vault is a Mycelium vault that is present in your computer",
      },
      {
        label: VaultType.REMOTE,
        detail: "eg. git@github.com:myceliumhq/mycelium-site.git",
        description:
          "A remote vault is a Mycelium vault that is available at a git endpoint",
      },
    ]);
    ...
    return window.withProgress(
      ...
      async (progress) => {
        ...
        if (workspace) {
          ...
          // Mycelium expects it, because we can't override the path.
  ...
  async checkAndWarnTransitiveDeps(opts: {
    ...
    try {
      if (
        await fs.pathExists(
          path.join(vaultRootPath, CONSTANTS.MYCELIUM_CONFIG_FILE)
        )
      ) {
        const vaultConfig = DConfig.getRaw(vaultRootPath) as MyceliumConfig;
        if (ConfigUtils.getVaults(vaultConfig)?.length > 1) {
          ...
          if (select?.title === openDocsOption) {
            ...
            await PluginFileUtils.openWithDefaultApp(
              "https://wiki.mycelium.so/notes/q9yo0y7czv8mxlkbnw1ugj1"
            );
          }
        }
      }
