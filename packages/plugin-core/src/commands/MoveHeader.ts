...
import { DConfig, file2Note, vault2Path } from "@myceliumhq/common-server";
import { Heading, HistoryEvent, Node } from "@myceliumhq/engine-server";
import {
  MDUtilsV5,
  Processor,
  MyceliumASTNode,
...
export class MoveHeaderCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.MOVE_HEADER.key;

  private headerNotSelectedError = new MyceliumError({
    message: "You must first select the header you want to move.",
    severity: ERROR_SEVERITY.MINOR,
  });

  private noActiveNoteError = new MyceliumError({
    message: "No note open.",
    severity: ERROR_SEVERITY.MINOR,
  });

  private noNodesToMoveError = new MyceliumError({
    message:
      "There are no nodes to move. If your selection is valid, try again after reloading VSCode.",
    severity: ERROR_SEVERITY.MINOR,
  });

  private noDestError = new MyceliumError({
  ...
  private async validateAndProcessInput(engine: IEngineAPIService): Promise<{
    ...
    const bodyAST: MyceliumASTNode = proc.parse(
      maybeNote.body
    ) as MyceliumASTNode;
    ...
    visit(parsedLine, [MyceliumASTTypes.HEADING], (heading: Heading, index) => {
      targetHeader = heading;
      targetIndex = index;
      return false;
    });
    ...
    return new Promise((resolve) => {
      ...
      NoteLookupProviderUtils.subscribe({
        ...
        onDone: async (event: HistoryEvent) => {
          ...
          const quickpick: MyceliumQuickPickerV2 = lc.quickPick;
          ...
          disposable?.dispose();
          VSCodeUtils.setContext(MyceliumContext.NOTE_LOOK_UP_ACTIVE, false);
        },
      });
      this.promptForDestination(lc, opts);

      VSCodeUtils.setContext(MyceliumContext.NOTE_LOOK_UP_ACTIVE, true);
