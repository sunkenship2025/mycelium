...
export class InsertNoteIndexCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.INSERT_NOTE_INDEX.key;
  ...
  async execute(opts: CommandOpts) {
    ...
    if (_.isUndefined(maybeEditor)) {
      window.showErrorMessage(
        "No active text editor found. Try running this command in a Mycelium note."
      );
      return opts;
    }
    const activeNote = await WSUtils.getNoteFromDocument(maybeEditor.document)!;
    if (_.isUndefined(activeNote)) {
      window.showErrorMessage("Active file is not a Mycelium note.");
      return opts;
    }
