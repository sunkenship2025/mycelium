import { MyceliumError, NoteTrait } from "@myceliumhq/common-all";
...
export class CreateNoteWithUserDefinedTrait extends BaseCommand<
  ...
  CommandInput
> {
  key = MYCELIUM_COMMANDS.CREATE_USER_DEFINED_NOTE.key;
  ...
  async execute(opts: CommandOpts): Promise<CommandOpts> {
    ...
    if (!cmd) {
      throw new MyceliumError({ message: "Unexpected unregistered type" });
    }
