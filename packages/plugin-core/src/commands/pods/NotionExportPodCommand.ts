import { ErrorFactory, NoteProps, ResponseUtil } from "@myceliumhq/common-all";
import {
  Client,
  ConfigFileUtils,
...
export class NotionExportPodCommand extends BaseExportPodCommand<
  ...
  NotionExportReturnType
> {
  public key = "mycelium.notionexport";
      ...
      if (opts?.connectionId) {
        ...
        if (!config) {
          window.showErrorMessage(
            `Couldn't find service config with the specified connection ID ${opts.connectionId}. Ensure that the connection ID is correct. You can check existing connections with the Mycelium: Configure Service Connection command.`
          );
          return;
        }
