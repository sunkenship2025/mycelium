...
export class CreateDailyJournalCommand extends CreateNoteWithTraitCommand {
  ...
  public static MYCELIUM_TEMPLATES_FNAME: string = "templates";
  ...
  constructor(ext: IMyceliumExtension) {
    ...
    super(ext, "mycelium.journal", initTrait);
    // override the key to maintain compatibility
    this.key = MYCELIUM_COMMANDS.CREATE_DAILY_JOURNAL_NOTE.key;
  }
  ...
  private async makeSchemaFileIfNotExisting(
    ...
    const uri = vscode.Uri.file(
      SchemaUtils.getPath({ root: vaultPath, fname: `mycelium.${dailyDomain}` })
    );
    ...
    const tokenizedMatrix: SchemaToken[][] = [
      [
        ...
        },
        {
          ...
          template: {
            id:
              CreateDailyJournalCommand.MYCELIUM_TEMPLATES_FNAME +
              `.${dailyDomain}`,
            type: "note",
          },
        },
  ...
  private async createTemplateFileIfNotExisting(
    ...
    const myceliumWSTemplate = VSCodeUtils.joinPath(assetUri, "mycelium-ws");

    const src = path.join(myceliumWSTemplate.fsPath, "templates", fileName);
