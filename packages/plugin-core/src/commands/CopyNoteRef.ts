...
import { refLink2Stringv2 } from "@myceliumhq/engine-server";
...
export class CopyNoteRefCommand extends BasicCommand<
  ...
  CommandOutput
> {
  key = MYCELIUM_COMMANDS.COPY_NOTE_REF.key;
  ...
  async execute(_opts: CommandOpts) {
    ...
    if (note) {
      const useVaultPrefix = MyceliumClientUtilsV2.shouldUseVaultPrefix(engine);
