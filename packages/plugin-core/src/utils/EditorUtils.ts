...
import { Heading } from "@myceliumhq/engine-server";
import {
  AnchorUtils,
  BlockAnchor,
  MyceliumASTDest,
...
export class EditorUtils {
  ...
  static getHeaderAt({
    ...
    if (headerLine.startsWith("#")) {
      ...
      const header = select(MyceliumASTTypes.HEADING, parsed) as Heading | null;
  ...
  static async isBrokenWikilink({
    ...
    await MdastUtils.visitAsync(
      ...
      async (linkvalue) => {
        ...
        fname =
          link.type === MyceliumASTTypes.WIKI_LINK ? link.value : link.fname;
  ...
  static async selectionContainsFrontmatter(opts: {
    ...
    if (frontmatterEndPosition) {
      ...
      throw new MyceliumError({ message: "Note a note." });
    }
  }
}
