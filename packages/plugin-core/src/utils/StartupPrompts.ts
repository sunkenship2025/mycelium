import { Time, VSCodeEvents } from "@myceliumhq/common-all";
import {
...
export class StartupPrompts {
  ...
  static shouldDisplayLapsedUserMsg(): boolean {
    ...
    // If the user has never initialized, has never activated a mycelium workspace,
    // and it's time to refresh the lapsed user message
    return (
      !metaData.myceliumWorkspaceActivated &&
  ...
  static async showLapsedUserMessage(assetUri: vscode.Uri) {
    ...
    vscode.window
      .showInformationMessage(
        "Hey, we noticed you haven't started using Mycelium yet. Would you like to get started?",
        { modal: true },
        { title: START_TITLE }
      )
      .then(async (resp) => {
