...
import { DConfig, readMD } from "@myceliumhq/common-server";
import {
  DEPRECATED_PATHS,
  DoctorActionsEnum,
  execa,
...
export class StartupUtils {
  ...
  static showManualUpgradeMessage() {
    ...
    const MESSAGE =
      "You are upgrading from a legacy version of Mycelium. Please follow the instructions to manually migrate your configuration.";
    vscode.window
      ...
      .then(async (resp) => {
        if (resp === SHOW_ME_HOW) {
          ...
          VSCodeUtils.openLink(
            "https://wiki.mycelium.so/notes/4119x15gl9w90qx8qh1truj"
          );
        } else {
          AnalyticsUtils.track(MigrationEvents.ManualUpgradeMessageConfirm, {
  ...
  static async runMigrationsIfNecessary({
    ...
    // see [[Migration|mycelium://mycelium.docs/pkg.plugin-core.t.migration]] for overview of migration process
  ...
  static getDuplicateKeysMessage(opts: { ext: IMyceliumExtension }) {
  ...
  static showDuplicateConfigEntryMessage(opts: {
    ...
    const MESSAGE =
      "We have detected duplicate key(s) in mycelium.yml. Mycelium has activated using the last entry of the duplicate key(s)";
    vscode.window
      ...
      .then(async (resp) => {
        if (resp === FIX_ISSUE) {
          ...
          const content = [
            `# Duplicate Keys in \`mycelium.yml\``,
            "",
            "The message at the bottom displays the _first_ duplicate key mapping that was detected in `mycelium.yml`",
            "",
            "**There may be more duplicate key mappings**.",
            "",
            "Take the following steps to fix this issue.",
            "1. Look through `mycelium.yml` and remove all duplicate mappings.",
            "",
            `    - We recommend installing the [YAML extension](${vscode.Uri.parse(
              `command:workbench.extensions.search?${JSON.stringify(
                "@id:redhat.vscode-yaml"
              )}`
            )}) for validating \`mycelium.yml\``,
            "",
            "1. When you are done, save your changes made to `mycelium.yml`",
            "",
  ...
  static showDeprecatedConfigMessage(opts: { ext: IMyceliumExtension }) {
    AnalyticsUtils.track(ConfigEvents.DeprecatedConfigMessageShow);
    const REMOVE_CONFIG = "Remove Deprecated Configuration";
    const MESSAGE =
      "We have detected some deprecated configurations. Would you like to remove them from mycelium.yml?";
    vscode.window
  ...
  static showMissingDefaultConfigMessage(opts: { ext: IMyceliumExtension }) {
    AnalyticsUtils.track(ConfigEvents.ShowMissingDefaultConfigMessage);
    const ADD_CONFIG = "Add Missing Configuration";
    const MESSAGE =
      "We have detected a missing configuration. This may happen because a new configuration was introduced, or because an existing required configuration has been deleted. Would you like to add them to mycelium.yml?";
    vscode.window
  ...
  static shouldDisplayInactiveUserSurvey(): boolean {
    ...
    if (inactiveSurveyMsgStatus === InactvieUserMsgStatusEnum.cancelled) {
      ...
      const shouldSend =
        metaData.myceliumWorkspaceActivated !== undefined &&
  ...
  static warnIncompatibleExtensions(opts: { ext: IMyceliumExtension }) {
    ...
    if (shouldDisplayWarning) {
      ...
      vscode.window
        .showWarningMessage(
          "We have detected some extensions that may conflict with Mycelium. Further action is needed for Mycelium to function correctly",
          "Fix conflicts..."
        )
        .then(async (resp) => {
  ...
  static showUninstallMarkdownLinksExtensionMessage() {
    if (VSCodeUtils.isExtensionInstalled("mycelium.mycelium-markdown-links")) {
      vscode.window
        .showInformationMessage(
          "Please uninstall the Mycelium Markdown Links extension. Mycelium has the note graph feature built-in now and having this legacy extension installed will interfere with its functionality.",
          { modal: true },
          { title: "Uninstall" }
        )
        .then(async (resp) => {
  ...
  static maybeShowProductHuntMessage() {
    ...
    const title = "Mycelium Release Notes";
  ...
  static async showWhitelistingLocalhostDocsIfNecessary() {
    ...
    if (failed) {
      ...
      vscode.window
        .showWarningMessage(
          "Mycelium is facing issues while connecting with localhost. Please ensure that you don't have anything running that can block localhost.",
        ...
        .then((resp) => {
          if (resp === "Open troubleshooting docs") {
            ...
            vscode.commands.executeCommand(
              "vscode.open",
              "https://wiki.mycelium.so/notes/a6c03f9b-8959-4d67-8394-4d204ab69bfe/#whitelisting-localhost"
            );
          } else {
