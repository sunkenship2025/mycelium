import { launchv2, ServerUtils } from "@myceliumhq/api-server";
import {
  ConfigEvents,
  ConfigUtils,
  CONSTANTS,
...
import { MetadataService, WorkspaceService } from "@myceliumhq/engine-server";
import { ExecaChildProcess } from "execa";
import fs from "fs-extra";
import _ from "lodash";
import path from "path";
import * as vscode from "vscode";
import { CONFIG, MyceliumContext } from "../constants";
...
async function startServerProcess(): Promise<{
  ...
  if (getStage() !== "prod") {
    const out = await launchv2({
      logPath: path.join(__dirname, "..", "..", "mycelium.server.log"),
  ...
  try {
    ...
    // TODO: change to error, wait for https://github.com/myceliumhq/mycelium/issues/3227 to be resolved first
...
export class ExtensionUtils {
  ...
  static getExtension() {
    const extName =
      getStage() === "dev"
        ? "mycelium.@myceliumhq/plugin-core"
        : "mycelium.mycelium";
  ...
  static setWorkspaceContextOnActivate(myceliumConfig: MyceliumConfig) {
    if (VSCodeUtils.isDevMode()) {
      vscode.commands.executeCommand(
        "setContext",
        MyceliumContext.DEV_MODE,
        true
      );
    }
    // used for enablement of legacy show preview command.
    VSCodeUtils.setContext(
      MyceliumContext.HAS_LEGACY_PREVIEW,
      MarkdownUtils.hasLegacyPreview()
    );

    //used for enablement of export pod v2 command
    VSCodeUtils.setContext(
      MyceliumContext.ENABLE_EXPORT_PODV2,
      myceliumConfig.dev?.enableExportPodV2 ?? false
    );

    // @deprecate: should track as property of workspace init instead
    if (myceliumConfig.dev?.enableExportPodV2) {
  ...
  static async trackWorkspaceInit({
    ...
    const codeWorkspacePresent = await fs.pathExists(
      path.join(wsRoot, CONSTANTS.MYCELIUM_WS_NAME)
    );
    const publishigConfig = ConfigUtils.getPublishing(myceliumConfig);
    const siteUrl = publishigConfig.siteUrl;
    const publishingTheme = myceliumConfig?.publishing?.theme;
    const previewTheme = myceliumConfig?.preview?.theme;
    const enabledExportPodV2 = myceliumConfig.dev?.enableExportPodV2;
    const { workspaceFile, workspaceFolders } = vscode.workspace;
    const configVersion = ConfigUtils.getVersion(myceliumConfig);

    const configDiff = ConfigUtils.findDifference({ config: myceliumConfig });
    const myceliumConfigChanged = configDiff.length > 0;
    ...
    const trackProps = {
      ...
      selfContainedVaultsEnabled:
        myceliumConfig.dev?.enableSelfContainedVaults || false,
      ...
      myceliumConfigChanged,
    };

    if (myceliumConfigChanged) {
