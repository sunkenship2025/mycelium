...
export class MeetingNotesTip implements IFeatureShowcaseMessage {
  ...
  getDisplayMessage(displayLocation: DisplayLocation): string {
    switch (displayLocation) {
      case DisplayLocation.InformationMessage:
        return `Mycelium now has meeting notes. Try it out!`;
      case DisplayLocation.TipOfTheDayView:
        return "Mycelium now has meeting notes, which lets you easily create a meeting note with a pre-built template. Try it out now!";
      default:
  ...
  onConfirm() {
    vscode.commands.executeCommand("mycelium.createMeetingNote");
  }
