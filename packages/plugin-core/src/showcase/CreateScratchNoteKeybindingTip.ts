...
export class CreateScratchNoteKeybindingTip implements IFeatureShowcaseMessage {
  ...
  getDisplayMessage(_displayLocation: DisplayLocation): string {
    return `Keyboard shortcut for "Mycelium: Create Scratch Note" has changed. If you wish to keep the original, please follow the instructions online.`;
  }

  onConfirm() {
    VSCodeUtils.openLink(
      "https://wiki.mycelium.so/notes/50kdbcwwda3gphjhccb0e5t"
    );
  }
