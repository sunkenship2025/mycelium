...
const TAGS_TIP = createTipOfDayMsgWithDocsLink({
  ...
  displayMessage:
    "Quickly add a tag in your notes by typing '#my-tag-name'. Tags are just notes in Mycelium, and you can view all references to a tag by examining the backlinks of that tag",
  docsUrl: "https://wiki.mycelium.so/notes/8bc9b3f1-8508-4d3a-a2de-be9f12ef1821",
  confirmText: "See Details",
});

const RENAME_HEADER = createSimpleTipOfDayMsg(
  ShowcaseEntry.RenameHeader,
  "If you rename a header with the 'Mycelium Rename Header' command, all links pointing to that header also get updated."
);
...
const TASK_MANAGEMENT = createTipOfDayMsgWithDocsLink({
  ...
  docsUrl: "https://wiki.mycelium.so/notes/8hwz4bvyy556frx9y04c1cv/",
});
...
const INSERT_NOTE_LINK = createTipOfDayMsgWithDocsLink({
  ...
  docsUrl:
    "https://wiki.mycelium.so/notes/eea2b078-1acc-4071-a14e-18299fc28f47/#insert-note-link",
});

const PUBLISH_THEME_LINK = createTipOfDayMsgWithDocsLink({
  showcaseEntry: ShowcaseEntry.PublishTheme,
  displayMessage:
    "You can now customize how your published Mycelium site looks! We have an example custom theme to get you started.",
  confirmText: "Show me how",
  docsUrl: "https://wiki.mycelium.so/notes/jknrdi492m8nhq7mw4faydu",
});
...
const PREVIEW_THEME_LINK = createTipOfDayMsgWithDocsLink({
  ...
  confirmText: "Show me how",
  docsUrl: "https://wiki.mycelium.so/notes/lb9wd7z62ch7b4slscp05i4",
});
