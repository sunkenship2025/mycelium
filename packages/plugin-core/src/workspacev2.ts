import { DVault, Time } from "@myceliumhq/common-all";
import { createFileWatcher } from "@myceliumhq/common-server";
import {
...
export class DWorkspace {
  ...
  async initEngine({ port }: { port: number }) {
    ...
    await myceliumEngine.sync();
    this._engine = myceliumEngine;
