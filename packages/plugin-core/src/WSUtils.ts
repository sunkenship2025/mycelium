...
import { HistoryEvent, HistoryService } from "@myceliumhq/engine-server";
 ...
 * See [[Migration of static  methods to a non-static|mycelium://mycelium.docs/dev.ref.impactful-change-notice#migration-of-static--methods-to-a-non-static]]
 * */
          ...
          HistoryService.instance().subscribe(
            ...
            async (_event: HistoryEvent) => {
              if (_event.action === "not_initialized") {
                Logger.error({ ctx, msg: "issue initializing Mycelium" });
                resolve(undefined);
              }
            }
          );
