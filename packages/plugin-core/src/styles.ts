...
import { FOLDERS } from "@myceliumhq/common-all";
...
Add Mycelium graph styles below. The graph can be styled with any valid Cytoscape.js selector: https://js.cytoscape.org/#cy.style
Full Mycelium-specific styling documentation can be found here: https://wiki.mycelium.so/notes/587e6d62-3c5b-49b0-aedc-02f62f0448e6/#adding-styles
If you are new to Cystoscape styling, use our built-in snippet to help you get started: https://wiki.mycelium.so/notes/587e6d62-3c5b-49b0-aedc-02f62f0448e6#built-in-snippet copied
Note: Empty selectors may affect parsing of following selectors, so be sure to comment/remove them when not in use.
If style properties are not applying, make sure each property is followed with a semicolon. Troubleshoot guide can be found here: https://wiki.mycelium.so/notes/587e6d62-3c5b-49b0-aedc-02f62f0448e6/#troubleshooting
*/
...
export class GraphStyleService {
  static styleFilePath() {
    return path.join(os.homedir(), FOLDERS.MYCELIUM_SYSTEM_ROOT, "styles.css");
  }
