...
import { CONFIG, MYCELIUM_CHANNEL_NAME } from "./constants";
...
export const UNKNOWN_ERROR_MSG = `You found a bug! We didn't think this could happen but you proved us wrong. Please file the bug here -->  https://github.com/myceliumhq/mycelium/issues/new?assignees=&labels=&template=bug_report.md&title= We will put our best bug exterminators on this right away!`;
...
export class Logger {
  ...
  static configure(context: ExtensionContext, level: TraceLevel) {
    ...
    const logPath = path.join(context.logPath, "mycelium.log");
    ...
    this.logger = createLogger("mycelium", logPath);
    this.level = level;
  ...
  static set level(value: TraceLevel) {
    this._level = value;
    this.output =
      this.output || window.createOutputChannel(MYCELIUM_CHANNEL_NAME);
  }

  /** Log an error.
   *
   * If an `error` is attached to log payload, the error is also sent to Sentry.
   * This should be used for internal Mycelium errors that we can fix, or for
