...
@singleton()
export class MyceliumEngineV3Web
  ...
  implements ReducedDEngine, EngineEventEmitter
{
  ...
  async writeNote(
    ...
    const ctx = "MyceliumEngineV3Web:writeNewNote";
    this.logger.info({
      ctx,
      msg: `enter with ${opts}`,
    ...
    //     if (valResp instanceof MyceliumError) {
    //       error = valResp;
    //       this.logger.error({ ctx, error: stringifyError(error) });
    //     } else {
    //       note = resp.note;
  ...
  async deleteNote(
    ...
    const ctx = "MyceliumEngineV3Web:delete";
  ...
  private async initNotesNew(
    ...
    const ctx = "MyceliumEngineV3Web:initNotes";
