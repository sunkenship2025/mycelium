...
suite("GIVEN a workspace folder", () => {
  test("WHEN assetsPrefix is present in mycelium.yml THEN return correct value", async () => {
  ...
  test("WHEN assetsPrefix is not present in mycelium.yml THEN return empty value", async () => {
    const wsRoot = await WorkspaceHelpers.getWSRootForTest();
