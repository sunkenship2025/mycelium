...
suite("GIVEN a workspace folder", () => {
  test("WHEN siteUrl is present in mycelium.yml THEN return correct value", async () => {
  ...
  test("WHEN siteUrl is not present in mycelium.yml THEN return empty value", async () => {
    const wsRoot = await WorkspaceHelpers.getWSRootForTest();
