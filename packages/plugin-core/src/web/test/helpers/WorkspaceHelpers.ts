...
import { CONSTANTS } from "@myceliumhq/common-all";
...
export class WorkspaceHelpers {
  ...
  /**
   * Create a test Mycelium YAML config file at the specified location
   * @param wsRoot
   * @param config
   */
  static async createTestYAMLConfigFile(wsRoot: URI, config: any) {
    ...
    await vscode.workspace.fs.writeFile(
      Utils.joinPath(wsRoot, CONSTANTS.MYCELIUM_CONFIG_FILE),
