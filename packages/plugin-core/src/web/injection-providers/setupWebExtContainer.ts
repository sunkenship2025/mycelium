...
export async function setupWebExtContainer(context: vscode.ExtensionContext) {
  ...
  // The EngineEventEmitter is also MyceliumEngineV3Web, so reuse the same token
  ...
  container.afterResolution<MyceliumEngineV3Web>(
    "ReducedDEngine",
    (_t, result) => {
      if ("init" in result) {
        result.init().then(
          () => {},
          (reason) => {
            throw new Error(`Mycelium Engine Failed to Initialize: ${reason}`);
          }
        );
      }
    },
  ...
  container.register<MyceliumConfig>("MyceliumConfig", {
    useValue: config as MyceliumConfig,
  });
