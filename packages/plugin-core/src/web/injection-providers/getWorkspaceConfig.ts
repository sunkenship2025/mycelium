...
import { CONSTANTS, MyceliumConfig } from "@myceliumhq/common-all";
import YAML from "js-yaml";
import * as vscode from "vscode";
import { Uri } from "vscode";

export async function getWorkspaceConfig(wsRoot: Uri) {
  const configPath = Uri.joinPath(wsRoot, CONSTANTS.MYCELIUM_CONFIG_FILE);
  const config = (await readYAML(configPath, true)) as MyceliumConfig;
  return config;
}
