...
export class WebViewUtils {
   ...
   * When running in development, this is in the build folder of `mycelium-plugin-views`
   * @returns
   */
  getViewRootUri() {
    ...
    //     message: "Unable to find the folder where Mycelium assets are stored",
    //   });
    // }
    // return getStage() === "dev"
    //   ? vscode.Uri.file(
    //       path.join(pkgRoot, "..", "mycelium-plugin-views", "build")
    //     )
    //   : assetUri;
  }
  ...
  genHTMLForView = async ({
    ...
    title: string;
    view: MyceliumTreeViewKey | MyceliumEditorViewKey;
  }) => {
    ...
    // View is `mycelium.{camelCase}`
    // we want to remove `mycelium` and transform camelCase to snake case
...
<body>
  ...
  private genVSCodeHTMLIndex({
    ...
    const builtinStyle = "mycelium-editor-follow-style";
    const defaultStyle = "mycelium-editor-default-style";
    const overrideStyle = "mycelium-editor-override-style";
    return `<!DOCTYPE html>
    <html lang="en">
      <head>
        <meta charset="utf-8" />
