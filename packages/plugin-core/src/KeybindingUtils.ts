...
export class KeybindingUtils {
  ...
  static getConflictingKeybindings(opts: {
    ...
    userKeybindingConfig.forEach((keybinding) => {
      ...
      // remapping of either the conflicting / mycelium command's keybinding
      // or disabling the mycelium command's keybinding is not considered a resolution.
  ...
  static async showKeybindingConflictPreview(opts: {
    ...
    const contents = [
      ...
      "",
      "The keybindings listed at the bottom are known to have conflicts with default keybindings for Mycelium commands.",
      "",
      "Neither Mycelium nor the extension may function properly if the keybinding conflict is not resolved.",
      "",
      ...
      conflicts
        .map((conflict) => {
          ...
          const myceliumKeybindingEntry = defaultKeybindingJSON.find(
            (keybinding) => {
          ...
          if (myceliumKeybindingEntry === undefined) {
            return undefined;
          }
          ...
          const copyCommandUri = (args: CopyToClipboardCommandOpts) =>
            `command:mycelium.copyToClipboard?${encodeURIComponent(
              JSON.stringify(args)
            )}`;
  ...
  static async showKeybindingConflictConfirmationMessage(opts: {
    ...
    const message =
      "We noticed some extensions that have known keybinding conflicts with Mycelium. Would you like to view a list of keybinding conflicts?";
  ...
  static getKeybindingForPodIfExists(podId: string): string | undefined {
    ...
    const result = keybindings.filter(
      (item) =>
        item.command &&
        item.command === MYCELIUM_COMMANDS.EXPORT_POD_V2.key &&
        item.args === podId
    );
  ...
  static getKeybindingsForCopyAsIfExists(format: string): string | undefined {
    ...
    const result = keybindings.filter((item) => {
      return (
        item.command &&
        item.command === MYCELIUM_COMMANDS.COPY_AS.key &&
        item.args === format
      );
    });
