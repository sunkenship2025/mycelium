import { ConfirmStatus, getStage, SurveyEvents } from "@myceliumhq/common-all";
...
export class MyceliumQuickInputSurvey {
  opts: {
    title: string;
...
export class MyceliumQuickPickSurvey {
...
export class ContextSurvey extends MyceliumQuickPickSurvey {
  ...
  async onAnswer(result: vscode.QuickPickItem) {
    ...
    if (answer === "other") {
      maybeOtherResult = await vscode.window.showInputBox({
        ...
        prompt:
          'You have checked "Other". Please describe what other context you intend to use Mycelium.',
        title: "In what context do you intend to use Mycelium? - Other",
      });
    }
  ...
  static create() {
    const title = "In what context do you intend to use Mycelium?";
...
export class BackgroundSurvey extends MyceliumQuickPickSurvey {
  async onAnswer(result: vscode.QuickPickItem) {
    let maybeOtherResult: string | undefined;
...
export class UseCaseSurvey extends MyceliumQuickPickSurvey {
  async onAnswer(results: vscode.QuickPickItem[]) {
    ...
    if (results.some((result) => result.label === "Other")) {
      maybeOtherResult = await vscode.window.showInputBox({
        ...
        title: "What do you want to use Mycelium for? - Other",
      });
    }
  ...
  static create() {
    const title = "What do you want to use Mycelium for?";
    const choices = [
...
export class PriorToolsSurvey extends MyceliumQuickPickSurvey {
  async onAnswer(results: vscode.QuickPickItem[]) {
    let maybeOtherResult: string | undefined;
    if (results.some((result) => result.label === "Other")) {
      maybeOtherResult = await vscode.window.showInputBox({
...
export class PublishingUseCaseSurvey extends MyceliumQuickPickSurvey {
  static CHOICES: { [index: string]: string } = {
  ...
  static create() {
    const title =
      "Mycelium lets you publish your notes as a static website. Is this something you'd be interested in?";
...
export class NewsletterSubscriptionSurvey extends MyceliumQuickInputSurvey {
...
export class LapsedUserReasonSurvey extends MyceliumQuickPickSurvey {
  async onAnswer(result: vscode.QuickPickItem) {
    ...
    switch (label) {
      ...
      case "I've encountered a bug which stopped me from using Mycelium.": {
        reason = "bug";
      ...
      case "I found a different tool that suits me better.": {
        ...
        extra = await vscode.window.showInputBox({
          ignoreFocusOut: true,
          placeHolder: "Type here",
          prompt: "What feature was missing in Mycelium for your use case?",
          title: label,
        });
        break;
      }
  ...
  static create() {
    const title = "What is the reason you haven't started using Mycelium yet?";
    const choices = [
      { label: "I haven't had time to start, but I still want to." },
      { label: "I am not sure how to get started." },
      { label: "I've encountered a bug which stopped me from using Mycelium." },
...
export class LapsedUserOnboardingSurvey extends MyceliumQuickPickSurvey {
...
export class LapsedUserAdditionalCommentSurvey extends MyceliumQuickInputSurvey {
  async onAnswer(result: string) {
...
export class LapsedUserPlugDiscordSurvey extends MyceliumQuickPickSurvey {
...
export class SurveyUtils {
    ...
    vscode.window
      ...
      .then(async (resp) => {
        if (resp?.title === "Proceed") {
          ...
          vscode.window.showInformationMessage(
            "Survey submitted! Thanks for helping us make Mycelium better 🌱"
          );
        } else {
          MetadataService.instance().setInitialSurveyStatus(
  ...
  static async showInactiveUserSurvey() {
    ...
    await vscode.window
      .showInformationMessage(
        "Hey, we noticed you haven't used Mycelium for a while. We would love to have you back! Could you give us some feedback on how we can do better?",
        { modal: true },
        { title: "Go to Survey" }
      )
      .then(async (resp) => {
        if (resp?.title === "Go to Survey") {
          ...
          vscode.window.showInformationMessage(
            "Thanks for helping us make Mycelium better 🌱"
          );
