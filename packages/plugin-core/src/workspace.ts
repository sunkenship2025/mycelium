...
import { resolvePath } from "@myceliumhq/common-server";
import {
  HistoryService,
  WorkspaceService,
  WorkspaceUtils,
} from "@myceliumhq/engine-server";
...
import { MYCELIUM_COMMANDS, GLOBAL_STATE } from "./constants";
import {
  MyceliumWorkspaceSettings,
  IMyceliumExtension,
} from "./myceliumExtensionInterface";
...
let _MyceliumWorkspace: MyceliumExtension | null;
 ...
 * Or preferably pass IMyceliumExtension to constructors of your classes. */
 ...
 * Or preferably pass IMyceliumExtension to constructors of your classes.
 * */
export function getExtension(): MyceliumExtension {
  return MyceliumExtension.instanceV2();
}
...
// --- Main
export class MyceliumExtension implements IMyceliumExtension {
  static MYCELIUM_WORKSPACE_FILE: string = "mycelium.code-workspace";
  ...
  static instanceV2(): MyceliumExtension {
    if (!_MyceliumWorkspace) {
      throw Error("Myceliumworkspace not initialized");
    }
    return _MyceliumWorkspace;
  }

  static serverConfiguration() {
    if (!MyceliumExtension._SERVER_CONFIGURATION) {
      MyceliumExtension._SERVER_CONFIGURATION = {};
    }
    return MyceliumExtension._SERVER_CONFIGURATION as ServerConfiguration;
  }
  ...
  async pauseWatchers<T = void>(cb: () => Promise<T>) {
    ...
    try {
      ...
      Logger.error({ ctx, error: err as MyceliumError });
      throw err;
    } finally {
      if (this.fileWatcher) {
  ...
  /** Checks if the current workspace open in VSCode is a Mycelium workspace or not. */
  static async isMyceliumWorkspace(): Promise<boolean | undefined> {
    // we do a try catch because `MyceliumWorkspace.workspaceFile` throws an error if workspace file doesn't exist
    try {
      // code workspace takes precedence, if code workspace, return
      if (
        vscode.workspace.workspaceFile &&
        path.basename(MyceliumExtension.workspaceFile().fsPath) ===
          this.MYCELIUM_WORKSPACE_FILE
      )
        return true;

      const workspaceFolders = MyceliumExtension.workspaceFolders();
      if (workspaceFolders) {
        return !_.isEmpty(
  ...
  static isActive(_context?: vscode.ExtensionContext): boolean {
    const ctx = "MyceliumExtension.isActive";
    try {
      //
      const { wsRoot } = getDWorkspace();
      if (fs.existsSync(path.join(wsRoot, CONSTANTS.MYCELIUM_CONFIG_FILE))) {
        return true;
      }
    } catch (err: any) {
      ...
      const error =
        err instanceof MyceliumError
          ? err
          : new MyceliumError({ message: ctx, payload: err });
  ...
  async isActiveAndIsMyceliumNote(fpath: string): Promise<boolean> {
    if (!this.isActive()) {
      return false;
    }
    const { wsRoot, vaults } = this.getDWorkspace();
    return WorkspaceUtils.isMyceliumNote({
      wsRoot,
      vaults,
      fpath,
    });
  ...
  ): Promise<MyceliumExtension> {
    if (!_MyceliumWorkspace) {
      _MyceliumWorkspace = new MyceliumExtension(context, opts);
      _MyceliumWorkspace.type = await WorkspaceUtils.getWorkspaceType({
        workspaceFile: vscode.workspace.workspaceFile,
        workspaceFolders: vscode.workspace.workspaceFolders,
      });

      ExtensionProvider.register(_MyceliumWorkspace);
    }
    return _MyceliumWorkspace;
  }
  ...
  isActive(): boolean {
    return MyceliumExtension.isActive();
  }
  ...
  async getWorkspaceSettings(): Promise<WorkspaceSettings | undefined> {
    const ctx = "MyceliumExtension.getWorkspaceSettings";
    const workspaceFile = MyceliumExtension.tryWorkspaceFile();
  ...
  getWorkspaceSettingsSync(): WorkspaceSettings | undefined {
    const ctx = "MyceliumExtension.getWorkspaceSettingsSync";
    const workspaceFile = MyceliumExtension.tryWorkspaceFile();
    if (!workspaceFile) return undefined;
  ...
  getWorkspaceSettingOrDefault({
    ...
    if (_.get(config, myceliumConfigKey)) {
      return _.get(config, myceliumConfigKey);
    }
    // migrate value from workspace setting. if not exist, migrate from new default
    const out = _.get(
      this.getMyceliumWorkspaceSettingsSync(),
      wsConfigKey,
      _.get(ConfigUtils.genDefaultConfig(), myceliumConfigKey)
    );
    // this should not happen
    if (_.isUndefined(out)) {
      throw new MyceliumError({
        message: `no config key found. workspace: ${wsConfigKey}, mycelium.yml: ${myceliumConfigKey}`,
      });
    }
    return out;
  }
  ...
  get rootWorkspace(): vscode.WorkspaceFolder {
    const wsFolders = MyceliumExtension.workspaceFolders();
  ...
  private setupBacklinkTreeView() {
    ...
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.BACKLINK_SORT_BY_LAST_UPDATED.key,
    ...
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.BACKLINK_SORT_BY_PATH_NAMES.key,
      sentryReportingCallback(() => {
        AnalyticsUtils.track(VSCodeEvents.BacklinksPanelUsed, {
          type: "SortOrderChanged",
    ...
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.BACKLINK_SORT_BY_LAST_UPDATED_CHECKED.key,
      sentryReportingCallback(() => {
        AnalyticsUtils.track(VSCodeEvents.BacklinksPanelUsed, {
          type: "SortOrderChanged",
          state: "SortByLastUpdated",
    ...
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.BACKLINK_SORT_BY_PATH_NAMES_CHECKED.key,
      sentryReportingCallback(() => {
        AnalyticsUtils.track(VSCodeEvents.BacklinksPanelUsed, {
          type: "SortOrderChanged",
          state: "SortByPathName",
    ...
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.BACKLINK_EXPAND_ALL.key,
    ...
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GOTO_BACKLINK.key,
  ...
  private setupGraphPanel() {
    ...
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GRAPH_PANEL_INCREASE_DEPTH.key,
      sentryReportingCallback(() => {
        graphPanel.increaseGraphDepth();
      })
    );
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GRAPH_PANEL_DECREASE_DEPTH.key,
      sentryReportingCallback(() => {
        graphPanel.decreaseGraphDepth();
      })
    );
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GRAPH_PANEL_SHOW_BACKLINKS_CHECKED.key,
      sentryReportingCallback(() => {
        graphPanel.showBacklinks = false;
      })
    );
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GRAPH_PANEL_SHOW_BACKLINKS.key,
      sentryReportingCallback(() => {
        graphPanel.showBacklinks = true;
      })
    );
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GRAPH_PANEL_SHOW_OUTWARD_LINKS_CHECKED.key,
      sentryReportingCallback(() => {
        graphPanel.showOutwardLinks = false;
      })
    );
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GRAPH_PANEL_SHOW_OUTWARD_LINKS.key,
      sentryReportingCallback(() => {
        graphPanel.showOutwardLinks = true;
      })
    );
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GRAPH_PANEL_SHOW_HIERARCHY_CHECKED.key,
      sentryReportingCallback(() => {
        graphPanel.showHierarchy = false;
      })
    );
    vscode.commands.registerCommand(
      MYCELIUM_COMMANDS.GRAPH_PANEL_SHOW_HIERARCHY.key,
      sentryReportingCallback(() => {
