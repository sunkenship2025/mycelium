...
const NOTES = {
  ...
  XVAULT_LINK: new TestPresetEntryV4(
      ...
      return runRename({
        ...
        cb: ({ barChange }) => {
          return [
            {
              actual: _.trim(barChange?.note.body),
              expected: "[[mycelium://vault1/baz#head1]]",
            },
          ];
        },
      });
    ...
    {
      preSetupHook: (opts) =>
        preSetupHook(opts, { barBody: `[[mycelium://vault1/foo#head1]]` }),
    }
  ),
  ...
  RENAME_FOR_CACHE: new TestPresetEntryV4(
    async ({ wsRoot, vaults, engine }) => {
      ...
      const cachePath = path.join(
        ...
        CONSTANTS.MYCELIUM_CACHE_FILE
      );
      const notesCache = new NotesFileSystemCache({
        cachePath,
        logger: (engine as MyceliumEngineClient).logger,
      });
    ...
    async ({ wsRoot, vaults, engine }) => {
      ...
      return [
        ...
        },
        {
          ...
          expected: `![[mycelium://${VaultUtils.getName(vaults[1])}/baz]]`,
        },
        {
          actual: checkVault,
          expected: true,
    ...
    },
    {
      preSetupHook: async ({ vaults, wsRoot }) => {
        await NoteTestUtilsV4.createNote({
          ...
          fname: "bar",
          vault: vaults[0],
          body: `![[mycelium://${VaultUtils.getName(vaults[1])}/foo]]`,
        });
        await NoteTestUtilsV4.createNote({
          wsRoot,
          fname: "foo",
  ...
  NOTE_REF_XVAULT_VAULT_CHANGE: new TestPresetEntryV4(
    async ({ wsRoot, vaults, engine }) => {
      ...
      return [
        ...
        {
          actual: _.trim(changed![1].note.body),
          expected: `![[mycelium://${VaultUtils.getName(vaults[2])}/baz]]`,
        },
        {
          actual: checkVault,
          expected: true,
