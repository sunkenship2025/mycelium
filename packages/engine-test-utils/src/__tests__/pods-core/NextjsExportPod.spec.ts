...
import { NextjsExportConfig, NextjsExportPod } from "@myceliumhq/pods-core";
...
const setupConfig = ({
  ...
  wsRoot: string;
  siteConfig?: Partial<MyceliumPublishingConfig>;
}) => {
  ...
  test("THEN ok", async () => {
    await runEngineTestV5(
      async ({ engine, vaults, wsRoot }) => {
        ...
        await verifyExport(dest);
        await checkFile(
          { fpath: path.join(dest, "data", "mycelium.json") },
      ...
      await runEngineTestV5(
        ...
        },
        {
          ...
          preSetupHook: async (opts) => {
            ...
            TestConfigUtils.withConfig(
              (config) => {
                ConfigUtils.setGithubProp(config, "cname", "11ty.mycelium.so");
                return config;
              },
              { wsRoot: opts.wsRoot }
            );
