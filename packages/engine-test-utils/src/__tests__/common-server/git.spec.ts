...
import { GitUtils } from "@myceliumhq/common-server";
...
describe("GitUtils", () => {
  describe("getGithubEditUrl", () => {
    const gitUrl = "https://github.com/myceliumhq/mycelium-site";
    ...
    testWithEngine("basic", async ({ engine, wsRoot }) => {
      ...
      expect(GitUtils.getGithubEditUrl({ note, config, wsRoot })).toEqual(
        "https://github.com/myceliumhq/mycelium-site/edit/main/vault1/foo.md"
      );
    });
    ...
    testWithEngine("note override", async ({ engine, wsRoot }) => {
      ...
      expect(GitUtils.getGithubEditUrl({ note, config, wsRoot })).toEqual(
        "https://github.com/myceliumhq/mycelium-site/edit/main/foo/ch1.md"
      );
    });
  });
      ...
      await runEngineTestV5(
        async ({ wsRoot, vaults }) => {
          ...
          expect(
            ...
            "https://github.com/myceliumhq/mycelium-site/blob/master/foo.md#L2:L3"
          );
        },
        {
          expect,
