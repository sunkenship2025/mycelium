import { DNoteRefData, DNoteRefLink } from "@myceliumhq/common-all";
import {
  matchRefMarker,
  parseMyceliumRef,
  parseFileLink,
...
describe("parseRef", () => {
  it("describe file ref without extension", () => {
    expect(parseMyceliumRef("ref: [[foo]]")).toEqual({
      direction: "to",
      link: createFileLink(),
    });
  });
  ...
  it("describe file ref", () => {
    expect(parseMyceliumRef("ref: [[foo.md]]")).toEqual({
      direction: "to",
      link: createFileLink(),
    });
  });
  ...
  it("describe file ref with anchor", () => {
    expect(parseMyceliumRef("ref: [[foo.md]]#head1")).toEqual({
      direction: "to",
  ...
  it("describe file ref with anchor start and end", () => {
    expect(parseMyceliumRef("ref: [[foo.md]]#head1:#head2")).toEqual({
      direction: "to",
  ...
  it("describe file ref with anchor start and end, start offset", () => {
    expect(parseMyceliumRef("ref: [[foo.md]]#head1,1:#head2")).toEqual({
      direction: "to",
      link: createFileLink({
        anchorStart: "head1",
        anchorEnd: "head2",
