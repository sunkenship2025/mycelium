...
function proc({
  ...
  mycelium: MyceliumASTData;
  opts?: MyceliumPubOpts;
  flavor?: ProcFlavor;
        ...
        await runEngineTestV5(
          async (opts) => {
            ...
            resp = await createProc({
              ...
              config,
              fname,
              linkText: `[[mycelium://${vaultName}/beta]] [[mycelium://${vaultName}/alpha]]`,
            });
          },
          {
            preSetupHook: ENGINE_HOOKS.setupLinks,
...
describe("GIVEN myceliumPub (old tests - need to be migrated)", () => {
  describe("prefix", () => {
  ...
  testWithEngine("in IMPORT mode", async ({ vaults, wsRoot }) => {
    ...
    const proc = MDUtilsV5.procRemarkParse(
      ...
      { dest: MyceliumASTDest.HTML, wsRoot, vault: vaults[0], config }
    );
