 ...
 * @param setupFunc function {@link SetupTestFunctionV4} with `extra` param of {dest: {@link MyceliumASTDest}}
...
export const createProcTests = (opts: {
  ...
  verifyFuncDict?: {
    [key in MyceliumASTDest]?: TestPresetEntryV4["testFunc"] | MyceliumASTDest;
  };
  ...
  if (verifyFuncDict) {
    allTests = Object.values(MyceliumASTDest)
      .map((dest) => {
...
export const createProcCompileTests = (opts: {
  ...
  verify?: {
    [key in MyceliumASTDest]?: FlavorDict;
  };
  preSetupHook?: TestPresetEntryV4["preSetupHook"];
...
export function genMyceliumData(
  opts?: Partial<MyceliumASTData>
): MyceliumASTData & { fname: string } {
  return { ...opts } as any;
}

export const generateVerifyFunction = (opts: {
  target: MyceliumASTDest;
  exclude?: MyceliumASTDest[];
}) => {
  const { target, exclude } = _.defaults(opts, { exclude: [] });
  const out: any = {};
  const excludeList = exclude.concat(target);
  Object.values(MyceliumASTDest)
...
export const processTextV2 = async (opts: ProcessTextV2Opts) => {
  ...
  if (opts.dest !== MyceliumASTDest.HTML) {
