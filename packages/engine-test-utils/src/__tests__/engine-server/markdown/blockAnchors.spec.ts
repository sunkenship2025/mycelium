import { ConfigUtils } from "@myceliumhq/common-all";
import { DConfig } from "@myceliumhq/common-server";
import { AssertUtils, TestPresetEntryV4 } from "@myceliumhq/common-test-utils";
import { Parent, Text } from "@myceliumhq/engine-server";
import {
  BlockAnchor,
  MyceliumASTDest,
  MyceliumASTTypes,
...
function proc() {
  return MDUtilsV5.procRemarkParseNoData({}, { dest: MyceliumASTDest.HTML });
}
...
describe("blockAnchors", () => {
  describe("parse", () => {
    test("parses anchor by itself", () => {
      ...
      expect(getBlockAnchor(resp).type).toEqual(MyceliumASTTypes.BLOCK_ANCHOR);
    ...
    test("doesn't parse block anchor inside a link", async () => {
      const resp = proc().parse("[[#^block-id]]");
      expect(getBlockAnchor(resp).type).toEqual(MyceliumASTTypes.WIKI_LINK);
    });
    ...
    test("parses anchors at the end of the line", () => {
      ...
      expect(anchor.type).toEqual(MyceliumASTTypes.BLOCK_ANCHOR);
  ...
  describe("rendering", () => {
    ...
    const SIMPLE = createProcTests({
      ...
      verifyFuncDict: {
        [MyceliumASTDest.HTML]: async ({ extra }) => {
          const { resp } = extra;
        ...
        [MyceliumASTDest.MD_MYCELIUM]: async ({ extra }) => {
          const { resp } = extra;
          expect(resp).toMatchSnapshot();
          return [
            {
