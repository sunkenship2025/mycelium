...
describe("user tags", () => {
  describe("parse", () => {
    test("parses basic user tag", () => {
      ...
      expect(getUserTag(resp).type).toEqual(MyceliumASTTypes.USERTAG);
    ...
    test("doesn't parse user tag inside a link", () => {
      ...
      expect(getUserTag(resp).type).toEqual(MyceliumASTTypes.WIKI_LINK);
    });
  ...
  describe("rendering", () => {
    ...
    const SIMPLE = createProcTests({
      ...
      verifyFuncDict: {
        ...
        [MyceliumASTDest.MD_REGULAR]: async ({ extra }) => {
          const { resp } = extra;
          return [
            {
              actual: await AssertUtils.assertInString({
    ...
    const INSIDE_LINK = createProcTests({
      ...
      verifyFuncDict: {
        [MyceliumASTDest.HTML]: async ({ extra }) => {
          ...
          await checkVFile(
            resp,
            '<a href="https://twitter.com/myceliumhq">@myceliumhq</a>'
          );
