...
import { getParsingDependencyDicts, MDUtilsV5 } from "@myceliumhq/unified";
  ...
  describe("common cases", () => {
    ...
    const WITH_ANCHOR = createProcTests({
      ...
      verifyFuncDict: {
        ...
        ...generateVerifyFunction({
          target: MyceliumASTDest.MD_REGULAR,
          exclude: [MyceliumASTDest.MD_MYCELIUM],
        }),
      },
    });
    ...
    const WITH_FM_TITLE = createProcTests({
      ...
      setupFunc: async (opts) => {
        ...
        const config = DConfig.readConfigSync(wsRoot);
        const configOverride: MyceliumConfig = {
          ...config,
          publishing: {
            ...config.publishing,
            enableFMTitle: true,
    ...
    const WILDCARD_CASE = createProcTests({
      ...
      verifyFuncDict: {
        [MyceliumASTDest.MD_MYCELIUM]: async ({ extra, engine }) => {
          const note = (await engine.getNote("id.journal")).data!;
          const { resp } = extra;
          expect(resp).toMatchSnapshot();
          expect(
        ...
        [MyceliumASTDest.MD_REGULAR]: async ({ extra, engine }) => {
          const { resp } = extra;
          expect(resp).toMatchSnapshot();
          // @ts-ignore
