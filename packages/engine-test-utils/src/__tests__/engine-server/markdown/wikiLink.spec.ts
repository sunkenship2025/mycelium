...
function proc() {
  return MDUtilsV5.procRemarkParseNoData(
    {},
    { dest: MyceliumASTDest.MD_REGULAR, fname: "placeholder" }
  );
}
...
describe("wikiLinks", () => {
  describe("parse", () => {
    ...
    test("cross vault wikilink with spaces", () => {
      const resp = proc().parse(`[[foo | mycelium://vault1/foo]]`);
    ...
    describe("block references", () => {
      ...
      test("block reference to same file", () => {
        ...
        const wikiLink = getWikiLink(resp);
        expect(wikiLink?.type).toEqual(MyceliumASTTypes.WIKI_LINK);
        expect(wikiLink?.data.anchorHeader).toEqual("^block-id");
      });
      ...
      test("avoids parsing broken links", () => {
        ...
        expect(wikiLink?.type).not.toEqual(MyceliumASTTypes.WIKI_LINK);
      });
    });
  });
  ...
  describe("compile", () => {
    ...
    const linkFromSameVaultWithAlias = "[[bar|mycelium://vault1/foo]]";
    ...
    const linkFromSameVault = "[[mycelium://vault1/foo]]";
    ...
    const linkFromOtherVault = "[[mycelium://vault2/foo]]";
