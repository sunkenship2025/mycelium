...
describe("hashtag", () => {
  describe("parse", () => {
    test("parses basic hashtag", () => {
      ...
      expect(getHashtag(resp).type).toEqual(MyceliumASTTypes.HASHTAG);
    ...
    test("doesn't parse hash inside a link", () => {
      const resp = proc().parse("[[#my-hash-tag]]");
      expect(getHashtag(resp).type).toEqual(MyceliumASTTypes.WIKI_LINK);
    });
    ...
    const INSIDE_LINK = createProcTests({
      ...
      verifyFuncDict: {
        [MyceliumASTDest.HTML]: async ({ extra }) => {
          ...
          await checkVFile(
            resp,
            '<a href="https://twitter.com/hashtag/mycelium">#mycelium</a>'
          );
