...
describe("WorkspaceService", () => {
  ...
  // TODO: migrate test to src/__tests__/mycelium-cli/commands/workspaceCli.spec.ts
  describe("initialize", () => {
    test("remote seed present", async () => {
      await runEngineTestV5(
        async ({ engine, wsRoot, vaults }) => {
          ...
          await checkFile({
            fpath: path.join(wsRoot, "mycelium.yml"),
          });
          await checkFile(
            {
              fpath: path.join(wsRoot, "mycelium.code-workspace"),
              snapshot: false,
            },
      ...
      await runEngineTestV5(
        async ({ wsRoot }) => {
          ...
          await checkDir(
            ...
            CONSTANTS.MYCELIUM_WS_NAME,
            CONSTANTS.MYCELIUM_CONFIG_FILE,
            FOLDERS.NOTES
          );
