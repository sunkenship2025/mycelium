...
import { MyceliumEngineClient } from "@myceliumhq/engine-server";
import { createEngineFromServer, ENGINE_HOOKS, runEngineTestV5 } from "../..";

/**
 * Tests that the EngineEvents interface of the client-side MyceliumEngineClient signals properly
 */
describe("GIVEN a MyceliumEngineClient running on client-side", () => {
    ...
    test("THEN create event fired with the correct props AND update note event fired for its parent", (done) => {
      runEngineTestV5(
        async ({ engine, vaults, wsRoot }) => {
          // TODO: Get rid of this casting once the proper way to expose EngineEvents of MyceliumEngineClient is implemented
          const engineClient = engine as MyceliumEngineClient;
