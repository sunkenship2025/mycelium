import { VaultUtils } from "@myceliumhq/common-all";
...
describe("WHEN run 'mycelium note lookup'", () => {
      ...
      await runEngineTestV5(
        async ({ engine, wsRoot }) => {
          const {
            ...
            wsRoot,
            engine,
            query: `[[mycelium://${VaultUtils.getName(
              engine.vaults[0]
            )}/foo.ch1]]`,
          });
          expect(_.map(notesOutput, (n) => n.fname)).toEqual(["foo.ch1"]);
...
describe("WHEN run 'mycelium note get'", () => {
...
describe("WHEN run 'mycelium note find'", () => {
...
describe("WHEN run 'mycelium note lookup_legacy'", () => {
  ...
  describe("AND WHEN lookup note and --output = markdown_mycelium", () => {
    test("THEN get note body in markdown_mycelium", async () => {
      await runEngineTestV5(
        async ({ engine, wsRoot, vaults }) => {
          const vault = vaults[0];
          const {
            ...
            engine,
            query: "foo",
            output: NoteCLIOutput.MARKDOWN_MYCELIUM,
          });
...
describe("WHEN run 'mycelium note delete", () => {
...
describe("WHEN run 'mycelium note write", () => {
...
describe("WHEN run 'mycelium note move'", () => {
