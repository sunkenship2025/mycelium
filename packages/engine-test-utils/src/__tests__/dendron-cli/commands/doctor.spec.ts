...
import { DoctorCLICommand, DoctorCLICommandOpts } from "@myceliumhq/mycelium-cli";
...
const setupWithWikilink = async (opts: WorkspaceOpts) => {
  ...
  await NoteTestUtilsV4.createNote({
    ...
    vault: vaults[0],
    fname: "foo",
    body: "[[mycelium://vault1/foo.bar]]\n",
  });
  await NoteTestUtilsV4.createNote({
    wsRoot,
    vault: vaults[0],
    fname: "foo.bar",
    body: ["[[mycelium://vault1/fake.link]]", "[[fake.link2]]"].join("\n"),
  });
};
...
const setupMultiWithWikilink = async (opts: WorkspaceOpts) => {
  ...
  await NoteTestUtilsV4.createNote({
    ...
    vault: vaults[0],
    fname: "foo.bar",
    body: "[[mycelium://vault1/fake.link]]\n",
  });
  await NoteTestUtilsV4.createNote({
    wsRoot,
    vault: vaults[1],
    fname: "baz",
    body: "[[mycelium://vault2/baz.qaaz]]\n",
  });
  await NoteTestUtilsV4.createNote({
    wsRoot,
    vault: vaults[1],
    fname: "baz.qaaz",
    body: "[[mycelium://vault2/fake]]\n",
  });
};
...
const setupWithAliasedWikilink = async (opts: WorkspaceOpts) => {
  ...
  await NoteTestUtilsV4.createNote({
    ...
    fname: "foo",
    body: [
      "[[foo bar|mycelium://vault1/foo.bar]]",
      "[[foobaz|mycelium://vault1/foo.baz]]",
    ].join("\n"),
  });
};
...
const setupWithXVaultWikilink = async (opts: WorkspaceOpts) => {
  ...
  await NoteTestUtilsV4.createNote({
    ...
    body: [
      "[[mycelium://vault2/bar]]",
      "[[baz|mycelium://vault2/baz]]",
      "[[qaaaz note|mycelium://vault2/qaaaz]]",
    ].join("\n"),
  });
};
    ...
    await runEngineTestV5(
      async ({ engine, wsRoot, vaults }) => {
        ...
        await NoteTestUtilsV4.createNote({
          ...
          fname: "foo2",
          body: [
            "[[mycelium://vault2/bar2]]",
            "[[baz|mycelium://vault2/baz2]]",
            "[[qaaaz note|mycelium://vault2/qaaaz2]]",
          ].join("\n"),
        });
        const fooFile = (
          await engine.findNotes({
