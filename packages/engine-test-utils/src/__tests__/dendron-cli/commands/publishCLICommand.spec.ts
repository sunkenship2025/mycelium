import { ConfigUtils, MyceliumConfig, URI } from "@myceliumhq/common-all";
import {
  PublishCLICommand,
...
import { NextjsExportPodUtils, PublishTarget } from "@myceliumhq/pods-core";
...
describe("WHEN run `mycelium publish init`", () => {
...
describe("WHEN run `mycelium publish build`", () => {
    ...
    test("THEN update siteUrl", async () => {
      await runEngineTestV5(
        async ({ wsRoot }) => {
          ...
          const cpath = NextjsExportPodUtils.getMyceliumConfigPath(dest);
          const config = fs.readJSONSync(cpath) as MyceliumConfig;
...
describe("WHEN run `mycelium publish dev`", () => {
...
describe("WHEN run `mycelium publish export`", () => {
