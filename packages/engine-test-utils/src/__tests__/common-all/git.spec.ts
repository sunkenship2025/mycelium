...
describe("GIVEN GitUtils", () => {
  ...
  describe("WHEN config contains github properties", () => {
    ...
    config.publishing.github = {
      ...
      editRepository: "https://github.com/kevinslin/mycelium-11ty-test",
      editBranch: "main",
    ...
    test("THEN `getGithubEditUrl` should return github edit url", () => {
      expect(GitUtils.getGithubEditUrl({ config, note })).toEqual(
        "https://github.com/kevinslin/mycelium-11ty-test/edit/main/fooVault/foo.md"
      );
    });
  ...
  test("THEN `git2Github` conversion", () => {
    expect(
      GitUtils.git2Github("git@github.com:kevinslin/mycelium-vault.git")
    ).toEqual("https://github.com/kevinslin/mycelium-vault");
  });

  test("THEN `getGithubAccessTokenUrl` conversion", () => {
    expect(
      GitUtils.getGithubAccessTokenUrl({
        remotePath: "https://github.com/kevinslin/mycelium-11ty-test",
        accessToken: "abc",
      })
    ).toEqual(
      "https://abc:x-oauth-basic@github.com/kevinslin/mycelium-11ty-test"
    );
  });
});
