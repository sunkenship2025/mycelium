...
export class ServerUtils {
  ...
  static async startServerNode({
    ...
    const { port: finalPort } = await launchv2({
      port,
      logPath: path.join(logPath, "mycelium.server.log"),
      nextServerUrl,
      nextStaticRoot,
      googleOauthClientId,
      googleOauthClientSecret,
    ...
    if (!process.send) {
      throw new MyceliumError({ message: "expect a child process" });
    }
  ...
  static async execServerNode({
    ...
    const logger = createLogger(
      "execServer",
      path.join(logPath, "mycelium.log")
    );
    return new Promise((resolve, reject) => {
      ...
      subprocess.on("close", (code) => {
        ...
        reject(new MyceliumError({ message: "close", payload: { code } }));
      });
      subprocess.on("disconnect", () => {
        logger.error({ state: "disconnect" });
        reject(new MyceliumError({ message: "disconnect" }));
      });
      subprocess.on("exit", (code) => {
        logger.error({ state: "exit" });
        reject(new MyceliumError({ message: "exit", payload: { code } }));
      });
      subprocess.on("error", (err) => {
        logger.error({ state: "error", payload: err });
        reject(
          new MyceliumError({ message: "error", payload: stringifyError(err) })
        );
      });
