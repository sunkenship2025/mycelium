...
export const PATH_MAP = new Map<string, mappedConfigPath>([
  ...
  // workspace namespace
  ["workspace.myceliumVersion", { target: "myceliumVersion" }],
...
type MyceliumConfigV4 = {
  /**
   * Disable caching behavior
   */
  noCaching?: boolean;

  /** Maximum number of rendered previews to cache in Mycelium Engine.
   *  Note: this value is ignored when {@link MyceliumConfig.noCaching} is set to true.
  ...
  noTelemetry?: boolean;
  /**
   * Mycelium version. Setup by plugin
   @deprecated
   */
  version: number;
  /**
   ...
   * [Docs](https://mycelium.so/notes/24b176f1-685d-44e1-a1b0-1704b1a92ca0.html#specify-vault-location-when-creating-a-note)
   */
   ...
   */
  graph?: LegacyMyceliumGraphConfig;
  ...
  /** Notes that are too large can cause serious slowdowns for Mycelium. For
...
type MyceliumSiteConfig = {
   ...
   * Relative to Mycelium workspace
   */
  siteRootDir: string;
...
type LegacyMyceliumGraphConfig = {
  zoomSpeed: number;
};
...
type IntermediateNewConfig = Partial<
  Pick<MyceliumConfig, "commands" | "workspace" | "preview" | "publishing">
>;

type IntermediateOldConfig = Partial<MyceliumConfigV4> &
  Required<Pick<MyceliumConfigV4, "version">>;
...
export class DConfigLegacy {
  ...
  static v4ToV5(config: StrictConfigV4): MyceliumConfig {
...
const v4ToV5 = ({ legacyConfig }: { legacyConfig: any }) => {
  ...
  const rawMyceliumConfig = legacyConfig;
  ...
  const migratedConfig = _.defaultsDeep(cleanMyceliumConfig, defaultV5Config);
