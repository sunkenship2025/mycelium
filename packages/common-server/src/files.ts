 ...
 *
 * @throws a `MyceliumError` with `ERROR_SEVERITY.MINOR`. This is to avoid
 * crashing the Mycelium initialization, please catch the error and modify the
 * severity if needed.
 */
...
export function readString(path: string) {
  return readFileSync(path, "utf8") as Result<string, MyceliumError>;
}

export function readJson(path: string) {
  return fromPromise<AnyJson, MyceliumError>(fs.readJSON(path), (error) => {
    return new MyceliumError({
      message: `Cannot find ${path}`,
      severity: ERROR_SEVERITY.FATAL,
      ...(error instanceof Error && { innerError: error }),
