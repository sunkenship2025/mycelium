import { RespV3 } from "@myceliumhq/common-all";
...
export interface IBackupService {
   ...
   *
   * e.g.) Given `mycelium.yml`, timestamp, and infix `migrate-config`,
   * the resulting backup file name is `mycelium.2022.03.14.3239848.migrate-config.yml`
   *
   ...
   * @returns A promise of response containing either the path of the backup or a MyceliumError
   */
  backup(opts: {
    key: BackupKeyEnum;
    pathToBackup: string;
