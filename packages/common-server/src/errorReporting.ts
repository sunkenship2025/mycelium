import { MyceliumError, Stage } from "@myceliumhq/common-all";
...
export function rewriteFilename(filename: string) {
  ...
  // Remove everything including the mycelium directory, this is usually stuff like '/Users/someone/...'
  // We have to do two regexes because doing mycelium(\.[A-Za-z]*-[0-9.]*)? does not work properly
  const prodRegex = /^\/.*mycelium\.[A-Za-z_]*-[0-9.]*\//;
  const devRegex = /^\/.*mycelium\//;
  const prefix = "app:///";
...
export function eventModifier(
  ...
  if (error && error instanceof MyceliumError) {
