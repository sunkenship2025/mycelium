...
/** Mycelium should ignore any of these folders when watching or searching folders.
 *
 * These folders are unlikely to contain anything Mycelium would like to find, so we can ignore them.
 *
 * Example usage:
 * ```ts
...
async function _createFileWatcher(
  ...
  if (numTries <= 0) {
    throw new MyceliumError({ message: "exceeded numTries" });
  }
  ...
  static matchFilePrefix = async ({
    ...
    return new Promise((resolve) => {
      ...
      fileStream
        .on("error", (err) =>
          resolve({
            error: new MyceliumError({ innerError: err, message: "error" }),
          })
        )
...
export async function isSelfContainedVaultFolder(dir: string) {
  return _.every(
    await Promise.all([
      fs.pathExists(path.join(dir, CONSTANTS.MYCELIUM_CONFIG_FILE)),
