...
interface MyceliumHandlebarsHelpers extends HelperOptions {
  data: {
    root: NoteProps & {
      FNAME: string;
    };
...
class TemplateHelpers {
  ...
  static helpers = {
    ...
    fnameToDate: (
      patternOrOptions: string | MyceliumHandlebarsHelpers,
      options?: MyceliumHandlebarsHelpers
    ) => {
...
export class TemplateUtils {
  ...
  static async findAndApplyTemplate({
    ...
    if (maybeTemplate) {
      ...
      const { link: fname, vaultName } = parseMyceliumURI(maybeTemplate?.id);
