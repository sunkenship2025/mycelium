...
import { createLogger, findUpTo } from "@myceliumhq/common-server";
...
export class BuildUtils {
  ...
  static getPluginViewsRootPath() {
    return path.join(this.getLernaRoot(), "packages", "mycelium-plugin-views");
  }
  ...
  static installPluginLocally(version: string) {
    return Promise.all([
      $$(
        `code-insiders --install-extension "mycelium-${version}.vsix" --force`,
        { cwd: this.getPluginRootPath() }
      ),
      $$(`codium --install-extension "mycelium-${version}.vsix" --force`, {
        cwd: this.getPluginRootPath(),
      }),
    ]);
  }
  ...
  static async prepPluginPkg(target: ExtensionType = ExtensionType.MYCELIUM) {
    const pkgPath = path.join(this.getPluginRootPath(), "package.json");
    ...
    if (target === ExtensionType.NIGHTLY) {
      ...
      description =
        "This is a prerelease version of Mycelium that may be unstable. Please install the main mycelium extension instead.";
    ...
    this.updatePkgMeta({
      ...
      repository: {
        url: "https://github.com/myceliumhq/mycelium.git",
        type: "git",
      },
      version,
      icon,
    ...
    this.removeDevDepsFromPkgJson({
      pkgPath,
      dependencies: [
        "@myceliumhq/common-test-utils",
        "@myceliumhq/engine-test-utils",
        "vscode-test",
      ],
    });
  ...
  static async getIncrementedVerForNightly() {
    ...
    try {
      const extMetadata = await $$(`npx vsce show mycelium.nightly --json`, {
        cwd: this.getPluginRootPath(),
      });
  ...
  static async syncStaticAssetsToNextjsTemplate() {
    ...
    const templateAssetPath = path.join(templatePublicPath, "assets-mycelium");
  ...
  static async publishInsider() {
    ...
    const bucket = "org-mycelium-public-assets";
