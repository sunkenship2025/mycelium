...
export class VaultCLICommand extends CLICommand<CommandOpts> {
  ...
  buildArgs(args: yargs.Argv) {
    ...
    args.option("noAddToConfig", {
      describe: "if set, don't add vault to mycelium.yml",
      type: "boolean",
    });
    args.option("remoteUrl", {
      describe:
        "If converting to a remote vault, URL of the remote to use. Like https://github.com/myceliumhq/mycelium-site.git or git@github.com:myceliumhq/mycelium-site.git",
      type: "string",
    });
    args.option("type", {
    ...
    try {
      switch (cmd) {
        ...
        case VaultCommands.CONVERT: {
          // Find the full vault object because convert commands will need it to correctly edit `mycelium.yml`
