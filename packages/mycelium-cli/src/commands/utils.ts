...
import { createLogger, resolvePath } from "@myceliumhq/common-server";
import {
  MyceliumEngineV2,
  MyceliumEngineV3,
  DEngineClient,
...
export async function setupEngine(
  ...
  if (useLocalEngine) {
    const engine = newEngine
      ? MyceliumEngineV3.create({ wsRoot, logger })
      : MyceliumEngineV2.create({ wsRoot, logger });
...
export function setupEngineArgs(args: yargs.Argv) {
  args.option("enginePort", {
    describe:
      "If set, connect to to running engine. If not set, create new instance of Mycelium Engine",
  });
  args.option("attach", {
