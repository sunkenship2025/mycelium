...
import { MIGRATION_ENTRIES, WorkspaceUtils } from "@myceliumhq/engine-server";
...
> extends BaseCommand<TOpts, TOut> {
  ...
  setUpSegmentClient() {
    ...
    // if running CLI without ever having used mycelium plugin,
  ...
  async validateConfig(opts: { wsRoot: string }) {
    ...
    if (!validationResp.isValid) {
      ...
      const instruction =
        reason === "client"
          ? "Please make sure mycelium-cli is up to date by running the following: \n npm install @myceliumhq/mycelium-cli@latest"
          : `Please make sure mycelium.yml is up to date by running the following: \n mycelium dev run_migration --migrationVersion=${MIGRATION_ENTRIES[0].version}`;
      const clientVersionOkay =
        reason === "client" ? MYCELIUM_EMOJIS.NOT_OKAY : MYCELIUM_EMOJIS.OKAY;
      const configVersionOkay =
        reason === "config" ? MYCELIUM_EMOJIS.NOT_OKAY : MYCELIUM_EMOJIS.OKAY;
  ...
  eval = async (args: any) => {
    ...
    if (out.error instanceof MyceliumError && out.error) {
