...
type CommandOutput = Partial<{ error: MyceliumError; data: any }>;
...
/**
 * To use when working on mycelium
 */
export class DevCLICommand extends CLICommand<CommandOpts, CommandOutput> {
  constructor() {
    super({
      name: "dev <cmd>",
      desc: "commands related to development of Mycelium",
    });
  ...
  buildArgs(args: yargs.Argv) {
    ...
    args.option("extensionType", {
      describe:
        "extension name to publish in the marketplace (Mycelium / Nightly)",
    ...
    args.option("wsRoot", {
      describe: "root directory of the Mycelium workspace",
    });
  ...
  syncTutorial() {
    const myceliumSiteVaultPath = path.join(
    ...
    // wipe everything in /assets/mycelium-ws/tutorial/treatments
    ...
    const tutorialNotePaths = fs
      .readdirSync(myceliumSiteVaultPath)
      .filter((basename) => {
        return (
    ...
    treatmentNames.forEach((treatmentName) => {
      ...
      tutorialNotePaths
        ...
        .forEach((basename) => {
          const src = path.join(myceliumSiteVaultPath, basename);
  ...
  enableTelemetry() {
    ...
    const message = [
      ...
      "Thank you for helping us improve Mycelium 🌱",
    ].join("\n");
    this.print(message);
  }
  ...
  showMigrations() {
    const headerMessage = [
      ...
      "",
      "e.g.)",
      "> mycelium dev run_migration --migrationVersion=0.64.1",
      "",
    ].join("\n");
    const body: string[] = [];
    let maxLength = 0;
  ...
  async runMigration(opts: CommandOpts) {
    ...
    const myceliumConfig = readYAML(configPath);
