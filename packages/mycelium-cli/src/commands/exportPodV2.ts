...
import { EngineUtils, openPortFile } from "@myceliumhq/engine-server";
...
> {
    ...
    switch (config.podType) {
      case PodV2Types.MarkdownExportV2:
        return new MarkdownExportPodV2({
          podConfig: config,
          engine,
          myceliumConfig: DConfig.readConfigSync(engine.wsRoot),
        });
  ...
  async execute(opts: CommandOpts): Promise<CommandOutput> {
    ...
    return new Promise((resolve) => {
      server.close((err: any) => {
        ...
        if (err) {
          return resolve({
            error: new MyceliumError({ message: "error closing", payload: err }),
          });
        }
  ...
  multiNoteExportCheck(opts: {
    ...
    if (
      ...
      throw new MyceliumError({
        message:
          "Multi Note Export cannot have clipboard as destination. Please configure your destination by using Mycelium: Configure Export Pod V2 command",
      });
    }
  }
}
