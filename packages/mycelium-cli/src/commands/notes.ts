...
import { MarkdownPublishPod } from "@myceliumhq/pods-core";
...
export enum NoteCLIOutput {
  JSON = "json",
  MARKDOWN_GFM = "md_gfm",
  MARKDOWN_MYCELIUM = "md_mycelium",
}

type CommandOpts = CommandCLIOpts & SetupEngineResp & CommandCommonProps;

type CommandOutput = { data: any; error?: MyceliumError };
...
async function formatNote({
  ...
  switch (output) {
    case NoteCLIOutput.JSON:
      ...
      payload = note;
      break;
    case NoteCLIOutput.MARKDOWN_MYCELIUM:
      ...
      switch (cmd) {
        ...
        case NoteCommands.MOVE: {
          ...
          if (note) {
            ...
            throw new MyceliumError({ message: `note ${fname} not found` });
          }
        }
        default: {
