import { launchv2 } from "@myceliumhq/api-server";
import { ConfigUtils } from "@myceliumhq/common-all";
import { LogLvl, resolvePath } from "@myceliumhq/common-server";
import {
...
type CommandCLIOnlyOpts = {
  /**
   *
   * Whether Mycelium should write the port to the * {@link file | https://wiki.mycelium.so/notes/446723ba-c310-4302-a651-df14ce6e002b.html#mycelium-port-file }
   */
  noWritePort?: boolean;
};
type CommandCLIOpts = {
...
> {
  constructor() {
    super({
      ...
      desc: "launch instance of mycelium engine",
    });
  }
  ...
  async enrichArgs(args: CommandCLIOpts) {
    ...
    const engine = MyceliumEngineClient.create({
      port: _port,
      vaults,
      ws: wsRoot,
    });
