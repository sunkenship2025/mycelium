...
import { SiteUtils } from "@myceliumhq/engine-server";
import {
  NextjsExportConfig,
  NextjsExportPod,
...
import ora from "ora";
import { DConfig, GitUtils } from "@myceliumhq/common-server";
...
export enum PublishCommands {
  /**
   * Initiliaze the nextjs-template from Mycelium in the mycelium workspace
   */
  INIT = "init",
  /**
   * Create metadata needed to builid mycelium nextjs template
   */
  BUILD = "build",
  /**
   * Builds the website
...
const isBuildOverrideKey = (key: string): key is keyof BuildOverrides => {
  const allowedKeys = [
    "siteUrl",
    "assetsPrefix",
  ] as (keyof MyceliumPublishingConfig)[];
...
export class PublishCLICommand extends CLICommand<CommandOpts, CommandOutput> {
  ...
  buildArgs(args: yargs.Argv) {
    ...
    args.option("attach", {
      describe: "use existing mycelium engine instead of spawning a new one",
      type: "boolean",
    });
    args.option("noBuild", {
  ...
  async _buildNextData({
    ...
    if (stage !== "prod") {
      if (!publishingConfig.siteUrl && !overrides?.siteUrl) {
        _.set(
          opts.config.overrides as Partial<MyceliumPublishingConfig>,
          "siteUrl",
          "localhost:3000"
        );
      }
