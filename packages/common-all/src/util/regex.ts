...
/** Returns true if this is a non-mycelium uri, false if it is mycelium://, undefined if it's not a URI */
export const containsNonMyceliumUri = (uri: string): boolean | undefined => {
  const groups = uriRegex.exec(uri)?.groups;
  if (_.isUndefined(groups) || _.isUndefined(groups.scheme)) return undefined;
  if (groups.scheme === "mycelium") return false;
  return true;
};
