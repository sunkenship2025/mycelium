...
export class VaultUtils {
  ...
  static getVaultByNameOrThrow({
    ...
    if (!vault) {
      throw new MyceliumError({ message: `vault with name ${vname} not found` });
    }
    return vault;
  }
  ...
  static toURIPrefix(vault: DVault) {
    return CONSTANTS.MYCELIUM_DELIMETER + VaultUtils.getName(vault);
  }
  ...
  /** Creates a dummy vault for files that are not in Mycelium workspace, for example a markdown file that's not in any vault. Do not use for notes. */
