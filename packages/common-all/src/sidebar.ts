...
import type { IMyceliumError } from "./error";
import { MyceliumError, assertUnreachable } from "./error";
import { ERROR_STATUS } from "./constants";

export type SidebarResult<T> = Result<T, IMyceliumError>;
...
export function processSidebar(
  ...
  const safeProcessItem = fromThrowable(processItem, (error: unknown) =>
    MyceliumError.isMyceliumError(error)
      ? error
...
export function parseSidebarConfig(
  input: unknown
): Result<SidebarConfig, IMyceliumError> {
  ...
  return err(new MyceliumError({ message: "Sidebar object is not an array" }));
}
