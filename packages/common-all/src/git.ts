...
import type { DVault, MyceliumConfig, NoteProps } from "./types";
...
export function githubUrl(opts: { note: NoteProps; config: MyceliumConfig }) {
...
export function git2Github(gitUrl: string) {
  // 'git@github.com:kevinslin/mycelium-vault.git'
  // @ts-ignore
 ...
 * `github.com/myceliumhq/mycelium-site`. For more details see the
 * [[Self Contained Vaults RFC|mycelium://mycelium.docs/rfc.42-self-contained-vaults]]
 */
export function remoteUrlToDependencyPath({
  ...
  // https://github.com/myceliumhq/mycelium-site.git This may also look like
  // http://example.com:8000/myceliumhq/mycelium-site.git, we skip the port
  const webMatch =
    ...
    // then we have the path of the URL, like myceliumhq/mycelium-site
  ...
  // git@github.com:myceliumhq/mycelium-site.git This may also look like
  // git@example.com:220/myceliumhq/mycelium-site.git, we skip the port and the
  const sshMatch =
