...
import { MYCELIUM_CONFIG } from "../constants/configs/myceliumConfig";
import { MyceliumError, ErrorMessages, IMyceliumError } from "../error";
...
export class ConfigUtils {
  static genDefaultConfig(): MyceliumConfig {
    const common = {
      dev: {
   ...
   */
  static genLatestConfig(defaults?: DeepPartial<MyceliumConfig>): MyceliumConfig {
    const common = {
      dev: {
        enablePreviewV2: true,
      },
  ...
  // get
  static getProp<K extends keyof MyceliumConfig>(
  ...
  static getCommands(config: MyceliumConfig): MyceliumCommandConfig {
    return ConfigUtils.getProp(config, "commands");
  }

  static getWorkspace(config: MyceliumConfig): MyceliumWorkspaceConfig {
    return ConfigUtils.getProp(config, "workspace");
  }

  static getPreview(config: MyceliumConfig): MyceliumPreviewConfig {
    const out = ConfigUtils.getProp(config, "preview");
    // FIXME: for some reason, this can return undefined when run in context of chrome in `mycelium-plugin-views`
  ...
  static getPublishing(config: MyceliumConfig): MyceliumPublishingConfig {
    return ConfigUtils.getProp(config, "publishing");
  }

  static getVaults(config: MyceliumConfig): DVault[] {
    return ConfigUtils.getWorkspace(config).vaults;
  }

  static getHooks(config: MyceliumConfig): DHookDict | undefined {
    return ConfigUtils.getWorkspace(config).hooks;
  }

  static getJournal(config: MyceliumConfig): JournalConfig {
    return ConfigUtils.getWorkspace(config).journal;
  }

  static getScratch(config: MyceliumConfig): ScratchConfig {
    return ConfigUtils.getWorkspace(config).scratch;
  }

  static getTask(config: MyceliumConfig): TaskConfig {
    return ConfigUtils.getWorkspace(config).task;
  }

  static getLookup(config: MyceliumConfig): LookupConfig {
  ...
  static getGithubConfig(config: MyceliumConfig): GithubConfig | undefined {
    return ConfigUtils.getPublishing(config).github;
  }

  static getGiscusConfig(config: MyceliumConfig): GiscusConfig | undefined {
    return ConfigUtils.getPublishing(config).giscus;
  }

  static getLogo(config: MyceliumConfig): string | undefined {
    return ConfigUtils.getPublishing(config).logoPath;
  }

  static getAssetsPrefix(config: MyceliumConfig): string | undefined {
  ...
  static getEnablePrettlyLinks(config: MyceliumConfig): boolean | undefined {
    return ConfigUtils.getPublishing(config).enablePrettyLinks;
  }

  static getGATracking(config: MyceliumConfig): string | undefined {
    return ConfigUtils.getPublishing(config).ga?.tracking;
  }

  static getSiteLastModified(config: MyceliumConfig): boolean | undefined {
    return ConfigUtils.getPublishing(config).enableSiteLastModified;
  }

  static getSiteLogoUrl(config: MyceliumConfig): string | undefined {
  ...
  static getEnableBackLinks(
    ...
    if (
      ConfigUtils.isMyceliumPublishingConfig(publishConfig) &&
  ...
  static getHierarchyDisplayConfigForPublishing(config: MyceliumConfig) {
  ...
  static getNonNoteLinkAnchorType(config: MyceliumConfig) {
    return (
      this.getCommands(config).copyNoteLink.nonNoteFile?.anchorType || "block"
    );
  }

  static getAliasMode(config: MyceliumConfig) {
    return this.getCommands(config).copyNoteLink.aliasMode;
  }

  static getVersion(config: MyceliumConfig): number {
    return config.version;
  }

  static getSearchMode(config: MyceliumConfig): SearchMode {
    const defaultMode = ConfigUtils.getPublishing(config).searchMode;
    return defaultMode || SearchMode.LOOKUP;
  }
  // set
  static setProp<K extends keyof MyceliumConfig>(
    config: MyceliumConfig,
    key: K,
    value: MyceliumConfig[K]
  ): void {
    _.set(config, key, value);
  }

  static setCommandsProp<K extends keyof MyceliumCommandConfig>(
    config: MyceliumConfig,
    key: K,
    value: MyceliumCommandConfig[K]
  ) {
    const path = `commands.${key}`;
    _.set(config, path, value);
  }

  static setWorkspaceProp<K extends keyof MyceliumWorkspaceConfig>(
    config: MyceliumConfig,
    key: K,
    value: MyceliumWorkspaceConfig[K]
  ) {
    const path = `workspace.${key}`;
    _.set(config, path, value);
  }

  static setPublishProp<K extends keyof MyceliumPublishingConfig>(
    config: MyceliumConfig,
    key: K,
    value: MyceliumPublishingConfig[K]
  ) {
  ...
  static isMyceliumPublishingConfig(
    config: unknown
  ): config is MyceliumPublishingConfig {
    return _.has(config, "enableBackLinks");
  }

  static overridePublishingConfig(
    config: MyceliumConfig,
    value: MyceliumPublishingConfig
  ) {
    return {
      ...config,
      publishing: value,
  ...
  static unsetProp<K extends keyof MyceliumConfig>(
    config: MyceliumConfig,
    key: K
  ) {
    _.unset(config, key);
  ...
  static unsetPublishProp<K extends keyof MyceliumPublishingConfig>(
    config: MyceliumConfig,
    key: K
  ) {
    const path = `publishing.${key}`;
  ...
  static unsetDuplicateNoteBehavior(config: MyceliumConfig): void {
    ConfigUtils.unsetPublishProp(config, "duplicateNoteBehavior");
  }

  static setVaults(config: MyceliumConfig, value: DVault[]): void {
  ...
  static setHooks(config: MyceliumConfig, value: DHookDict) {
    ConfigUtils.setWorkspaceProp(config, "hooks", value);
  }

  static setPreviewProps<K extends keyof MyceliumPreviewConfig>(
    config: MyceliumConfig,
    key: K,
    value: MyceliumPreviewConfig[K]
  ) {
  ...
  static setAliasMode(config: MyceliumConfig, aliasMode: "title" | "none") {
  ...
  static detectMissingDefaults(opts: {
    ...
    defaultConfig?: MyceliumConfig;
  }): {
    needsBackfill: boolean;
    backfilledConfig: MyceliumConfig;
  } {
    const { config } = opts;
  ...
  static getConfigDescription = (conf: string) => {
    return _.get(MYCELIUM_CONFIG, conf)?.desc;
  };
   ...
   */
  static findDifference(opts: { config: MyceliumConfig }) {
    const { config } = opts;
  ...
  /**
   * Parses an unkown input into a MyceliumConfig
   * @param input
   */

  static parse(input: unknown): Result<MyceliumConfig, IMyceliumError> {
    const schema = schemaForType<{ publishing: MyceliumPublishingConfig }>()(
      z
        .object({
          version: z.number(),
          dev: z.object({}).passthrough().optional(), // TODO MyceliumDevConfig;
          commands: z.object({}).passthrough(), // TODO MyceliumCommandConfig;
          workspace: z.object({}).passthrough(), // TODO MyceliumWorkspaceConfig;
          preview: z.object({}).passthrough(), // TODO MyceliumPreviewConfig;
          publishing: publishingSchema,
          global: z.object({}).passthrough().optional(), // TODO MyceliumGlobalConfig;
        })
        .passthrough()
    );

    return parse(schema, input, "Invalid Mycelium Config").map((value) => {
...
export function normalizeUnixPath(fsPath: string): string {
  return path.posix
    ...
    : normalizePath(fsPath); // `path.posix` might be not available depending on your build system. For example at the time of writing `mycelium-plugin-views` does not implement a `posix` property.
}
