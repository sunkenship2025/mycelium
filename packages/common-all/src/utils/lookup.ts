...
export class NoteLookupUtils {
  ...
  /**
   * The core of Mycelium lookup logic
   */
  static async lookup({
    qsRaw,
    engine,
  ...
  /**
   * Transform Mycelium lookup syntax to fusejs syntax
...
function wikiTransform(trimmedQuery: string): TransformedQueryString {
  ...
  if (transformed.includes("mycelium://")) {
    // https://regex101.com/r/ICcyK6/1/
    vaultName = transformed.match(/mycelium:\/\/(.*?)\//)?.[1];
