...
import { MyceliumCompositeError, MyceliumError } from "./error";
import {
  BulkWriteNotesResp,
  DeleteNoteResp,
  DeleteSchemaResp,
...
export interface IAPIPayload {
  ...
  error: undefined | MyceliumError | MyceliumCompositeError;
}
...
abstract class API {
  ...
  async _makeRequestRaw(args: IDoRequestArgs) {
    try {
      const resp = await this._doRequest(args);
      if (resp.data.error) {
        return new MyceliumError({ ...resp.data.error });
      }
      return resp.data;
    } catch (err: any) {
      return new MyceliumError({ ...err.response.data.error });
    }
  }
}
...
let _MyceliumAPI_INSTANCE: MyceliumAPI | undefined;

export class MyceliumAPI extends API {
  static getOrCreate(opts: IAPIConstructor) {
    if (!_.isUndefined(_MyceliumAPI_INSTANCE)) {
      return this.instance();
    }
    return new MyceliumAPI(opts);
  }

  static instance(): MyceliumAPI {
    if (_.isUndefined(_MyceliumAPI_INSTANCE)) {
      throw Error("no mycelium api");
    }
    // eslint-disable-next-line camelcase
    return _MyceliumAPI_INSTANCE;
  }

  assetGet(req: AssetGetRequest): Promise<MyceliumError | Buffer> {
  ...
  assetGetTheme(req: AssetGetThemeRequest): Promise<MyceliumError | Buffer> {
    return this._makeRequestRaw({
      path: "assets/theme",
      method: "get",
      qs: req,
  ...
  configGet(req: WorkspaceRequest): Promise<RespV3<MyceliumConfig>> {
...
export const MyceliumApiV2 = MyceliumAPI;
