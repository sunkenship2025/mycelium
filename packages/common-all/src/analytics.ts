...
export enum NativeWorkspaceEvents {
  DetectedInNonMyceliumWS = "Native_Workspace_Detected_In_Non_Mycelium_WS", // watcher has detected a Mycelium workspace getting created inside a non-Mycelium workspace
}
...
type UserProfileProps = {
  ...
  role?: string;
  /**
   * The use case of Mycelium for the user. Retrieved from initial survey.
   */
  useCases?: string[];
  /**
   * The context Mycelium is used for the user. Retrieved from initial survey.
   */
  useContext?: string;
  /**
  ...
  /**
   * Prior tools the user has used before Mycelium. Retrieved from initial survey.
   */
  priorTools?: string[];
  /**
  ...
  /**
   * Did the user express interest in using Mycelium for a team?
   */
  teamIntent?: boolean;
};
