...
export class DNodeUtils {
  ...
  static findClosestParent(
    ...
    if (dirname === "") {
      ...
      if (_.isUndefined(_node)) {
        throw new MyceliumError({ message: `no root found for ${fpath}` });
      }
      return _node;
    }
  ...
  /**
   * Custom props are anything that is not a reserved key in Mycelium
   * @param props
   * @returns
   */
...
export class NoteUtils {
  ...
  static addOrUpdateParents(opts: {
    ...
    if (!parent && !createStubs) {
      ...
      throw MyceliumError.createFromStatus(err);
    }
    if (!parent) {
  ...
  static createWikiLink(opts: {
    ...
    const vaultPrefix = useVaultPrefix
      ? `${CONSTANTS.MYCELIUM_DELIMETER}${VaultUtils.getName(vault)}/`
      : "";
  ...
  /** This should be only used for files not in Mycelium workspace, for example a markdown file that's not in any vault. */
  static genIdForFile({
    filePath,
    wsRoot,
  }: {
