...
import { isNotUndefined, TreeMenuNode, TreeUtils } from "@myceliumhq/common-all";
import { createLogger } from "@myceliumhq/common-frontend";
...
export default function MyceliumTreeMenu(
  ...
  const tree = ide.tree;
  const logger = createLogger("MyceliumTreeMenu");
  const myceliumRouter = useMyceliumRouter();
  const [activeNoteIds, setActiveNoteIds] = useState<string[]>([]);
  const noteActiveId = _.isUndefined(myceliumRouter.query.id)
    ? props.noteIndex?.id
    : myceliumRouter.query.id;
  ...
  useEffect(() => {
    ...
    return undefined;
  }, [props.noteIndex, myceliumRouter.query.id, noteActiveId, tree]);
...
function MenuView({
    ...
    if (menu.children && menu.children.length > 0) {
      return (
        ...
        <SubMenu
          ...
          icon={menu.icon}
          className={
            menu.key === activeNote ? "mycelium-ant-menu-submenu-selected" : ""
          }
          key={menu.key}
          title={
            <MenuItemTitle
  ...
  return (
    ...
    <Menu
      ...
      className="mycelium-tree-menu"
      mode="inline"
      {...(!collapsed && {
        openKeys: expandKeys,
        selectedKeys: [...expandKeys, activeNote],
      })}
      inlineIndent={MYCELIUM_STYLE_CONSTANTS.SIDER.INDENT}
      // @ts-ignore
...
function MenuItemTitle(
  ...
  const { getNoteUrl } = useMyceliumRouter();
