...
import { LoadingStatus } from "@myceliumhq/common-frontend";
...
import { MyceliumCommonProps, verifyNoteData } from "../utils/types";
import MyceliumSpinner from "./MyceliumSpinner";
import { useMyceliumLookup, useNoteActive, useNoteBodies } from "../utils/hooks";
...
export function MyceliumSearch(props: MyceliumCommonProps) {
  return <DebouncedMyceliumSearchComponent {...props} />;
}
...
function DebouncedMyceliumSearchComponent(props: MyceliumCommonProps) {
  // Splitting this part from MyceliumSearchComponent so that the debounced
  ...
  const debouncedSearch: SearchFunction | undefined = fuse
    ...
    : undefined;
  return (
    <MyceliumSearchComponent {...props} {...results} search={debouncedSearch} />
  );
}
...
function MyceliumSearchComponent(props: MyceliumCommonProps & SearchProps) {
  const { noteIndex, myceliumRouter, search, error, notes } = props;
  ...
  const lookup = useMyceliumLookup(props.notes);
  ...
  const onSelect = useCallback(
    ...
    (_selection, option) => {
      ...
      myceliumRouter.changeActiveNote(id, { noteIndex });
      dispatch(
        browserEngineSlice.actions.setLoadingStatus(LoadingStatus.PENDING)
      );
      setSearchQueryValue("");
    },
    [myceliumRouter, dispatch, noteIndex]
  );
