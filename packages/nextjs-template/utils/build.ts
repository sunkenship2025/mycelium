...
export interface MyceliumNotePageParams extends ParsedUrlQuery {
  id: string;
}
/**
 ...
 * @returns
 */
export function getNotePaths(): GetStaticPathsResult<MyceliumNotePageParams> {
...
let _CONFIG_CACHE: MyceliumConfig | undefined;
export function getConfig(): Promise<MyceliumConfig> {
  if (_.isUndefined(_CONFIG_CACHE)) {
    const dataDir = getDataDir();
    return fs.readJSON(path.join(dataDir, "mycelium.json"));
  }
