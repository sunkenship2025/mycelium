...
import { verifyEngineSliceState } from "@myceliumhq/common-frontend";
...
export type MyceliumRouterProps = ReturnType<typeof useMyceliumRouter>;

export function getNoteRouterQuery(router: NextRouter) {
  return router.query as Partial<NoteRouterQuery>;
}

export function useMyceliumRouter() {
 ...
 * @param setNoteIndex
 */
export function useMyceliumLookup(notes?: NotePropsByIdDict) {
  const engine = useEngineAppSelector((state) => state.engine);
  const config = engine.config as MyceliumConfig;
 ...
 * @returns
 */
export function useNoteActive(id: string | undefined): MyceliumNotesHookProps {
