...
import { useMyceliumConfig } from "../hooks/useMyceliumConfig";
...
function useFuse(
  ...
  const myceliumConfig = useMyceliumConfig();
  const fuzzThreshold = myceliumConfig
    ? ConfigUtils.getLookup(myceliumConfig).note.fuzzThreshold
    : 0.6;
      ...
      const req = async () => {
        ...
        try {
          ...
          if (_.isUndefined(value)) {
            ...
            setError(
              new MyceliumError({ message: "loaded index is undefined" })
            );
          }
        } catch (error) {
          setError(error);
