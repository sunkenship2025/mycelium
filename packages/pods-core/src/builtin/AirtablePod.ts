import Airtable, { FieldSet, Records } from "@myceliumhq/airtable";
import {
  asyncLoopOneAtATime,
...
import { createLogger, DLogger } from "@myceliumhq/common-server";
import {
  NoteMetadataUtils,
...
export class AirtableUtils {
  static addRequiredFields(mapping: { [key: string]: SrcFieldMapping }) {
    const _map = { ...mapping };
    _map["MyceliumId"] = { type: "string", to: "id" };
    return _map;
  }
        ...
        try {
          ...
          if (ErrorUtils.isAxiosError(error)) {
            ...
            if (
              ...
              _error = new MyceliumError({
                message: error.response.data.message,
                payload,
                severity: ERROR_SEVERITY.MINOR,
              });
            } else {
              _error = new MyceliumError({ message: "axios error", payload });
            }
          } else {
            payload = _.merge(payload, error);
            _error = new MyceliumError({ message: "general error", payload });
          }
          throw _error;
        }
      })
      ...
      default:
        return {
          error: new MyceliumError({
            message: `The type ${type} provided in srcFieldMapping is invalid. Please check the valid types here: https://wiki.mycelium.so/notes/oTW7BFzKIlOd6iQnnNulg.html#sourcefieldmapping and update the pod config by running Mycelium: Configure Export Pod V2 command.`,
            severity: ERROR_SEVERITY.MINOR,
          }),
        };
    }
  ...
  static async notesToSrcFieldMap(opts: {
    ...
    if (!_.isEmpty(errors)) {
      return { error: new MyceliumCompositeError(errors) };
    }
  ...
  static async updateAirtableIdForNewlySyncedNotes({
    ...
    const out = await Promise.all(
      records.map(async (ent) => {
        ...
        const myceliumId = ent.fields["MyceliumId"] as string;
        const resp = await engine.getNote(myceliumId);
        if (!resp.data) {
          return undefined;
        }
