...
import { execa, SiteUtils } from "@myceliumhq/engine-server";
import {
...
const ID = "mycelium.nextjs";

const TEMPLATE_REMOTE = "origin";
const TEMPLATE_REMOTE_URL = "https://github.com/myceliumhq/nextjs-template.git";
const TEMPLATE_BRANCH = "main";

const $$ = execa.command;

type NextjsExportPodCustomOpts = {
  overrides?: Partial<MyceliumPublishingConfig>;
};

export type BuildOverrides = Pick<MyceliumPublishingConfig, "siteUrl">;
...
async function validateSiteConfig({
  ...
  if (ConfigUtils.isMyceliumPublishingConfig(config)) {
...
export class NextjsExportPodUtils {
  ...
  static getMyceliumConfigPath = (dest: URI) => {
    const podDstDir = path.join(dest.fsPath, "data");
    const podConfigDstPath = path.join(podDstDir, "mycelium.json");
...
export class NextjsExportPod extends ExportPod<NextjsExportConfig> {
  ...
  async plant(opts: NextjsExportPlantOpts) {
    ...
    const engineConfig: MyceliumConfig = ConfigUtils.overridePublishingConfig(
      config,
      siteConfig
    );
