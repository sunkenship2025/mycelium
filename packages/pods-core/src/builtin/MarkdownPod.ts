...
export class MarkdownImportPod extends ImportPod<MarkdownImportPodConfig> {
  ...
  get config(): JSONSchemaType<MarkdownImportPodConfig> {
    return PodUtils.createImportConfig({
      ...
      properties: {
        ...
        frontmatterMapping: {
          description:
            "An optional set of variable mappings, with the key being the variable name in the import source and the value being the resulting variable name in Mycelium. See https://wiki.mycelium.so/notes/f23a6290-2dec-45dc-b616-c218ee53db6b.html for examples.",
          type: "object",
          nullable: true,
        },
      },
  ...
  /** Cleans up a link following Mycelium best practices, converting slashes to dots and spaces to dashes. */
  ...
  static async updateAssetLinks({
    ...
    const assetReferences = [
      ...selectAll(MyceliumASTTypes.IMAGE, tree),
      ...selectAll(MyceliumASTTypes.LINK, tree),
   ...
   * In case of conflict in keys of mycelium and imported note, checks frontmatterMapping provided in the
    ...
    const notesClean = await Promise.all(
      notes
        ...
        .map(async (note) => {
          ...
          const tree = proc.parse(note.body) as MyceliumASTNode;
...
export class MarkdownPublishPod extends PublishPod<MarkdownPublishPodConfig> {
  ...
  async plant(opts: PublishPodPlantOpts) {
    const { engine, note, config, myceliumConfig } = opts;
    ...
    let remark = MDUtilsV5.procRemarkFull({
      ...
      dest: MyceliumASTDest.MD_REGULAR,
      config: {
        ...rawConfig,
        preview: {
          ...rawConfig.preview,
    ...
    if (wikiLinkToURL && !_.isUndefined(myceliumConfig)) {
      remark = remark.use(
        RemarkUtils.convertWikiLinkToNoteUrl(note, [], engine, myceliumConfig)
      );
    } else {
