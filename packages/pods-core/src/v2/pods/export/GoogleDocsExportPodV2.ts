  ...
  implements ExportPodV2<GoogleDocsExportReturnType>
{
  ...
  getPayloadForNotes(notes: NoteProps[]): Promise<GoogleDocsPayload[]> {
    return Promise.all(
      notes.map(async (input) => {
        ...
        const blockquote = `<blockquote>This note was exported from [[${input.fname}]] with <a href="https://mycelium.so/">Mycelium</a></blockquote>`;
  ...
  async createGdoc(opts: {
  ...
  }): Promise<{ data: GoogleDocsFields[]; errors: IMyceliumError[] }> {
    ...
    const out: GoogleDocsFields[] = await Promise.all(
      docToCreate.map(async ({ name, content, myceliumId }) => {
        ...
        try {
          ...
          errors.push(err as MyceliumError);
          return;
        }
      })
    );
  ...
  async overwriteGdoc(opts: {
    ...
    const errors: IMyceliumError[] = [];
    const out: GoogleDocsFields[] = await Promise.all(
      docToUpdate.map(async ({ content, documentId, myceliumId }) => {
  ...
  async getRevisionId(opts: { accessToken: string; documentId: string }) {
    ...
    try {
      ...
      throw new MyceliumError({ message: stringifyError(err) });
    }
  }
  ...
  static async updateNotesWithCustomFrontmatter(
    ...
    await Promise.all(
      records.map(async (record) => {
        ...
        const { documentId, revisionId, myceliumId } = record;
        if (!myceliumId) return;
