  ...
  implements ExportPodV2<MarkdownExportReturnType>
{
  ...
  private _myceliumConfig: MyceliumConfig;
  ...
  constructor({
    ...
    this._engine = engine;
    this._myceliumConfig = myceliumConfig;
  }
  ...
  async renderNote({
    ...
    if (this._config.wikiLinkToURL && !_.isUndefined(this._myceliumConfig)) {
      remark = remark.use(
        RemarkUtils.convertWikiLinkToNoteUrl(
          note,
          [],
          this._engine,
          this._myceliumConfig
        )
      );
    } else {
