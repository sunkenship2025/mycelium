...
import { MyceliumComponent, MyceliumProps } from "../types";
import { useWorkspaceProps } from "../hooks";
import { engineHooks } from "@myceliumhq/common-frontend";
import {
  ConfigureUIMessage,
...
import { myceliumConfig, getSortedConfig } from "../utils/myceliumConfig";

const MyceliumConfigure: MyceliumComponent = ({ engine }: MyceliumProps) => {
  ...
  useEffect(() => {
    ...
    Object.keys(myceliumConfig).forEach((key) => {
      myceliumConfig[key].default = _.get(engine.config, key);
    });
  }, [engine.config]);
  ...
  const items = getSortedConfig().map((conf) => {
    return {
      key: conf,
      label: _.startCase(myceliumConfig[conf].group),
    };
  });
  ...
  const postMessage = ({ key, value }: { key: string; value: any }) => {
    ...
    myceliumConfig[key].default = value;
    setConfig(newConf);
    postVSCodeMessage({
        ...
        return (
          ...
          >
            <Typography.Paragraph>
              {myceliumConfig[conf]?.type !== "boolean"
                ...
                >
                  {getSortedConfig()
                    ...
                    .map((conf) => {
                      ...
                      return (
                        <>
                          {!configGroupMap.has(myceliumConfig[conf]?.group) &&
                          configGroupMap.set(myceliumConfig[conf]?.group, "") ? (
                            <Divider orientation="left">
                              <Typography.Title
                                level={2}
                                id={myceliumConfig[conf]?.group}
                              >
                                {_.startCase(myceliumConfig[conf]?.group)}
                              </Typography.Title>
                            </Divider>
                          ) : null}
                          {conf.split(".").length > 2 ? (
...
export default MyceliumConfigure;
