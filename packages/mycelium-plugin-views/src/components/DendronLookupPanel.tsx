import { MyceliumProps } from "../types";
...
import { ideSlice } from "@myceliumhq/common-frontend";

const { Option } = Select;

export default function MyceliumLookupPanel({ ide }: MyceliumProps) {
...
function MyceliumGreenTooltip(props: React.PropsWithChildren<any>) {
  return (
    <>
      <Tooltip
        color={MYCELIUM_COLORS["mycelium green"]}
        placement="topLeft"
        title={props.title}
      >
        {props.children}
...
function OptionWithTooltip(key: string) {
  return (
    <>
      <Option value={key}>
        <MyceliumGreenTooltip title={MODIFIER_DESCRIPTIONS[key]}>
          {key}
        </MyceliumGreenTooltip>
      </Option>
    </>
  );
}
