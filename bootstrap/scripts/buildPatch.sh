...
if [ -z $FAST ]; then
	LOG_LEVEL=info $MYCELIUM_CLI dev build --upgradeType $UPGRADE_TYPE --publishEndpoint $PUBLISH_ENDPOINT --extensionType $EXT_TYPE
else
	echo "running fast mode..."
	SKIP_SENTRY=1 LOG_LEVEL=info $MYCELIUM_CLI dev build --upgradeType $UPGRADE_TYPE --publishEndpoint $PUBLISH_ENDPOINT --fast --extensionType $EXT_TYPE
fi
