...
### Look up note
* exact match: `mycelium.welcome`
* by tokens (space works as AND): `mycelium welcome`
* by tokens (| is OR): `bar | foo`

### Hierarchy with omitted path match
* `mycelium.multi` should match among others:
    * `mycelium.ref.frontmatter-tags.multi-array`
    
### Direct child look up:
1. Turn on direct child look up 
![](assets/images/Screen_Shot_2021-10-14_at_9.05.21_PM.png)

2. Look up `mycelium.blog.` Make sure only its children are shown. 

### Go Down command
1. Navigate to `mycelium.blog` 
2. Activate `Go Down` command
3. Make sure only the children of `mycelium.blog` are shown.

### Just activated lookup
1. Navigate to `mycelium.blog.one`
1. Activate `Note lookup`
1. Make sure to be able to see siblings of `mycelium.blog.one` such as ``mycelium.blog.two`.
...
* [[with description with header with vault|mycelium://vault/mycelium.welcome#header1]]
* [[with description with header|mycelium.welcome#header1]]
* [[with description|mycelium.welcome]]
* [[mycelium.welcome]]

## Create notes look up related
![[mycelium.note-create#simple-case,1:#*]]

![[mycelium.note-create#with-matching-simple-schema,1]]
